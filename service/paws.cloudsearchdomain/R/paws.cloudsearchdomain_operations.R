...
#' 
#' `{ "actors": {}, "title": {"format": "text","max_phrases": 2,"pre_tag": "","post_tag": ""} }","post_tag": ""} }`
...
#'
#' @export
search <- function (query, cursor = NULL, expr = NULL, facet = NULL, 
    filterQuery = NULL, highlight = NULL, partial = NULL, queryOptions = NULL, 
    queryParser = NULL, return = NULL, size = NULL, sort = NULL, 
    start = NULL, stats = NULL) 
{
    op <- Operation(name = "Search", http_method = "GET", http_path = "/2013-01-01/search?format=sdk&pretty=true", 
        paginator = list())
    input <- search_input(query = query, cursor = cursor, expr = expr, 
        facet = facet, filterQuery = filterQuery, highlight = highlight, 
        partial = partial, queryOptions = queryOptions, queryParser = queryParser, 
        return = return, size = size, sort = sort, start = start, 
        stats = stats)
...
#'
#' @export
suggest <- function (query, suggester, size = NULL) 
{
    op <- Operation(name = "Suggest", http_method = "GET", http_path = "/2013-01-01/suggest?format=sdk&pretty=true", 
        paginator = list())
    input <- suggest_input(query = query, suggester = suggester, 
        size = size)
...
#'
#' @export
upload_documents <- function (documents, contentType) 
{
    op <- Operation(name = "UploadDocuments", http_method = "POST", 
        http_path = "/2013-01-01/documents/batch?format=sdk", 
        paginator = list())
    input <- upload_documents_input(documents = documents, contentType = contentType)
    output <- upload_documents_output()
    svc <- service()
    request <- new_request(svc, op, input, output)
    response <- send_request(request)
    return(response)
