...
query_parse_structure <- function(values, value, prefix, tag, is_ec2 = FALSE) {
  for (key in names(value)) {
    ...
    name <- ""
    if (is_ec2) {
      name <- get_tag(field, "queryName")
    }
    if (name == "") {
      if (get_tag(field, "flattened") != "" && get_tag(field, "locationNameList") != "") {
        name <- get_tag(field, "locationNameList")
...
query_parse_list <- function(values, value, prefix, tag, is_ec2 = FALSE) {
  ...
  if (!is_ec2 && get_tag(value, "flattened") == "") {
    list_name <- get_tag(value, "locationNameList")
    if (list_name == "") {
...
query_parse_map <- function(values, value, prefix, tag, is_ec2 = FALSE) {
  ...
  if (map_entries) {
    ...
    for (i in seq_along(value)) {
      ...
      key_name = get_tag(value, "locationNameKey")
      if (key_name == "") key_name <- "key"
      value_name = get_tag(value, "locationNameValue")
