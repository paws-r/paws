...
#'
#' @param StartSelector Identifies the starting chunk to get from the specified stream.
...
#'
#' @export
get_media <- function (StartSelector, StreamName = NULL, StreamARN = NULL) 
{
    op <- Operation(name = "GetMedia", http_method = "POST", 
        http_path = "/getMedia", paginator = list())
    input <- get_media_input(StartSelector = StartSelector, StreamName = StreamName, 
        StreamARN = StreamARN)
    output <- get_media_output()
    svc <- service()
