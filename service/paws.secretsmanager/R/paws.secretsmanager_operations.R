...
#'
#' @param SecretId Specifies the secret for which you want to cancel a rotation request. You can specify either the Amazon Resource Name (ARN) or the friendly name of the secret.
#' 
...
#'
#' @export
cancel_rotate_secret <- function (SecretId) 
{
    op <- Operation(name = "CancelRotateSecret", http_method = "POST", 
        http_path = "/", paginator = list())
    input <- cancel_rotate_secret_input(SecretId = SecretId)
    output <- cancel_rotate_secret_output()
    svc <- service()
...
#'
#' @export
create_secret <- function (Name, ClientRequestToken = NULL, Description = NULL, 
    KmsKeyId = NULL, SecretBinary = NULL, SecretString = NULL, 
    Tags = NULL) 
{
    op <- Operation(name = "CreateSecret", http_method = "POST", 
        http_path = "/", paginator = list())
    input <- create_secret_input(Name = Name, ClientRequestToken = ClientRequestToken, 
        Description = Description, KmsKeyId = KmsKeyId, SecretBinary = SecretBinary, 
        SecretString = SecretString, Tags = Tags)
    output <- create_secret_output()
    svc <- service()
    request <- new_request(svc, op, input, output)
    response <- send_request(request)
    return(response)
...
#'
#' @export
delete_resource_policy <- function (SecretId) 
{
    op <- Operation(name = "DeleteResourcePolicy", http_method = "POST", 
        http_path = "/", paginator = list())
    input <- delete_resource_policy_input(SecretId = SecretId)
...
#'
#' @param SecretId Specifies the secret that you want to delete. You can specify either the Amazon Resource Name (ARN) or the friendly name of the secret.
#' 
#' If you specify an ARN, we generally recommend that you specify a complete ARN. You can specify a partial ARN too---for example, if you don't include the final hyphen and six random characters that Secrets Manager adds at the end of the ARN when you created the secret. A partial ARN match can work as long as it uniquely matches only one secret. However, if your secret has a name that ends in a hyphen followed by six characters (before Secrets Manager adds the hyphen and six characters to the ARN) and you try to use that as a partial ARN, then those characters cause Secrets Manager to assume that you're specifying a complete ARN. This confusion can cause unexpected results. To avoid this situation, we recommend that you don't create secret names that end with a hyphen followed by six characters.
...
#'
#' @export
delete_secret <- function (SecretId, RecoveryWindowInDays = NULL, 
    ForceDeleteWithoutRecovery = NULL) 
{
    op <- Operation(name = "DeleteSecret", http_method = "POST", 
        http_path = "/", paginator = list())
    input <- delete_secret_input(SecretId = SecretId, RecoveryWindowInDays = RecoveryWindowInDays, 
        ForceDeleteWithoutRecovery = ForceDeleteWithoutRecovery)
    output <- delete_secret_output()
    svc <- service()
    request <- new_request(svc, op, input, output)
    response <- send_request(request)
    return(response)
...
#'
#' @param SecretId The identifier of the secret whose details you want to retrieve. You can specify either the Amazon Resource Name (ARN) or the friendly name of the secret.
#' 
#' If you specify an ARN, we generally recommend that you specify a complete ARN. You can specify a partial ARN too---for example, if you don't include the final hyphen and six random characters that Secrets Manager adds at the end of the ARN when you created the secret. A partial ARN match can work as long as it uniquely matches only one secret. However, if your secret has a name that ends in a hyphen followed by six characters (before Secrets Manager adds the hyphen and six characters to the ARN) and you try to use that as a partial ARN, then those characters cause Secrets Manager to assume that you're specifying a complete ARN. This confusion can cause unexpected results. To avoid this situation, we recommend that you don't create secret names that end with a hyphen followed by six characters.
#'
#' @examples
...
#'
#' @export
describe_secret <- function (SecretId) 
{
    op <- Operation(name = "DescribeSecret", http_method = "POST", 
        http_path = "/", paginator = list())
    input <- describe_secret_input(SecretId = SecretId)
    output <- describe_secret_output()
    svc <- service()
    request <- new_request(svc, op, input, output)
    response <- send_request(request)
    return(response)
...
#'
#' @export
get_random_password <- function (PasswordLength = NULL, ExcludeCharacters = NULL, 
    ExcludeNumbers = NULL, ExcludePunctuation = NULL, ExcludeUppercase = NULL, 
    ExcludeLowercase = NULL, IncludeSpace = NULL, RequireEachIncludedType = NULL) 
{
    op <- Operation(name = "GetRandomPassword", http_method = "POST", 
        http_path = "/", paginator = list())
    input <- get_random_password_input(PasswordLength = PasswordLength, 
        ExcludeCharacters = ExcludeCharacters, ExcludeNumbers = ExcludeNumbers, 
        ExcludePunctuation = ExcludePunctuation, ExcludeUppercase = ExcludeUppercase, 
        ExcludeLowercase = ExcludeLowercase, IncludeSpace = IncludeSpace, 
        RequireEachIncludedType = RequireEachIncludedType)
    output <- get_random_password_output()
    svc <- service()
    request <- new_request(svc, op, input, output)
    response <- send_request(request)
    return(response)
...
#' -   To list all of the currently available secrets, use ListSecrets.
#'
#' @param SecretId Specifies the secret that you want to retrieve the attached resource-based policy for. You can specify either the Amazon Resource Name (ARN) or the friendly name of the secret.
#' 
#' If you specify an ARN, we generally recommend that you specify a complete ARN. You can specify a partial ARN too---for example, if you don't include the final hyphen and six random characters that Secrets Manager adds at the end of the ARN when you created the secret. A partial ARN match can work as long as it uniquely matches only one secret. However, if your secret has a name that ends in a hyphen followed by six characters (before Secrets Manager adds the hyphen and six characters to the ARN) and you try to use that as a partial ARN, then those characters cause Secrets Manager to assume that you're specifying a complete ARN. This confusion can cause unexpected results. To avoid this situation, we recommend that you don't create secret names that end with a hyphen followed by six characters.
#'
#' @examples
...
#'
#' @export
get_resource_policy <- function (SecretId) 
{
    op <- Operation(name = "GetResourcePolicy", http_method = "POST", 
        http_path = "/", paginator = list())
    input <- get_resource_policy_input(SecretId = SecretId)
...
#'
#' @export
get_secret_value <- function (SecretId, VersionId = NULL, VersionStage = NULL) 
{
    op <- Operation(name = "GetSecretValue", http_method = "POST", 
        http_path = "/", paginator = list())
    input <- get_secret_value_input(SecretId = SecretId, VersionId = VersionId, 
        VersionStage = VersionStage)
    output <- get_secret_value_output()
    svc <- service()
    request <- new_request(svc, op, input, output)
    response <- send_request(request)
    return(response)
...
#'
#' @export
list_secret_version_ids <- function (SecretId, MaxResults = NULL, 
    NextToken = NULL, IncludeDeprecated = NULL) 
{
    op <- Operation(name = "ListSecretVersionIds", http_method = "POST", 
        http_path = "/", paginator = list())
    input <- list_secret_version_ids_input(SecretId = SecretId, 
        MaxResults = MaxResults, NextToken = NextToken, IncludeDeprecated = IncludeDeprecated)
    output <- list_secret_version_ids_output()
    svc <- service()
    request <- new_request(svc, op, input, output)
    response <- send_request(request)
    return(response)
...
#'
#' @export
list_secrets <- function (MaxResults = NULL, NextToken = NULL) 
{
    op <- Operation(name = "ListSecrets", http_method = "POST", 
        http_path = "/", paginator = list())
    input <- list_secrets_input(MaxResults = MaxResults, NextToken = NextToken)
    output <- list_secrets_output()
    svc <- service()
    request <- new_request(svc, op, input, output)
    response <- send_request(request)
    return(response)
...
#' -   To list all of the currently available secrets, use ListSecrets.
#'
#' @param SecretId Specifies the secret that you want to attach the resource-based policy to. You can specify either the ARN or the friendly name of the secret.
#' 
#' If you specify an ARN, we generally recommend that you specify a complete ARN. You can specify a partial ARN too---for example, if you don't include the final hyphen and six random characters that Secrets Manager adds at the end of the ARN when you created the secret. A partial ARN match can work as long as it uniquely matches only one secret. However, if your secret has a name that ends in a hyphen followed by six characters (before Secrets Manager adds the hyphen and six characters to the ARN) and you try to use that as a partial ARN, then those characters cause Secrets Manager to assume that you're specifying a complete ARN. This confusion can cause unexpected results. To avoid this situation, we recommend that you don't create secret names that end with a hyphen followed by six characters.
#' @param ResourcePolicy A JSON-formatted string that\'s constructed according to the grammar and syntax for an AWS resource-based policy. The policy in the string identifies who can access or manage this secret and its versions. For information on how to format a JSON parameter for the various command line tool environments, see [Using JSON for Parameters](http://docs.aws.amazon.com/cli/latest/userguide/cli-using-param.html#cli-using-param-json) in the *AWS CLI User Guide*.
#'
#' @examples
...
#'
#' @export
put_resource_policy <- function (SecretId, ResourcePolicy) 
{
    op <- Operation(name = "PutResourcePolicy", http_method = "POST", 
        http_path = "/", paginator = list())
    input <- put_resource_policy_input(SecretId = SecretId, ResourcePolicy = ResourcePolicy)
...
#'
#' @export
put_secret_value <- function (SecretId, ClientRequestToken = NULL, 
    SecretBinary = NULL, SecretString = NULL, VersionStages = NULL) 
{
    op <- Operation(name = "PutSecretValue", http_method = "POST", 
        http_path = "/", paginator = list())
    input <- put_secret_value_input(SecretId = SecretId, ClientRequestToken = ClientRequestToken, 
        SecretBinary = SecretBinary, SecretString = SecretString, 
        VersionStages = VersionStages)
    output <- put_secret_value_output()
    svc <- service()
    request <- new_request(svc, op, input, output)
    response <- send_request(request)
    return(response)
...
#' -   To delete a secret, use DeleteSecret.
#'
#' @param SecretId Specifies the secret that you want to restore from a previously scheduled deletion. You can specify either the Amazon Resource Name (ARN) or the friendly name of the secret.
#' 
#' If you specify an ARN, we generally recommend that you specify a complete ARN. You can specify a partial ARN too---for example, if you don't include the final hyphen and six random characters that Secrets Manager adds at the end of the ARN when you created the secret. A partial ARN match can work as long as it uniquely matches only one secret. However, if your secret has a name that ends in a hyphen followed by six characters (before Secrets Manager adds the hyphen and six characters to the ARN) and you try to use that as a partial ARN, then those characters cause Secrets Manager to assume that you're specifying a complete ARN. This confusion can cause unexpected results. To avoid this situation, we recommend that you don't create secret names that end with a hyphen followed by six characters.
#'
#' @examples
...
#'
#' @export
restore_secret <- function (SecretId) 
{
    op <- Operation(name = "RestoreSecret", http_method = "POST", 
        http_path = "/", paginator = list())
    input <- restore_secret_input(SecretId = SecretId)
    output <- restore_secret_output()
    svc <- service()
    request <- new_request(svc, op, input, output)
    response <- send_request(request)
    return(response)
...
#'
#' @export
rotate_secret <- function (SecretId, ClientRequestToken = NULL, 
    RotationLambdaARN = NULL, RotationRules = NULL) 
{
    op <- Operation(name = "RotateSecret", http_method = "POST", 
        http_path = "/", paginator = list())
    input <- rotate_secret_input(SecretId = SecretId, ClientRequestToken = ClientRequestToken, 
        RotationLambdaARN = RotationLambdaARN, RotationRules = RotationRules)
    output <- rotate_secret_output()
    svc <- service()
    request <- new_request(svc, op, input, output)
    response <- send_request(request)
    return(response)
...
#'
#' @param SecretId The identifier for the secret that you want to attach tags to. You can specify either the Amazon Resource Name (ARN) or the friendly name of the secret.
#' 
#' If you specify an ARN, we generally recommend that you specify a complete ARN. You can specify a partial ARN too---for example, if you don't include the final hyphen and six random characters that Secrets Manager adds at the end of the ARN when you created the secret. A partial ARN match can work as long as it uniquely matches only one secret. However, if your secret has a name that ends in a hyphen followed by six characters (before Secrets Manager adds the hyphen and six characters to the ARN) and you try to use that as a partial ARN, then those characters cause Secrets Manager to assume that you're specifying a complete ARN. This confusion can cause unexpected results. To avoid this situation, we recommend that you don't create secret names that end with a hyphen followed by six characters.
#' @param Tags The tags to attach to the secret. Each element in the list consists of a `Key` and a `Value`.
#' 
...
#'
#' @export
tag_resource <- function (SecretId, Tags) 
{
    op <- Operation(name = "TagResource", http_method = "POST", 
        http_path = "/", paginator = list())
    input <- tag_resource_input(SecretId = SecretId, Tags = Tags)
...
#' -   To view the list of tags attached to a secret, use DescribeSecret.
#'
#' @param SecretId The identifier for the secret that you want to remove tags from. You can specify either the Amazon Resource Name (ARN) or the friendly name of the secret.
#' 
#' If you specify an ARN, we generally recommend that you specify a complete ARN. You can specify a partial ARN too---for example, if you don't include the final hyphen and six random characters that Secrets Manager adds at the end of the ARN when you created the secret. A partial ARN match can work as long as it uniquely matches only one secret. However, if your secret has a name that ends in a hyphen followed by six characters (before Secrets Manager adds the hyphen and six characters to the ARN) and you try to use that as a partial ARN, then those characters cause Secrets Manager to assume that you're specifying a complete ARN. This confusion can cause unexpected results. To avoid this situation, we recommend that you don't create secret names that end with a hyphen followed by six characters.
#' @param TagKeys A list of tag key names to remove from the secret. You don\'t specify the value. Both the key and its associated value are removed.
#' 
...
#'
#' @export
untag_resource <- function (SecretId, TagKeys) 
{
    op <- Operation(name = "UntagResource", http_method = "POST", 
        http_path = "/", paginator = list())
    input <- untag_resource_input(SecretId = SecretId, TagKeys = TagKeys)
...
#'
#' @param SecretId Specifies the secret that you want to modify or to which you want to add a new version. You can specify either the Amazon Resource Name (ARN) or the friendly name of the secret.
#' 
#' If you specify an ARN, we generally recommend that you specify a complete ARN. You can specify a partial ARN too---for example, if you don't include the final hyphen and six random characters that Secrets Manager adds at the end of the ARN when you created the secret. A partial ARN match can work as long as it uniquely matches only one secret. However, if your secret has a name that ends in a hyphen followed by six characters (before Secrets Manager adds the hyphen and six characters to the ARN) and you try to use that as a partial ARN, then those characters cause Secrets Manager to assume that you're specifying a complete ARN. This confusion can cause unexpected results. To avoid this situation, we recommend that you don't create secret names that end with a hyphen followed by six characters.
...
#'
#' @export
update_secret <- function (SecretId, ClientRequestToken = NULL, 
    Description = NULL, KmsKeyId = NULL, SecretBinary = NULL, 
    SecretString = NULL) 
{
    op <- Operation(name = "UpdateSecret", http_method = "POST", 
        http_path = "/", paginator = list())
    input <- update_secret_input(SecretId = SecretId, ClientRequestToken = ClientRequestToken, 
        Description = Description, KmsKeyId = KmsKeyId, SecretBinary = SecretBinary, 
        SecretString = SecretString)
    output <- update_secret_output()
    svc <- service()
    request <- new_request(svc, op, input, output)
    response <- send_request(request)
    return(response)
...
#' **Related operations**
#' 
#' -   To get the list of staging labels that are currently associated with a version of a secret, use ` DescribeSecret DescribeSecret ` and examine the `SecretVersionsToStages` response value.
#'
#' @param SecretId Specifies the secret with the version whose list of staging labels you want to modify. You can specify either the Amazon Resource Name (ARN) or the friendly name of the secret.
#' 
#' If you specify an ARN, we generally recommend that you specify a complete ARN. You can specify a partial ARN too---for example, if you don't include the final hyphen and six random characters that Secrets Manager adds at the end of the ARN when you created the secret. A partial ARN match can work as long as it uniquely matches only one secret. However, if your secret has a name that ends in a hyphen followed by six characters (before Secrets Manager adds the hyphen and six characters to the ARN) and you try to use that as a partial ARN, then those characters cause Secrets Manager to assume that you're specifying a complete ARN. This confusion can cause unexpected results. To avoid this situation, we recommend that you don't create secret names that end with a hyphen followed by six characters.
#' @param VersionStage The list of staging labels to add to this version.
#' @param RemoveFromVersionId Specifies the secret version ID of the version that the staging labels are to be removed from. If the staging label you are trying to attach to one version is already attached to a different version, then you must include this parameter and specify the version that the label is to be removed from. If the label is attached and you either do not specify this parameter, or the version ID does not match, then the operation fails.
#' @param MoveToVersionId (Optional) The secret version ID that you want to add the staging labels to. If you want to remove a label from a version, then do not specify this parameter.
#' 
...
#'
#' @export
update_secret_version_stage <- function (SecretId, VersionStage, 
    RemoveFromVersionId = NULL, MoveToVersionId = NULL) 
{
    op <- Operation(name = "UpdateSecretVersionStage", http_method = "POST", 
        http_path = "/", paginator = list())
    input <- update_secret_version_stage_input(SecretId = SecretId, 
        VersionStage = VersionStage, RemoveFromVersionId = RemoveFromVersionId, 
        MoveToVersionId = MoveToVersionId)
    output <- update_secret_version_stage_output()
    svc <- service()
    request <- new_request(svc, op, input, output)
    response <- send_request(request)
    return(response)
