...
#'
#' @export
get_hls_streaming_session_url <- function (StreamName = NULL, 
    StreamARN = NULL, PlaybackMode = NULL, HLSFragmentSelector = NULL, 
    DiscontinuityMode = NULL, Expires = NULL, MaxMediaPlaylistFragmentResults = NULL) 
{
    op <- Operation(name = "GetHLSStreamingSessionURL", http_method = "POST", 
        http_path = "/getHLSStreamingSessionURL", paginator = list())
    input <- get_hls_streaming_session_url_input(StreamName = StreamName, 
        StreamARN = StreamARN, PlaybackMode = PlaybackMode, HLSFragmentSelector = HLSFragmentSelector, 
        DiscontinuityMode = DiscontinuityMode, Expires = Expires, 
        MaxMediaPlaylistFragmentResults = MaxMediaPlaylistFragmentResults)
    output <- get_hls_streaming_session_url_output()
    svc <- service()
...
#'
#' @param StreamName The name of the stream from which to retrieve fragment media.
#' @param Fragments A list of the numbers of fragments for which to retrieve media. You retrieve these values with ListFragments.
#'
#' @examples
#'
#' @export
get_media_for_fragment_list <- function (StreamName, Fragments) 
{
    op <- Operation(name = "GetMediaForFragmentList", http_method = "POST", 
        http_path = "/getMediaForFragmentList", paginator = list())
    input <- get_media_for_fragment_list_input(StreamName = StreamName, 
        Fragments = Fragments)
    output <- get_media_for_fragment_list_output()
    svc <- service()
    request <- new_request(svc, op, input, output)
    response <- send_request(request)
    return(response)
...
#' Returns a list of Fragment objects from the specified stream and start location within the archived data.
#'
#' @param StreamName The name of the stream from which to retrieve a fragment list.
...
#'
#' @export
list_fragments <- function (StreamName, MaxResults = NULL, NextToken = NULL, 
    FragmentSelector = NULL) 
{
    op <- Operation(name = "ListFragments", http_method = "POST", 
        http_path = "/listFragments", paginator = list())
    input <- list_fragments_input(StreamName = StreamName, MaxResults = MaxResults, 
        NextToken = NextToken, FragmentSelector = FragmentSelector)
    output <- list_fragments_output()
    svc <- service()
    request <- new_request(svc, op, input, output)
    response <- send_request(request)
    return(response)
