...
#' @param attachments &#91;required&#93; One or more attachments to add to the set. The limit is 3 attachments per set, and the size limit is 5 MB per attachment.
#'
#' @export
add_attachments_to_set <- function (attachmentSetId = NULL, attachments) 
{
    op <- new_operation(name = "AddAttachmentsToSet", http_method = "POST", 
        http_path = "/", paginator = list())
    input <- add_attachments_to_set_input(attachmentSetId = attachmentSetId, 
...
#'
#' @export
add_communication_to_case <- function (caseId = NULL, communicationBody, 
    ccEmailAddresses = NULL, attachmentSetId = NULL) 
{
    op <- new_operation(name = "AddCommunicationToCase", http_method = "POST", 
        http_path = "/", paginator = list())
    input <- add_communication_to_case_input(caseId = caseId, 
        communicationBody = communicationBody, ccEmailAddresses = ccEmailAddresses, 
...
#' @param communicationBody &#91;required&#93; The communication body text when you create an AWS Support case by calling CreateCase.
...
#'
#' @export
create_case <- function (subject, serviceCode = NULL, severityCode = NULL, 
    categoryCode = NULL, communicationBody, ccEmailAddresses = NULL, 
    language = NULL, issueType = NULL, attachmentSetId = NULL) 
{
    op <- new_operation(name = "CreateCase", http_method = "POST", 
        http_path = "/", paginator = list())
    input <- create_case_input(subject = subject, serviceCode = serviceCode, 
        severityCode = severityCode, categoryCode = categoryCode, 
        communicationBody = communicationBody, ccEmailAddresses = ccEmailAddresses, 
...
#' ```
#'
#' @param attachmentId &#91;required&#93; The ID of the attachment to return. Attachment IDs are returned by the DescribeCommunications operation.
#'
#' @export
describe_attachment <- function (attachmentId) 
{
    op <- new_operation(name = "DescribeAttachment", http_method = "POST", 
        http_path = "/", paginator = list())
...
{
    op <- new_operation(name = "DescribeCases", http_method = "POST", 
        http_path = "/", paginator = list())
...
#' ```
#'
#' @param caseId &#91;required&#93; The AWS Support case ID requested or returned in the call. The case ID is an alphanumeric string formatted as shown in this example: case-*12345678910-2013-c4c1d2bf33c5cf47*
#' @param beforeTime The end date for a filtered date search on support case communications. Case communications are available for 12 months after creation.
#' @param afterTime The start date for a filtered date search on support case communications. Case communications are available for 12 months after creation.
#' @param nextToken A resumption point for pagination.
#' @param maxResults The maximum number of results to return before paginating.
...
{
    op <- new_operation(name = "DescribeCommunications", http_method = "POST", 
        http_path = "/", paginator = list())
...
{
    op <- new_operation(name = "DescribeSeverityLevels", http_method = "POST", 
        http_path = "/", paginator = list())
...
#' ```
#'
#' @param checkIds &#91;required&#93; The IDs of the Trusted Advisor checks to get the status of. **Note:** Specifying the check ID of a check that is automatically refreshed causes an `InvalidParameterValue` error.
#'
#' @export
describe_trusted_advisor_check_refresh_statuses <- function (checkIds) 
{
    op <- new_operation(name = "DescribeTrustedAdvisorCheckRefreshStatuses", 
...
{
    op <- new_operation(name = "DescribeTrustedAdvisorCheckResult", 
        http_method = "POST", http_path = "/", paginator = list())
...
{
    op <- new_operation(name = "DescribeTrustedAdvisorCheckSummaries", 
        http_method = "POST", http_path = "/", paginator = list())
...
#' ```
#'
#' @param language &#91;required&#93; The ISO 639-1 code for the language in which AWS provides support. AWS Support currently supports English (\"en\") and Japanese (\"ja\"). Language parameters must be passed explicitly for operations that take them.
#'
#' @export
describe_trusted_advisor_checks <- function (language) 
{
    op <- new_operation(name = "DescribeTrustedAdvisorChecks", 
        http_method = "POST", http_path = "/", paginator = list())
...
#' ```
#'
#' @param checkId &#91;required&#93; The unique identifier for the Trusted Advisor check to refresh. **Note:** Specifying the check ID of a check that is automatically refreshed causes an `InvalidParameterValue` error.
#'
#' @export
refresh_trusted_advisor_check <- function (checkId) 
{
    op <- new_operation(name = "RefreshTrustedAdvisorCheck", 
        http_method = "POST", http_path = "/", paginator = list())
...
{
    op <- new_operation(name = "ResolveCase", http_method = "POST", 
        http_path = "/", paginator = list())
