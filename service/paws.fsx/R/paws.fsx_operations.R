...
#'
#' @export
create_backup <- function (FileSystemId, ClientRequestToken = NULL, 
    Tags = NULL) 
{
    op <- new_operation(name = "CreateBackup", http_method = "POST", 
        http_path = "/", paginator = list())
    input <- create_backup_input(FileSystemId = FileSystemId, 
...
#' @param SubnetIds &#91;required&#93; A list of IDs for the subnets that the file system will be accessible from. File systems support only one subnet. The file server is also launched in that subnet\'s Availability Zone.
...
#'
#' @export
create_file_system <- function (ClientRequestToken = NULL, FileSystemType, 
    StorageCapacity, SubnetIds, SecurityGroupIds = NULL, Tags = NULL, 
    KmsKeyId = NULL, WindowsConfiguration = NULL, LustreConfiguration = NULL) 
{
    op <- new_operation(name = "CreateFileSystem", http_method = "POST", 
        http_path = "/", paginator = list())
    input <- create_file_system_input(ClientRequestToken = ClientRequestToken, 
        FileSystemType = FileSystemType, StorageCapacity = StorageCapacity, 
        SubnetIds = SubnetIds, SecurityGroupIds = SecurityGroupIds, 
        Tags = Tags, KmsKeyId = KmsKeyId, WindowsConfiguration = WindowsConfiguration, 
...
#' @param ClientRequestToken (Optional) A string of up to 64 ASCII characters that Amazon FSx uses to ensure idempotent creation. This string is automatically filled on your behalf when you use the AWS Command Line Interface (AWS CLI) or an AWS SDK.
#' @param SubnetIds &#91;required&#93; A list of IDs for the subnets that the file system will be accessible from. Currently, you can specify only one subnet. The file server is also launched in that subnet\'s Availability Zone.
...
#' \donttest{create_file_system_from_backup(
...
#'
#' @export
create_file_system_from_backup <- function (BackupId, ClientRequestToken = NULL, 
    SubnetIds, SecurityGroupIds = NULL, Tags = NULL, WindowsConfiguration = NULL) 
{
    op <- new_operation(name = "CreateFileSystemFromBackup", 
        http_method = "POST", http_path = "/", paginator = list())
    input <- create_file_system_from_backup_input(BackupId = BackupId, 
        ClientRequestToken = ClientRequestToken, SubnetIds = SubnetIds, 
        SecurityGroupIds = SecurityGroupIds, Tags = Tags, WindowsConfiguration = WindowsConfiguration)
    output <- create_file_system_from_backup_output()
    svc <- service()
    request <- new_request(svc, op, input, output)
    response <- send_request(request)
    return(response)
...
#' \donttest{delete_backup(
#'   BackupId = "backup-03e3c82e0183b7b6b"
#' )}
#'
#' @export
delete_backup <- function (BackupId, ClientRequestToken = NULL) 
{
    op <- new_operation(name = "DeleteBackup", http_method = "POST", 
        http_path = "/", paginator = list())
    input <- delete_backup_input(BackupId = BackupId, ClientRequestToken = ClientRequestToken)
...
#'
#' @export
delete_file_system <- function (FileSystemId, ClientRequestToken = NULL, 
    WindowsConfiguration = NULL) 
{
    op <- new_operation(name = "DeleteFileSystem", http_method = "POST", 
        http_path = "/", paginator = list())
...
#' \donttest{describe_backups()}
#'
#' @export
describe_backups <- function (BackupIds = NULL, Filters = NULL, 
    MaxResults = NULL, NextToken = NULL) 
{
    op <- new_operation(name = "DescribeBackups", http_method = "POST", 
        http_path = "/", paginator = list())
    input <- describe_backups_input(BackupIds = BackupIds, Filters = Filters, 
...
#' # account.
#' \donttest{describe_file_systems()}
#'
#' @export
describe_file_systems <- function (FileSystemIds = NULL, MaxResults = NULL, 
    NextToken = NULL) 
{
    op <- new_operation(name = "DescribeFileSystems", http_method = "POST", 
        http_path = "/", paginator = list())
    input <- describe_file_systems_input(FileSystemIds = FileSystemIds, 
        MaxResults = MaxResults, NextToken = NextToken)
...
#'
#' @export
update_file_system <- function (FileSystemId, ClientRequestToken = NULL, 
    WindowsConfiguration = NULL, LustreConfiguration = NULL) 
{
    op <- new_operation(name = "UpdateFileSystem", http_method = "POST", 
        http_path = "/", paginator = list())
