# This file is generated by make.paws. Please do not edit here.

test_that("describe_ec2_instance_limits", {
    expect_error(describe_ec2_instance_limits(), NA)
})

test_that("describe_fleet_attributes", {
    expect_error(describe_fleet_attributes(), NA)
})

test_that("describe_fleet_capacity", {
    expect_error(describe_fleet_capacity(), NA)
})

test_that("describe_fleet_utilization", {
    expect_error(describe_fleet_utilization(), NA)
})

test_that("describe_game_session_details", {
    expect_error(describe_game_session_details(), NA)
})

test_that("describe_game_session_queues", {
    expect_error(describe_game_session_queues(), NA)
})

test_that("describe_game_sessions", {
    expect_error(describe_game_sessions(), NA)
})

test_that("describe_matchmaking_configurations", {
    expect_error(describe_matchmaking_configurations(), NA)
})

test_that("describe_matchmaking_rule_sets", {
    expect_error(describe_matchmaking_rule_sets(), NA)
})

test_that("describe_player_sessions", {
    expect_error(describe_player_sessions(), NA)
})

test_that("describe_vpc_peering_authorizations", {
    expect_error(describe_vpc_peering_authorizations(), NA)
})

test_that("describe_vpc_peering_connections", {
    expect_error(describe_vpc_peering_connections(), NA)
})

test_that("list_aliases", {
    expect_error(list_aliases(), NA)
})

test_that("list_builds", {
    expect_error(list_builds(), NA)
})

test_that("list_fleets", {
    expect_error(list_fleets(), NA)
})
