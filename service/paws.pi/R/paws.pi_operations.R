...
#' ```
#'
#' @param ServiceType &#91;required&#93; The AWS service for which Performance Insights will return metrics. The only valid value for *ServiceType* is: `RDS`
#' @param Identifier &#91;required&#93; An immutable, AWS Region-unique identifier for a data source. Performance Insights gathers metrics from this data source.
#' 
...
#' @param StartTime &#91;required&#93; The date and time specifying the beginning of the requested time series data. You can\'t specify a `StartTime` that\'s earlier than 7 days ago. The value specified is *inclusive* - data points equal to or greater than `StartTime` will be returned.
#' 
#' The value for `StartTime` must be earlier than the value for `EndTime`.
#' @param EndTime &#91;required&#93; The date and time specifying the end of the requested time series data. The value specified is *exclusive* - data points less than (but not equal to) `EndTime` will be returned.
#' 
...
#' 
#' To use an Amazon RDS instance as a data source, you specify its `DbiResourceId` value - for example: `db-FAIHNTYBKTGAUSUZQYPDS2GW4A`
#' @param MetricQueries &#91;required&#93; An array of one or more queries to perform. Each query must specify a Performance Insights metric, and can optionally specify aggregation and filtering criteria.
#' @param StartTime &#91;required&#93; The date and time specifying the beginning of the requested time series data. You can\'t specify a `StartTime` that\'s earlier than 7 days ago. The value specified is *inclusive* - data points equal to or greater than `StartTime` will be returned.
#' 
#' The value for `StartTime` must be earlier than the value for `EndTime`.
#' @param EndTime &#91;required&#93; The date and time specifiying the end of the requested time series data. The value specified is *exclusive* - data points less than (but not equal to) `EndTime` will be returned.
#' 
#' The value for `EndTime` must be later than the value for `StartTime`.
#' @param PeriodInSeconds The granularity, in seconds, of the data points returned from Performance Insights. A period can be as short as one second, or as long as one day (86400 seconds). Valid values are:
#' 
...
get_resource_metrics <- function (ServiceType, Identifier, MetricQueries, 
    ...
    NextToken = NULL) 
{
    op <- new_operation(name = "GetResourceMetrics", http_method = "POST", 
        http_path = "/", paginator = list())
