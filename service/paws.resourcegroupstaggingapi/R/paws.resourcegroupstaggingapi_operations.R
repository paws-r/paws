...
#'
#' @export
get_resources <- function (PaginationToken = NULL, TagFilters = NULL, 
    ResourcesPerPage = NULL, TagsPerPage = NULL, ResourceTypeFilters = NULL) 
{
    op <- Operation(name = "GetResources", http_method = "POST", 
        http_path = "/", paginator = list())
    input <- get_resources_input(PaginationToken = PaginationToken, 
        TagFilters = TagFilters, ResourcesPerPage = ResourcesPerPage, 
        TagsPerPage = TagsPerPage, ResourceTypeFilters = ResourceTypeFilters)
...
#'
#' @export
get_tag_keys <- function (PaginationToken = NULL) 
{
    op <- Operation(name = "GetTagKeys", http_method = "POST", 
        http_path = "/", paginator = list())
    input <- get_tag_keys_input(PaginationToken = PaginationToken)
    output <- get_tag_keys_output()
    svc <- service()
    request <- new_request(svc, op, input, output)
    response <- send_request(request)
    return(response)
...
#' Returns all tag values for the specified key in the specified region for the AWS account.
#'
#' @param Key The key for which you want to list all existing values in the specified region for the AWS account.
#' @param PaginationToken A string that indicates that additional data is available. Leave this value empty for your initial request. If the response includes a PaginationToken, use that string for this value to request an additional page of data.
#'
#' @examples
#'
#' @export
get_tag_values <- function (Key, PaginationToken = NULL) 
{
    op <- Operation(name = "GetTagValues", http_method = "POST", 
        http_path = "/", paginator = list())
    input <- get_tag_values_input(Key = Key, PaginationToken = PaginationToken)
    output <- get_tag_values_output()
    svc <- service()
    request <- new_request(svc, op, input, output)
    response <- send_request(request)
    return(response)
...
#'
#' @param ResourceARNList A list of ARNs. An ARN (Amazon Resource Name) uniquely identifies a resource. You can specify a minimum of 1 and a maximum of 20 ARNs (resources) to tag. An ARN can be set to a maximum of 1600 characters. For more information, see [Amazon Resource Names (ARNs) and AWS Service Namespaces](http://docs.aws.amazon.com/general/latest/gr/aws-arns-and-namespaces.html) in the *AWS General Reference*.
#' @param Tags The tags that you want to add to the specified resources. A tag consists of a key and a value that you define.
#'
#' @examples
#'
#' @export
tag_resources <- function (ResourceARNList, Tags) 
{
    op <- Operation(name = "TagResources", http_method = "POST", 
        http_path = "/", paginator = list())
    input <- tag_resources_input(ResourceARNList = ResourceARNList, 
        Tags = Tags)
    output <- tag_resources_output()
    svc <- service()
    request <- new_request(svc, op, input, output)
    response <- send_request(request)
    return(response)
...
#' -   You can only tag resources that are located in the specified region for the AWS account.
#'
#' @param ResourceARNList A list of ARNs. An ARN (Amazon Resource Name) uniquely identifies a resource. You can specify a minimum of 1 and a maximum of 20 ARNs (resources) to untag. An ARN can be set to a maximum of 1600 characters. For more information, see [Amazon Resource Names (ARNs) and AWS Service Namespaces](http://docs.aws.amazon.com/general/latest/gr/aws-arns-and-namespaces.html) in the *AWS General Reference*.
#' @param TagKeys A list of the tag keys that you want to remove from the specified resources.
#'
#' @examples
#'
#' @export
untag_resources <- function (ResourceARNList, TagKeys) 
{
    op <- Operation(name = "UntagResources", http_method = "POST", 
        http_path = "/", paginator = list())
    input <- untag_resources_input(ResourceARNList = ResourceARNList, 
        TagKeys = TagKeys)
    output <- untag_resources_output()
    svc <- service()
    request <- new_request(svc, op, input, output)
    response <- send_request(request)
    return(response)
