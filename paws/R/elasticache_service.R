...
#'  \link[=elasticache_create_global_replication_group]{create_global_replication_group} \tab Global Datastore for Redis OSS offers fully managed, fast, reliable and secure cross-region replication\cr
#'  \link[=elasticache_create_replication_group]{create_replication_group} \tab Creates a Redis OSS (cluster mode disabled) or a Redis OSS (cluster mode enabled) replication group\cr
...
#'  \link[=elasticache_create_user]{create_user} \tab For Redis OSS engine version 6\cr
#'  \link[=elasticache_create_user_group]{create_user_group} \tab For Redis OSS engine version 6\cr
...
#'  \link[=elasticache_decrease_replica_count]{decrease_replica_count} \tab Dynamically decreases the number of replicas in a Redis OSS (cluster mode disabled) replication group or the number of replica nodes in one or more node groups (shards) of a Redis OSS (cluster mode enabled) replication group\cr
...
#'  \link[=elasticache_delete_user]{delete_user} \tab For Redis OSS engine version 6\cr
#'  \link[=elasticache_delete_user_group]{delete_user_group} \tab For Redis OSS engine version 6\cr
...
#'  \link[=elasticache_increase_replica_count]{increase_replica_count} \tab Dynamically increases the number of replicas in a Redis OSS (cluster mode disabled) replication group or the number of replica nodes in one or more node groups (shards) of a Redis OSS (cluster mode enabled) replication group\cr
#'  \link[=elasticache_list_allowed_node_type_modifications]{list_allowed_node_type_modifications} \tab Lists all available node types that you can scale your Redis OSS cluster's or replication group's current node type\cr
