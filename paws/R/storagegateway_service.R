...
#'  \link[=storagegateway_cancel_cache_report]{cancel_cache_report} \tab Cancels generation of a specified cache report\cr
...
#'  \link[=storagegateway_delete_cache_report]{delete_cache_report} \tab Deletes the specified cache report and any associated tags from the Storage Gateway database\cr
...
#'  \link[=storagegateway_describe_cache_report]{describe_cache_report} \tab Returns information about the specified cache report, including completion status and generation progress\cr
...
#'  \link[=storagegateway_describe_maintenance_start_time]{describe_maintenance_start_time} \tab Returns your gateway's maintenance window schedule information, with values for monthly or weekly cadence, specific day and time to begin maintenance, and which types of updates to apply\cr
...
#'  \link[=storagegateway_notify_when_uploaded]{notify_when_uploaded} \tab Sends you notification through Amazon EventBridge when all files written to your file share have been uploaded to Amazon S3\cr
...
#'  \link[=storagegateway_start_cache_report]{start_cache_report} \tab Starts generating a report of the file metadata currently cached by an S3 File Gateway for a specific file share\cr
...
#'  \link[=storagegateway_update_gateway_information]{update_gateway_information} \tab Updates a gateway's metadata, which includes the gateway's name, time zone, and metadata cache size\cr
...
#'  \link[=storagegateway_update_maintenance_start_time]{update_maintenance_start_time} \tab Updates a gateway's maintenance window schedule, with settings for monthly or weekly cadence, specific day and time to begin maintenance, and which types of updates to apply\cr
...
#'  \link[=storagegateway_update_smb_security_strategy]{update_smb_security_strategy} \tab Updates the SMB security strategy level for an Amazon S3 file gateway\cr
...
.storagegateway$service <- function(config = list(), op = NULL) {
  handlers <- new_handlers("jsonrpc", "v4")
  new_service(.storagegateway$metadata, handlers, config, op)
}
