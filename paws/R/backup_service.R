...
#' @section Operations:
#' \tabular{ll}{
#'  \link[=backup_cancel_legal_hold]{cancel_legal_hold} \tab This action removes the specified legal hold on a recovery point\cr
...
#'  \link[=backup_create_legal_hold]{create_legal_hold} \tab This action creates a legal hold on a recovery point (backup)\cr
#'  \link[=backup_create_logically_air_gapped_backup_vault]{create_logically_air_gapped_backup_vault} \tab This request creates a logical container to where backups may be copied\cr
#'  \link[=backup_create_report_plan]{create_report_plan} \tab Creates a report plan\cr
#'  \link[=backup_create_restore_testing_plan]{create_restore_testing_plan} \tab This is the first of two steps to create a restore testing plan; once this request is successful, finish the procedure with request CreateRestoreTestingSelection\cr
...
#'  \link[=backup_list_backup_plans]{list_backup_plans} \tab Returns a list of all active backup plans for an authenticated account\cr
...
#'  \link[=backup_list_recovery_points_by_resource]{list_recovery_points_by_resource} \tab Returns detailed information about all the recovery points of the type specified by a resource Amazon Resource Name (ARN)\cr
...
#'  \link[=backup_update_framework]{update_framework} \tab Updates an existing framework identified by its FrameworkName with the input document in JSON format\cr
...
#'  \link[=backup_update_report_plan]{update_report_plan} \tab Updates an existing report plan identified by its ReportPlanName with the input document in JSON format\cr
...
#'  \link[=backup_update_restore_testing_selection]{update_restore_testing_selection} \tab Most elements except the RestoreTestingSelectionName can be updated with this request
#' }
#'
#' @return
...
.backup$metadata <- list(
  service_name = "backup",
  endpoints = list("*" = list(endpoint = "backup.{region}.amazonaws.com", global = FALSE), "cn-*" = list(endpoint = "backup.{region}.amazonaws.com.cn", global = FALSE), "eu-isoe-*" = list(endpoint = "backup.{region}.cloud.adc-e.uk", global = FALSE), "us-iso-*" = list(endpoint = "backup.{region}.c2s.ic.gov", global = FALSE), "us-isob-*" = list(endpoint = "backup.{region}.sc2s.sgov.gov", global = FALSE), "us-isof-*" = list(endpoint = "backup.{region}.csp.hci.ic.gov", global = FALSE)),
  service_id = "Backup",
