...
#'  \link[=ssmsap_get_configuration_check_operation]{get_configuration_check_operation} \tab Gets the details of a configuration check operation by specifying the operation ID\cr
...
#'  \link[=ssmsap_list_configuration_check_definitions]{list_configuration_check_definitions} \tab Lists all configuration check types supported by AWS Systems Manager for SAP\cr
#'  \link[=ssmsap_list_configuration_check_operations]{list_configuration_check_operations} \tab Lists the configuration check operations performed by AWS Systems Manager for SAP\cr
...
#'  \link[=ssmsap_list_sub_check_results]{list_sub_check_results} \tab Lists the sub-check results of a specified configuration check operation\cr
#'  \link[=ssmsap_list_sub_check_rule_results]{list_sub_check_rule_results} \tab Lists the rules of a specified sub-check belonging to a configuration check operation\cr
...
#'  \link[=ssmsap_start_configuration_checks]{start_configuration_checks} \tab Initiates configuration check operations against a specified application\cr
...
.ssmsap$metadata <- list(
  service_name = "ssmsap",
  endpoints = list("^(us|eu|ap|sa|ca|me|af|il|mx)\\-\\w+\\-\\d+$" = list(endpoint = "ssm-sap.{region}.amazonaws.com", global = FALSE), "^cn\\-\\w+\\-\\d+$" = list(endpoint = "ssm-sap.{region}.amazonaws.com.cn", global = FALSE), "^us\\-gov\\-\\w+\\-\\d+$" = list(endpoint = "ssm-sap.{region}.amazonaws.com", global = FALSE), "^us\\-iso\\-\\w+\\-\\d+$" = list(endpoint = "ssm-sap.{region}.c2s.ic.gov", global = FALSE), "^us\\-isob\\-\\w+\\-\\d+$" = list(endpoint = "ssm-sap.{region}.sc2s.sgov.gov", global = FALSE), "^eu\\-isoe\\-\\w+\\-\\d+$" = list(endpoint = "ssm-sap.{region}.cloud.adc-e.uk", global = FALSE), "^us\\-isof\\-\\w+\\-\\d+$" = list(endpoint = "ssm-sap.{region}.csp.hci.ic.gov", global = FALSE), "^eusc\\-(de)\\-\\w+\\-\\d+$" = list(endpoint = "ssm-sap.{region}.amazonaws.eu", global = FALSE)),
  service_id = "Ssm Sap",
