...
#' Cancels a Handshake.
#' 
#' Only the account that sent a handshake can call this operation. The recipient of the handshake can't cancel it, but can use [`decline_handshake`][organizations_decline_handshake] to decline. After a handshake is canceled, the recipient can no longer respond to the handshake.
#' 
...
#' 
#' -   Use the `AccountId` that you sent in the [`close_account`][organizations_close_account] request to provide as a parameter to the [`describe_account`][organizations_describe_account] operation.
#' 
...
#' 
#' -   Use the `Id` value of the `CreateAccountStatus` response element from this operation to provide as a parameter to the [`describe_create_account_status`][organizations_describe_create_account_status] operation.
#' 
...
#' 
#'     For more information, see [Verifying CloudTrail Is Enabled](https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/verifying-cloudtrail.html) in the *Amazon Web Services GovCloud User Guide*.
#' 
#' If the request includes tags, then the requester must have the `organizations:TagResource` permission. The tags are attached to the commercial account associated with the GovCloud account, rather than the GovCloud account itself. To add tags to the GovCloud account, call the [`tag_resource`][organizations_tag_resource] operation in the GovCloud Region after the new GovCloud account exists.
#' 
...
#' 
#' When you call the [`create_gov_cloud_account`][organizations_create_gov_cloud_account] action, you create two accounts: a standalone account in the Amazon Web Services GovCloud (US) Region and an associated account in the commercial Region for billing and support purposes. The account in the commercial Region is automatically a member of the organization whose credentials made the request. Both accounts are associated with the same email address.
#' 
...
#' -   If you get an exception that indicates that the operation failed because your organization is still initializing, wait one hour and then try again. If the error persists, contact Amazon Web Services Support.
#' 
#' -   Using [`create_gov_cloud_account`][organizations_create_gov_cloud_account] to create multiple temporary accounts isn't recommended. You can only close an account from the Amazon Web Services Billing and Cost Management console, and you must be signed in as the root user. For information on the requirements and process for closing an account, see [Closing a member account in your organization](https://docs.aws.amazon.com/organizations/latest/userguide/orgs_manage_accounts_close.html) in the *Organizations User Guide*.
#' 
...
#' Declines a Handshake.
#' 
#' Only the account that receives a handshake can call this operation. The sender of the handshake can use [`cancel_handshake`][organizations_cancel_handshake] to cancel if the handshake hasn't yet been responded to.
#' 
#' You can view canceled handshakes in API responses for 30 days before they are deleted.
#'
#' @usage
...
#' You can call this operation from any account in a organization.
#' 
#' Even if a policy type is shown as available in the organization, you can disable it separately at the root level with [`disable_policy_type`][organizations_disable_policy_type]. Use [`list_roots`][organizations_list_roots] to see the status of policy types for a specified root.
#'
#' @usage
...
#' 
#' Every root, OU, and account must have at least one SCP attached. If you want to replace the default `FullAWSAccess` policy with an SCP that limits the permissions that can be delegated, you must attach the replacement SCP before you can remove the default SCP. This is the authorization strategy of an "[allow list](https://docs.aws.amazon.com/organizations/latest/userguide/#orgs_policies_allowlist)". If you instead attach a second SCP and leave the `FullAWSAccess` SCP still attached, and specify `"Effect": "Deny"` in the second SCP to override the `"Effect": "Allow"` in the `FullAWSAccess` policy (or any other attached SCP), you're using the authorization strategy of a "[deny list](https://docs.aws.amazon.com/organizations/latest/userguide/#orgs_policies_denylist)".
#' 
#' You can only call this operation from the management account or a member account that is a delegated administrator.
#'
#' @usage
...
#' 
#' After you perform the [`disable_aws_service_access`][organizations_disable_aws_service_access] operation, the specified service can no longer perform operations in your organization's accounts
#' 
...
#' 
#' This is an asynchronous request that Amazon Web Services performs in the background. If you disable a policy type for a root, it still appears enabled for the organization if [all features](https://docs.aws.amazon.com/organizations/latest/userguide/orgs_manage_org_support-all-features.html) are enabled for the organization. Amazon Web Services recommends that you first use [`list_roots`][organizations_list_roots] to see the status of policy types for a specified root, and then use this operation.
#' 
#' You can only call this operation from the management account or a member account that is a delegated administrator.
#' 
#' To view the status of available policy types in the organization, use [`list_roots`][organizations_list_roots].
#'
#' @usage
...
#' 
#' After you enable all features, you can separately enable or disable individual policy types in a root using [`enable_policy_type`][organizations_enable_policy_type] and [`disable_policy_type`][organizations_disable_policy_type]. To see the status of policy types in a root, use [`list_roots`][organizations_list_roots].
#' 
...
#' 
#' This is an asynchronous request that Amazon Web Services performs in the background. Amazon Web Services recommends that you first use [`list_roots`][organizations_list_roots] to see the status of policy types for a specified root, and then use this operation.
#' 
#' You can only call this operation from the management account or a member account that is a delegated administrator.
#' 
#' You can enable a policy type in a root only if that policy type is available in the organization. To view the status of available policy types in the organization, use [`list_roots`][organizations_list_roots].
#'
#' @usage
...
#' 
#' -   If you are using an organization principal to call [`leave_organization`][organizations_leave_organization] across multiple accounts, you can only do this up to 5 accounts per second in a single organization.
#'
#' @usage
...
#' You can only call this operation from the management account or a member account that is a delegated administrator.
#' 
#' Policy types can be enabled and disabled in roots. This is distinct from whether they're available in the organization. When you enable all features, you make policy types available for use in that organization. Individual policy types can then be enabled and disabled in a root. To see the availability of a policy type in an organization, use [`describe_organization`][organizations_describe_organization].
#'
#' @usage
