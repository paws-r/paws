...
#' **Introduction**
#' 
#' The Amazon IVS Chat control-plane API enables you to create and manage Amazon IVS Chat resources. You also need to integrate with the [Amazon IVS Chat Messaging API](https://docs.aws.amazon.com/ivs/latest/chatmsgapireference/), to enable users to interact with chat rooms in real time.
#' 
...
#' 
#' This document describes HTTP operations. There is a separate *messaging* API for managing Chat resources; see the [Amazon IVS Chat Messaging API Reference](https://docs.aws.amazon.com/ivs/latest/chatmsgapireference/).
#' 
...
#' 
#' The Amazon IVS Chat API has these tag-related operations: [`tag_resource`][ivschat_tag_resource], [`untag_resource`][ivschat_untag_resource], and [`list_tags_for_resource`][ivschat_list_tags_for_resource]. The following resource supports tagging: Room.
#' 
...
#' 
#' Users (viewers) connect to a room using secure access tokens that you create using the [`create_chat_token`][ivschat_create_chat_token] operation through the AWS SDK. You call CreateChatToken for every user’s chat session, passing identity and authorization information about the user.
#' 
...
.ivschat$metadata <- list(
  ...
  endpoints = list("^(us|eu|ap|sa|ca|me|af|il|mx)\\-\\w+\\-\\d+$" = list(endpoint = "ivschat.{region}.amazonaws.com", global = FALSE), "^cn\\-\\w+\\-\\d+$" = list(endpoint = "ivschat.{region}.amazonaws.com.cn", global = FALSE), "^us\\-gov\\-\\w+\\-\\d+$" = list(endpoint = "ivschat.{region}.amazonaws.com", global = FALSE), "^us\\-iso\\-\\w+\\-\\d+$" = list(endpoint = "ivschat.{region}.c2s.ic.gov", global = FALSE), "^us\\-isob\\-\\w+\\-\\d+$" = list(endpoint = "ivschat.{region}.sc2s.sgov.gov", global = FALSE), "^eu\\-isoe\\-\\w+\\-\\d+$" = list(endpoint = "ivschat.{region}.cloud.adc-e.uk", global = FALSE), "^us\\-isof\\-\\w+\\-\\d+$" = list(endpoint = "ivschat.{region}.csp.hci.ic.gov", global = FALSE), "^eusc\\-(de)\\-\\w+\\-\\d+$" = list(endpoint = "ivschat.{region}.amazonaws.eu", global = FALSE)),
  service_id = "ivschat",
