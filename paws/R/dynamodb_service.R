...
#'  \link[=dynamodb_delete_resource_policy]{delete_resource_policy} \tab Deletes the resource-based policy attached to the resource, which can be a table or stream\cr
...
#'  \link[=dynamodb_get_resource_policy]{get_resource_policy} \tab Returns the resource-based policy document attached to the resource, which can be a table or stream, in JSON format\cr
#'  \link[=dynamodb_import_table]{import_table} \tab Imports table data from an S3 bucket\cr
#'  \link[=dynamodb_list_backups]{list_backups} \tab List DynamoDB backups that are associated with an Amazon Web Services account and weren't made with Amazon Web Services Backup\cr
...
#'  \link[=dynamodb_list_exports]{list_exports} \tab Lists completed exports within the past 90 days, in reverse alphanumeric order of ExportArn\cr
...
#'  \link[=dynamodb_put_resource_policy]{put_resource_policy} \tab Attaches a resource-based policy document to the resource, which can be a table or stream\cr
...
#'  \link[=dynamodb_update_kinesis_streaming_destination]{update_kinesis_streaming_destination} \tab The command to update the Kinesis stream destination\cr
...
.dynamodb$metadata <- list(
  ...
  endpoints = list("^(us|eu|ap|sa|ca|me|af|il|mx)\\-\\w+\\-\\d+$" = list(endpoint = "dynamodb.{region}.amazonaws.com", global = FALSE), "^cn\\-\\w+\\-\\d+$" = list(endpoint = "dynamodb.{region}.amazonaws.com.cn", global = FALSE), "^us\\-gov\\-\\w+\\-\\d+$" = list(endpoint = "dynamodb.{region}.amazonaws.com", global = FALSE), "^us\\-iso\\-\\w+\\-\\d+$" = list(endpoint = "dynamodb.{region}.c2s.ic.gov", global = FALSE), "^us\\-isob\\-\\w+\\-\\d+$" = list(endpoint = "dynamodb.{region}.sc2s.sgov.gov", global = FALSE), "^eu\\-isoe\\-\\w+\\-\\d+$" = list(endpoint = "dynamodb.{region}.cloud.adc-e.uk", global = FALSE), "^us\\-isof\\-\\w+\\-\\d+$" = list(endpoint = "dynamodb.{region}.csp.hci.ic.gov", global = FALSE), "^eusc\\-(de)\\-\\w+\\-\\d+$" = list(endpoint = "dynamodb.{region}.amazonaws.eu", global = FALSE)),
  service_id = "DynamoDB",
