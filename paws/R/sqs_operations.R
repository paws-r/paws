...
#' 
#' When you create a queue, you have full control access rights for the queue. Only you, the owner of the queue, can grant or deny permissions to the queue. For more information about these permissions, see [Allow Developers to Write Messages to a Shared Queue](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-basic-examples-of-iam-policies.html#write-messages-to-shared-queue) in the *Amazon SQS Developer Guide*.
#' 
#' -   [`add_permission`][sqs_add_permission] generates a policy for you. You can use [`set_queue_attributes`][sqs_set_queue_attributes] to upload your policy. For more information, see [Using Custom Policies with the Amazon SQS Access Policy Language](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-creating-custom-policies.html) in the *Amazon SQS Developer Guide*.
#' 
#' -   An Amazon SQS policy can have a maximum of seven actions per statement.
#' 
#' -   To remove the ability to change queue permissions, you must deny permission to the [`add_permission`][sqs_add_permission], [`remove_permission`][sqs_remove_permission], and [`set_queue_attributes`][sqs_set_queue_attributes] actions in your IAM policy.
#' 
#' -   Amazon SQS [`add_permission`][sqs_add_permission] does not support adding a non-account principal.
#' 
...
#' 
#' For example, if the default timeout for a queue is 60 seconds, 15 seconds have elapsed since you received the message, and you send a ChangeMessageVisibility call with `VisibilityTimeout` set to 10 seconds, the 10 seconds begin to count from the time that you make the [`change_message_visibility`][sqs_change_message_visibility] call. Thus, any attempt to change the visibility timeout or to delete that message 10 seconds after you initially change the visibility timeout (a total of 25 seconds) might result in an error.
#' 
...
#' 
#' Unlike with a queue, when you change the visibility timeout for a specific message the timeout value is applied immediately but isn't saved in memory for that message. If you don't delete a message after it is received, the visibility timeout for the message reverts to the original timeout value (not to the value you set using the [`change_message_visibility`][sqs_change_message_visibility] action) the next time the message is received.
#'
#' @usage
...
#' 
#'     You can't change the queue type after you create it and you can't convert an existing standard queue into a FIFO queue. You must either create a new FIFO queue for your application or delete your existing standard queue and recreate it as a FIFO queue. For more information, see [Moving From a standard queue to a FIFO queue](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/#FIFO-queues-moving) in the *Amazon SQS Developer Guide*.
#' 
#' -   If you don't provide a value for an attribute, the queue is created with the default value for the attribute.
#' 
#' -   If you delete a queue, you must wait at least 60 seconds before creating a queue with the same name.
#' 
#' To successfully create a new queue, you must provide a queue name that adheres to the [limits related to queues](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-quotas.html) and is unique within the scope of your queues.
#' 
#' After you create a queue, you must wait at least one second after the queue is created to be able to use the queue.
#' 
#' To retrieve the URL of a queue, use the [`get_queue_url`](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/APIReference/API_GetQueueUrl.html) action. This action only requires the [`QueueName`](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/APIReference/API_CreateQueue.html#API_CreateQueue_RequestSyntax) parameter.
#' 
#' When creating queues, keep the following points in mind:
#' 
#' -   If you specify the name of an existing queue and provide the exact same names and values for all its attributes, the [`create_queue`](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/APIReference/API_CreateQueue.html) action will return the URL of the existing queue instead of creating a new one.
#' 
#' -   If you attempt to create a queue with a name that already exists but with different attribute names or values, the [`create_queue`][sqs_create_queue] action will return an error. This ensures that existing queues are not inadvertently altered.
#' 
#' Cross-account permissions don't apply to this action. For more information, see [Grant cross-account permissions to a role and a username](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-basic-examples-of-sqs-policies.html#grant-cross-account-permissions-to-role-and-user-name) in the *Amazon SQS Developer Guide*.
#'
#' @usage
...
#' 
#' Each time you receive a message, meaning when a consumer retrieves a message from the queue, it comes with a unique `ReceiptHandle`. If you receive the same message more than once, you will get a different `ReceiptHandle` each time. When you want to delete a message using the [`delete_message`][sqs_delete_message] action, you must use the `ReceiptHandle` from the most recent time you received the message. If you use an old `ReceiptHandle`, the request will succeed, but the message might not be deleted.
#' 
...
#' 
#' Be careful with the [`delete_queue`][sqs_delete_queue] action: When you delete a queue, any messages in the queue are no longer available.
#' 
#' When you delete a queue, the deletion process takes up to 60 seconds. Requests you send involving that queue during the 60 seconds might succeed. For example, a [`send_message`][sqs_send_message] request might succeed, but after 60 seconds the queue and the message you sent no longer exist.
#' 
...
#' 
#' The [`list_dead_letter_source_queues`][sqs_list_dead_letter_source_queues] methods supports pagination. Set parameter `MaxResults` in the request to specify the maximum number of results to be returned in the response. If you do not set `MaxResults`, the response includes a maximum of 1,000 results. If you set `MaxResults` and there are additional results to display, the response includes a value for `NextToken`. Use `NextToken` as a parameter in your next request to [`list_dead_letter_source_queues`][sqs_list_dead_letter_source_queues] to receive the next page of results.
#' 
...
#' 
#' When you use the [`purge_queue`][sqs_purge_queue] action, you can't retrieve any messages deleted from a queue.
#' 
#' The message deletion process takes up to 60 seconds. We recommend waiting for 60 seconds regardless of your queue's size.
#' 
#' Messages sent to the queue *before* you call [`purge_queue`][sqs_purge_queue] might be received but are deleted within the next minute.
#' 
#' Messages sent to the queue *after* you call [`purge_queue`][sqs_purge_queue] might be deleted while the queue is being purged.
#'
#' @usage
...
#' 
#' Short poll is the default behavior where a weighted random set of machines is sampled on a [`receive_message`][sqs_receive_message] call. Therefore, only the messages on the sampled machines are returned. If the number of messages in the queue is small (fewer than 1,000), you most likely get fewer messages than you requested per [`receive_message`][sqs_receive_message] call. If the number of messages in the queue is extremely small, you might not receive any messages in a particular [`receive_message`][sqs_receive_message] response. If this happens, repeat the request.
#' 
...
#' 
#' -   In dead-letter queues redrive context, the [`start_message_move_task`][sqs_start_message_move_task] the source queue is the DLQ, while the destination queue can be the original source queue (from which the messages were driven to the dead-letter-queue), or a custom destination queue.
#' 
#' -   Only one active message movement task is supported per queue at any given time.
#'
#' @usage
