...
#' 
#' To verify that all parts have been removed and prevent getting charged for the part storage, you should call the [`list_parts`][s3_list_parts] API operation and ensure that the parts list is empty.
#' 
#' -   **Directory buckets** - If multipart uploads in a directory bucket are in progress, you can't delete the bucket until all the in-progress multipart uploads are aborted or completed. To delete these in-progress multipart uploads, use the [`list_multipart_uploads`][s3_list_multipart_uploads] operation to list the in-progress multipart uploads in the bucket and use the [`abort_multipart_upload`][s3_abort_multipart_upload] operation to abort all the in-progress multipart uploads.
#' 
...
#' ### Permissions
#' 
#' -   **General purpose bucket permissions** - For information about permissions required to use the multipart upload, see [Multipart Upload and Permissions](https://docs.aws.amazon.com/AmazonS3/latest/userguide/mpuoverview.html) in the *Amazon S3 User Guide*.
#' 
...
#' 
#' The following operations are related to [`abort_multipart_upload`][s3_abort_multipart_upload]:
#' 
...
#' Completes a multipart upload by assembling previously uploaded parts.
#' 
#' You first initiate the multipart upload and then upload all parts using the [`upload_part`][s3_upload_part] operation or the [`upload_part_copy`][s3_upload_part_copy] operation. After successfully uploading all relevant parts of an upload, you call this [`complete_multipart_upload`][s3_complete_multipart_upload] operation to complete the upload. Upon receiving this request, Amazon S3 concatenates all the parts in ascending order by part number to create a new object. In the CompleteMultipartUpload request, you must provide the parts list and ensure that the parts list is complete. The CompleteMultipartUpload API operation concatenates the parts that you provide in the list. For each part in the list, you must provide the `PartNumber` value and the `ETag` value that are returned after that part was uploaded.
#' 
...
#' 
#' Note that if [`complete_multipart_upload`][s3_complete_multipart_upload] fails, applications should be prepared to retry any failed requests (including 500 error responses). For more information, see [Amazon S3 Error Best Practices](https://docs.aws.amazon.com/AmazonS3/latest/userguide/).
#' 
#' You can't use `Content-Type: application/x-www-form-urlencoded` for the CompleteMultipartUpload requests. Also, if you don't provide a `Content-Type` header, [`complete_multipart_upload`][s3_complete_multipart_upload] can still return a `200 OK` response.
#' 
#' For more information about multipart uploads, see [Uploading Objects Using Multipart Upload](https://docs.aws.amazon.com/AmazonS3/latest/userguide/mpuoverview.html) in the *Amazon S3 User Guide*.
#' 
#' **Directory buckets** - For directory buckets, you must make requests for this API operation to the Zonal endpoint. These endpoints support virtual-hosted-style requests in the format `https://amzn-s3-demo-bucket.s3express-zone-id.region-code.amazonaws.com/key-name `. Path-style requests are not supported. For more information about endpoints in Availability Zones, see [Regional and Zonal endpoints for directory buckets in Availability Zones](https://docs.aws.amazon.com/AmazonS3/latest/userguide/endpoint-directory-buckets-AZ.html) in the *Amazon S3 User Guide*. For more information about endpoints in Local Zones, see [Concepts for directory buckets in Local Zones](https://docs.aws.amazon.com/AmazonS3/latest/userguide/s3-lzs-for-directory-buckets.html) in the *Amazon S3 User Guide*.
#' 
#' ### Permissions
#' 
#' -   **General purpose bucket permissions** - For information about permissions required to use the multipart upload API, see [Multipart Upload and Permissions](https://docs.aws.amazon.com/AmazonS3/latest/userguide/mpuoverview.html) in the *Amazon S3 User Guide*.
#' 
#'     If you provide an [additional checksum value](https://docs.aws.amazon.com/AmazonS3/latest/API/API_Checksum.html) in your `MultipartUpload` requests and the object is encrypted with Key Management Service, you must have permission to use the `kms:Decrypt` action for the [`complete_multipart_upload`][s3_complete_multipart_upload] request to succeed.
#' 
#' -   **Directory bucket permissions** - To grant access to this API operation on a directory bucket, we recommend that you use the [`create_session`](https://docs.aws.amazon.com/AmazonS3/latest/API/API_CreateSession.html) API operation for session-based authorization. Specifically, you grant the `s3express:CreateSession` permission to the directory bucket in a bucket policy or an IAM identity-based policy. Then, you make the [`create_session`][s3_create_session] API call on the bucket to obtain a session token. With the session token in your request header, you can make API requests to this operation. After the session token expires, you make another [`create_session`][s3_create_session] API call to generate a new session token for use. Amazon Web Services CLI or SDKs create session and refresh the session token automatically to avoid service interruptions when a session expires. For more information about authorization, see [`create_session`](https://docs.aws.amazon.com/AmazonS3/latest/API/API_CreateSession.html) .
#' 
...
#' **Directory buckets** - The HTTP Host header syntax is ` Bucket-name.s3express-zone-id.region-code.amazonaws.com`.
#' 
#' The following operations are related to [`complete_multipart_upload`][s3_complete_multipart_upload]:
#' 
#' -   [`create_multipart_upload`][s3_create_multipart_upload]
#' 
#' -   [`upload_part`][s3_upload_part]
...
#' @param ChecksumType This header specifies the checksum type of the object, which determines how part-level checksums are combined to create an object-level checksum for multipart objects. You can use this header as a data integrity check to verify that the checksum type that is received is the same checksum that was specified. If the checksum type doesn’t match the checksum type that was specified for the object during the [`create_multipart_upload`][s3_create_multipart_upload] request, it’ll result in a `BadDigest` error. For more information, see Checking object integrity in the Amazon S3 User Guide.
#' @param MpuObjectSize The expected total object size of the multipart upload request. If there’s a mismatch between the specified object size value and the actual object size value, it results in an `HTTP 400 InvalidRequest` error.
#' @param RequestPayer Confirms that the requester knows that they will be charged for the request. Bucket owners need not specify this parameter in their requests. If either the source or destination S3 bucket has Requester Pays enabled, the requester will pay for the corresponding charges. For information about downloading objects from Requester Pays buckets, see [Downloading Objects in Requester Pays Buckets](https://docs.aws.amazon.com/AmazonS3/latest/userguide/ObjectsinRequesterPaysBuckets.html) in the *Amazon S3 User Guide*.
#' 
#' This functionality is not supported for directory buckets.
#' @param ExpectedBucketOwner The account ID of the expected bucket owner. If the account ID that you provide does not match the actual owner of the bucket, the request fails with the HTTP status code `403 Forbidden` (access denied).
...
#' 
#' If a conflicting operation occurs during the upload S3 returns a `409 ConditionalRequestConflict` response. On a 409 failure you should fetch the object's ETag, re-initiate the multipart upload with [`create_multipart_upload`][s3_create_multipart_upload], and re-upload each part.
#' 
...
#' 
#' This functionality is not supported for directory buckets.
#' @param SSECustomerKey The server-side encryption (SSE) customer managed key. This parameter is needed only when the object was created using a checksum algorithm. For more information, see [Protecting data using SSE-C keys](https://docs.aws.amazon.com/AmazonS3/latest/userguide/ServerSideEncryptionCustomerKeys.html) in the *Amazon S3 User Guide*.
#' 
#' This functionality is not supported for directory buckets.
#' @param SSECustomerKeyMD5 The MD5 server-side encryption (SSE) customer managed key. This parameter is needed only when the object was created using a checksum algorithm. For more information, see [Protecting data using SSE-C keys](https://docs.aws.amazon.com/AmazonS3/latest/userguide/ServerSideEncryptionCustomerKeys.html) in the *Amazon S3 User Guide*.
#' 
#' This functionality is not supported for directory buckets.
#'
#' @return
...
#' 
#' Amazon S3 transfer acceleration does not support cross-Region copies. If you request a cross-Region copy using a transfer acceleration endpoint, you get a `400 Bad Request` error. For more information, see [Transfer Acceleration](https://docs.aws.amazon.com/AmazonS3/latest/userguide/transfer-acceleration.html).
#' 
...
#' 
#' -   **General purpose bucket permissions** - You must have permissions in an IAM policy based on the source and destination bucket types in a [`copy_object`][s3_copy_object] operation.
#' 
...
#' 
#' -   **Directory bucket permissions** - You must have permissions in a bucket policy or an IAM identity-based policy based on the source and destination bucket types in a [`copy_object`][s3_copy_object] operation.
#' 
...
#' ### Charge
#' 
#' The copy request charge is based on the storage class and Region that you specify for the destination object. The request can also result in a data retrieval charge for the source if the source storage class bills for data retrieval. If the copy source is in a different region, the data transfer is billed to the copy source account. For pricing information, see [Amazon S3 pricing](https://aws.amazon.com/s3/pricing/).
#' 
#' ### HTTP Host header syntax
#' 
#' -   **Directory buckets** - The HTTP Host header syntax is ` Bucket-name.s3express-zone-id.region-code.amazonaws.com`.
...
#' 
#' When you copy an object, if the source object has a checksum, that checksum value will be copied to the new object by default. If the [`copy_object`][s3_copy_object] request does not include this `x-amz-checksum-algorithm` header, the checksum algorithm will be copied from the source object to the destination object (if it's present on the source object). You can optionally specify a different checksum algorithm to use with the `x-amz-checksum-algorithm` header. Unrecognized or unsupported values will respond with the HTTP status code `400 Bad Request`.
#' 
#' For directory buckets, when you use Amazon Web Services SDKs, `CRC32` is the default checksum algorithm that's used for performance.
#' @param ContentDisposition Specifies presentational information for the object. Indicates whether an object should be displayed in a web browser or downloaded as a file. It allows specifying the desired filename for the downloaded file.
...
#' @param CopySource &#91;required&#93; Specifies the source object for the copy operation. The source object can be up to 5 GB. If the source object is an object that was uploaded by using a multipart upload, the object copy will be a single part object after the source object is copied to the destination bucket.
#' 
...
#' 
#' **Directory buckets** - For directory buckets in a [`copy_object`][s3_copy_object] operation, only the empty tag-set is supported. Any requests that attempt to write non-empty tags into directory buckets will receive a `501 Not Implemented` status code. When the destination bucket is a directory bucket, you will receive a `501 Not Implemented` response in any of the following situations:
#' 
...
#' 
#' Because only the empty tag-set is supported for directory buckets in a [`copy_object`][s3_copy_object] operation, the following situations are allowed:
#' 
...
#' 
#' -   When you perform a [`copy_object`][s3_copy_object] operation, if you want to use a different type of encryption setting for the target object, you can specify appropriate encryption-related headers to encrypt the target object with an Amazon S3 managed key, a KMS key, or a customer-provided key. If the encryption setting in your request is different from the default encryption configuration of the destination bucket, the encryption setting in your request takes precedence.
#' 
#' **Directory buckets**
#' 
...
#' 
#' You can use the [`copy_object`][s3_copy_object] action to change the storage class of an object that is already stored in Amazon S3 by using the `x-amz-storage-class` header. For more information, see [Storage Classes](https://docs.aws.amazon.com/AmazonS3/latest/userguide/storage-class-intro.html) in the *Amazon S3 User Guide*.
#' 
...
#' 
#' For more information, see [`restore_object`][s3_restore_object] and [Copying Objects](https://docs.aws.amazon.com/AmazonS3/latest/userguide/copy-object.html) in the *Amazon S3 User Guide*.
...
#' 
#' This functionality is not supported when the destination bucket is a directory bucket.
#' @param SSEKMSKeyId Specifies the KMS key ID (Key ID, Key ARN, or Key Alias) to use for object encryption. All GET and PUT requests for an object protected by KMS will fail if they're not made via SSL or using SigV4. For information about configuring any of the officially supported Amazon Web Services SDKs and Amazon Web Services CLI, see [Specifying the Signature Version in Request Authentication](https://docs.aws.amazon.com/AmazonS3/latest/API/#specify-signature-version) in the *Amazon S3 User Guide*.
#' 
...
#' @param SSEKMSEncryptionContext Specifies the Amazon Web Services KMS Encryption Context as an additional encryption context to use for the destination object encryption. The value of this header is a base64-encoded UTF-8 string holding JSON with the encryption context key-value pairs.
#' 
...
#' 
#' For more information, see [Amazon S3 Bucket Keys](https://docs.aws.amazon.com/AmazonS3/latest/userguide/bucket-key.html) in the *Amazon S3 User Guide*.
#' 
...
#' 
#' -   **General purpose buckets** - If you send your [`create_bucket`][s3_create_bucket] request to the `s3.amazonaws.com` global endpoint, the request goes to the `us-east-1` Region. So the signature calculations in Signature Version 4 must use `us-east-1` as the Region, even if the location constraint in the request specifies another Region where the bucket is to be created. If you create a bucket in a Region other than US East (N. Virginia), your application must be able to handle 307 redirect. For more information, see [Virtual hosting of buckets](https://docs.aws.amazon.com/AmazonS3/latest/userguide/VirtualHosting.html) in the *Amazon S3 User Guide*.
#' 
#' -   **Directory buckets** - For directory buckets, you must make requests for this API operation to the Regional endpoint. These endpoints support path-style requests in the format `https://s3express-control.region-code.amazonaws.com/bucket-name `. Virtual-hosted-style requests aren't supported. For more information about endpoints in Availability Zones, see [Regional and Zonal endpoints for directory buckets in Availability Zones](https://docs.aws.amazon.com/AmazonS3/latest/userguide/endpoint-directory-buckets-AZ.html) in the *Amazon S3 User Guide*. For more information about endpoints in Local Zones, see [Concepts for directory buckets in Local Zones](https://docs.aws.amazon.com/AmazonS3/latest/userguide/s3-lzs-for-directory-buckets.html) in the *Amazon S3 User Guide*.
#' 
#' ### Permissions
#' 
#' -   **General purpose bucket permissions** - In addition to the `s3:CreateBucket` permission, the following permissions are required in a policy when your [`create_bucket`][s3_create_bucket] request includes specific headers:
#' 
#'     -   **Access control lists (ACLs)** - In your [`create_bucket`][s3_create_bucket] request, if you specify an access control list (ACL) and set it to `public-read`, `public-read-write`, `authenticated-read`, or if you explicitly specify any other custom ACLs, both `s3:CreateBucket` and `s3:PutBucketAcl` permissions are required. In your [`create_bucket`][s3_create_bucket] request, if you set the ACL to `private`, or if you don't specify any ACLs, only the `s3:CreateBucket` permission is required.
#' 
#'     -   **Object Lock** - In your [`create_bucket`][s3_create_bucket] request, if you set `x-amz-bucket-object-lock-enabled` to true, the `s3:PutBucketObjectLockConfiguration` and `s3:PutBucketVersioning` permissions are required.
#' 
#'     -   **S3 Object Ownership** - If your [`create_bucket`][s3_create_bucket] request includes the `x-amz-object-ownership` header, then the `s3:PutBucketOwnershipControls` permission is required.
#' 
#'         To set an ACL on a bucket as part of a [`create_bucket`][s3_create_bucket] request, you must explicitly set S3 Object Ownership for the bucket to a different value than the default, `BucketOwnerEnforced`. Additionally, if your desired bucket ACL grants public access, you must first create the bucket (without the bucket ACL) and then explicitly disable Block Public Access on the bucket before using [`put_bucket_acl`][s3_put_bucket_acl] to set the ACL. If you try to create a bucket with a public ACL, the request will fail.
#' 
...
#' @param BucketNamespace Specifies the namespace where you want to create your general purpose bucket. When you create a general purpose bucket, you can choose to create a bucket in the shared global namespace or you can choose to create a bucket in your account regional namespace. Your account regional namespace is a subdivision of the global namespace that only your account can create buckets in. For more information on bucket namespaces, see [Namespaces for general purpose buckets](https://docs.aws.amazon.com/AmazonS3/latest/userguide/gpbucketnamespaces.html).
#' 
...
#'
#' @param Bucket &#91;required&#93; The general purpose bucket that you want to create the metadata configuration for.
...
#' @param ExpectedBucketOwner The expected owner of the general purpose bucket that corresponds to your metadata configuration.
#'
#' @return
#' An empty list.
#'
...
#' 
#' If you created your S3 Metadata configuration before July 15, 2025, we recommend that you delete and re-create your configuration by using [`create_bucket_metadata_configuration`][s3_create_bucket_metadata_configuration] so that you can expire journal table records and create a live inventory table.
#' 
...
#' -   `s3tables:PutTablePolicy`
#' 
#' The following operations are related to [`create_bucket_metadata_table_configuration`][s3_create_bucket_metadata_table_configuration]:
#' 
...
#'
#' @param Bucket &#91;required&#93; The general purpose bucket that you want to create the metadata table configuration for.
...
#' @param ExpectedBucketOwner The expected owner of the general purpose bucket that corresponds to your metadata table configuration.
#'
#' @return
#' An empty list.
#'
...
#' 
#'     For directory buckets, when you perform a [`create_multipart_upload`][s3_create_multipart_upload] operation and an [`upload_part_copy`][s3_upload_part_copy] operation, the request headers you provide in the [`create_multipart_upload`][s3_create_multipart_upload] request must match the default encryption configuration of the destination bucket.
#' 
#' ### HTTP Host header syntax
#' 
#' **Directory buckets** - The HTTP Host header syntax is ` Bucket-name.s3express-zone-id.region-code.amazonaws.com`.
#' 
#' The following operations are related to [`create_multipart_upload`][s3_create_multipart_upload]:
#' 
#' -   [`upload_part`][s3_upload_part]
#' 
#' -   [`complete_multipart_upload`][s3_complete_multipart_upload]
...
#' @param Metadata A map of metadata to store with the object in S3.
#' @param ServerSideEncryption The server-side encryption algorithm used when you store this object in Amazon S3 or Amazon FSx.
#' 
#' -   **Directory buckets** - For directory buckets, there are only two supported options for server-side encryption: server-side encryption with Amazon S3 managed keys (SSE-S3) (`AES256`) and server-side encryption with KMS keys (SSE-KMS) (`aws:kms`). We recommend that the bucket's default encryption uses the desired encryption configuration and you don't override the bucket default encryption in your [`create_session`][s3_create_session] requests or `PUT` object requests. Then, new objects are automatically encrypted with the desired encryption settings. For more information, see [Protecting data with server-side encryption](https://docs.aws.amazon.com/AmazonS3/latest/userguide/) in the *Amazon S3 User Guide*. For more information about the encryption overriding behaviors in directory buckets, see [Specifying server-side encryption with KMS for new object uploads](https://docs.aws.amazon.com/AmazonS3/latest/userguide/s3-express-specifying-kms-encryption.html).
#' 
#'     In the Zonal endpoint API calls (except [`copy_object`][s3_copy_object] and [`upload_part_copy`][s3_upload_part_copy]) using the REST API, the encryption request headers must match the encryption settings that are specified in the [`create_session`][s3_create_session] request. You can't override the values of the encryption settings (`x-amz-server-side-encryption`, `x-amz-server-side-encryption-aws-kms-key-id`, `x-amz-server-side-encryption-context`, and `x-amz-server-side-encryption-bucket-key-enabled`) that are specified in the [`create_session`][s3_create_session] request. You don't need to explicitly specify these encryption settings values in Zonal endpoint API calls, and Amazon S3 will use the encryption settings values from the [`create_session`][s3_create_session] request to protect new objects in the directory bucket.
...
#' 
#' This functionality is not supported for directory buckets.
#' @param SSEKMSKeyId Specifies the KMS key ID (Key ID, Key ARN, or Key Alias) to use for object encryption. If the KMS key doesn't exist in the same account that's issuing the command, you must use the full Key ARN not the Key ID.
#' 
...
#' 
#' **Directory buckets** - To encrypt data using SSE-KMS, it's recommended to specify the `x-amz-server-side-encryption` header to `aws:kms`. Then, the `x-amz-server-side-encryption-aws-kms-key-id` header implicitly uses the bucket's default KMS customer managed key ID. If you want to explicitly set the ` x-amz-server-side-encryption-aws-kms-key-id` header, it must match the bucket's default customer managed key (using key ID or ARN, not alias). Your SSE-KMS configuration can only support 1 [customer managed key](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#customer-cmk) per directory bucket's lifetime. The [Amazon Web Services managed key](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#aws-managed-cmk) (`aws/s3`) isn't supported. Incorrect key specification results in an HTTP `400 Bad Request` error.
#' @param SSEKMSEncryptionContext Specifies the Amazon Web Services KMS Encryption Context to use for object encryption. The value of this header is a Base64 encoded string of a UTF-8 encoded JSON, which contains the encryption context as key-value pairs.
#' 
#' **Directory buckets** - You can optionally provide an explicit encryption context value. The value must match the default encryption context - the bucket Amazon Resource Name (ARN). An additional encryption context value is not supported.
#' @param BucketKeyEnabled Specifies whether Amazon S3 should use an S3 Bucket Key for object encryption with server-side encryption using Key Management Service (KMS) keys (SSE-KMS).
#' 
...
#' 
#' To make Zonal endpoint API requests on a directory bucket, use the [`create_session`][s3_create_session] API operation. Specifically, you grant `s3express:CreateSession` permission to a bucket in a bucket policy or an IAM identity-based policy. Then, you use IAM credentials to make the [`create_session`][s3_create_session] API request on the bucket, which returns temporary security credentials that include the access key ID, secret access key, session token, and expiration. These credentials have associated permissions to access the Zonal endpoint API operations. After the session is created, you don’t need to use other policies to grant permissions to each Zonal endpoint API individually. Instead, in your Zonal endpoint API requests, you sign your requests by applying the temporary security credentials of the session to the request headers and following the SigV4 protocol for authentication. You also apply the session token to the `x-amz-s3session-token` request header for authorization. Temporary security credentials are scoped to the bucket and expire after 5 minutes. After the expiration time, any calls that you make with those credentials will fail. You must use IAM credentials again to make a [`create_session`][s3_create_session] API request that generates a new set of temporary credentials for use. Temporary credentials cannot be extended or refreshed beyond the original specified interval.
#' 
#' If you use Amazon Web Services SDKs, SDKs handle the session token refreshes automatically to avoid service interruptions when a session expires. We recommend that you use the Amazon Web Services SDKs to initiate and manage requests to the CreateSession API. For more information, see [Performance guidelines and design patterns](https://docs.aws.amazon.com/AmazonS3/latest/userguide/#s3-express-optimizing-performance-session-authentication) in the *Amazon S3 User Guide*.
#' 
#' -   You must make requests for this API operation to the Zonal endpoint. These endpoints support virtual-hosted-style requests in the format `https://bucket-name.s3express-zone-id.region-code.amazonaws.com`. Path-style requests are not supported. For more information about endpoints in Availability Zones, see [Regional and Zonal endpoints for directory buckets in Availability Zones](https://docs.aws.amazon.com/AmazonS3/latest/userguide/endpoint-directory-buckets-AZ.html) in the *Amazon S3 User Guide*. For more information about endpoints in Local Zones, see [Concepts for directory buckets in Local Zones](https://docs.aws.amazon.com/AmazonS3/latest/userguide/s3-lzs-for-directory-buckets.html) in the *Amazon S3 User Guide*.
#' 
#' -   **[`copy_object`][s3_copy_object] API operation** - Unlike other Zonal endpoint API operations, the [`copy_object`][s3_copy_object] API operation doesn't use the temporary security credentials returned from the [`create_session`][s3_create_session] API operation for authentication and authorization. For information about authentication and authorization of the [`copy_object`][s3_copy_object] API operation on directory buckets, see [`copy_object`][s3_copy_object].
#' 
#' -   **[`head_bucket`][s3_head_bucket] API operation** - Unlike other Zonal endpoint API operations, the [`head_bucket`][s3_head_bucket] API operation doesn't use the temporary security credentials returned from the [`create_session`][s3_create_session] API operation for authentication and authorization. For information about authentication and authorization of the [`head_bucket`][s3_head_bucket] API operation on directory buckets, see [`head_bucket`][s3_head_bucket].
#' 
#' ### Permissions
#' 
...
#' 
#' When you use the CLI or the Amazon Web Services SDKs, for [`create_session`][s3_create_session], the session token refreshes automatically to avoid service interruptions when a session expires. The CLI or the Amazon Web Services SDKs use the bucket's default encryption configuration for the [`create_session`][s3_create_session] request. It's not supported to override the encryption settings values in the [`create_session`][s3_create_session] request. Also, in the Zonal endpoint API calls (except [`copy_object`][s3_copy_object] and [`upload_part_copy`][s3_upload_part_copy]), it's not supported to override the values of the encryption settings from the [`create_session`][s3_create_session] request.
#' 
#' ### HTTP Host header syntax
#' 
#' **Directory buckets** - The HTTP Host header syntax is ` Bucket-name.s3express-zone-id.region-code.amazonaws.com`.
...
#' 
#' **S3 access points for Amazon FSx** - When accessing data stored in Amazon FSx file systems using S3 access points, the only valid server side encryption option is `aws:fsx`. All Amazon FSx file systems have encryption configured by default and are encrypted at rest. Data is automatically encrypted before being written to the file system, and automatically decrypted as it is read. These processes are handled transparently by Amazon FSx.
#' @param SSEKMSKeyId If you specify `x-amz-server-side-encryption` with `aws:kms`, you must specify the ` x-amz-server-side-encryption-aws-kms-key-id` header with the ID (Key ID or Key ARN) of the KMS symmetric encryption customer managed key to use. Otherwise, you get an HTTP `400 Bad Request` error. Only use the key ID or key ARN. The key alias format of the KMS key isn't supported. Also, if the KMS key doesn't exist in the same account that't issuing the command, you must use the full Key ARN not the Key ID.
#' 
#' Your SSE-KMS configuration can only support 1 [customer managed key](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#customer-cmk) per directory bucket's lifetime. The [Amazon Web Services managed key](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#aws-managed-cmk) (`aws/s3`) isn't supported.
#' @param SSEKMSEncryptionContext Specifies the Amazon Web Services KMS Encryption Context as an additional encryption context to use for object encryption. The value of this header is a Base64 encoded string of a UTF-8 encoded JSON, which contains the encryption context as key-value pairs. This value is stored as object metadata and automatically gets passed on to Amazon Web Services KMS for future [`get_object`][s3_get_object] operations on this object.
#' 
#' **General purpose buckets** - This value must be explicitly added during [`copy_object`][s3_copy_object] operations if you want an additional encryption context for your object. For more information, see [Encryption context](https://docs.aws.amazon.com/AmazonS3/latest/userguide/UsingKMSEncryption.html#encryption-context) in the *Amazon S3 User Guide*.
#' 
#' **Directory buckets** - You can optionally provide an explicit encryption context value. The value must match the default encryption context - the bucket Amazon Resource Name (ARN). An additional encryption context value is not supported.
#' @param BucketKeyEnabled Specifies whether Amazon S3 should use an S3 Bucket Key for object encryption with server-side encryption using KMS keys (SSE-KMS).
#' 
...
#' **Directory buckets** - The HTTP Host header syntax is `s3express-control.region-code.amazonaws.com`.
#' 
#' The following operations are related to [`delete_bucket`][s3_delete_bucket]:
#' 
...
#' 
#' For information about the Amazon S3 analytics feature, see [Amazon S3 Analytics – Storage Class Analysis](https://docs.aws.amazon.com/AmazonS3/latest/userguide/analytics-storage-class.html).
#' 
#' The following operations are related to [`delete_bucket_analytics_configuration`][s3_delete_bucket_analytics_configuration]:
#' 
...
#' 
#' For information about `cors`, see [Enabling Cross-Origin Resource Sharing](https://docs.aws.amazon.com/AmazonS3/latest/userguide/cors.html) in the *Amazon S3 User Guide*.
#' 
#' **Related Resources**
#' 
...
#' **Directory buckets** - The HTTP Host header syntax is `s3express-control.region-code.amazonaws.com`.
#' 
#' The following operations are related to [`delete_bucket_encryption`][s3_delete_bucket_encryption]:
#' 
...
#' 
#' For more information, see [Storage class for automatically optimizing frequently and infrequently accessed objects](https://docs.aws.amazon.com/AmazonS3/latest/userguide/storage-class-intro.html#sc-dynamic-data-access).
#' 
#' Operations related to [`delete_bucket_intelligent_tiering_configuration`][s3_delete_bucket_intelligent_tiering_configuration] include:
#' 
...
#'   ExpectedBucketOwner)
#'
#' @param Bucket &#91;required&#93; The name of the Amazon S3 bucket whose configuration you want to modify or retrieve.
...
#' **Directory buckets** - The HTTP Host header syntax is `s3express-control.region-code.amazonaws.com`.
#' 
#' For information about the Amazon S3 inventory feature, see [Amazon S3 Inventory](https://docs.aws.amazon.com/AmazonS3/latest/userguide/storage-inventory.html).
#' 
...
#' 
#' Operations related to [`delete_bucket_inventory_configuration`][s3_delete_bucket_inventory_configuration] include:
#' 
...
#' 
#'     For more information about permissions, see [Managing Access Permissions to Your Amazon S3 Resources](https://docs.aws.amazon.com/AmazonS3/latest/userguide/security-iam.html).
#' 
#' 
...
#' 
#' For more information about the object expiration, see [Elements to Describe Lifecycle Actions](https://docs.aws.amazon.com/AmazonS3/latest/userguide/intro-lifecycle-rules.html#intro-lifecycle-rules-actions).
#' 
...
#' 
#' You can use the V2 [`delete_bucket_metadata_configuration`][s3_delete_bucket_metadata_configuration] API operation with V1 or V2 metadata configurations. However, if you try to use the V1 [`delete_bucket_metadata_table_configuration`][s3_delete_bucket_metadata_table_configuration] API operation with V2 configurations, you will receive an HTTP `405 Method Not Allowed` error.
#' 
#' ### Permissions
#' 
...
#' The IAM policy action name is the same for the V1 and V2 API operations.
#' 
#' The following operations are related to [`delete_bucket_metadata_configuration`][s3_delete_bucket_metadata_configuration]:
#' 
...
#'
#' @param Bucket &#91;required&#93; The general purpose bucket that you want to remove the metadata configuration from.
#' @param ExpectedBucketOwner The expected bucket owner of the general purpose bucket that you want to remove the metadata table configuration from.
#'
#' @return
#' An empty list.
#'
...
#' If you created your S3 Metadata configuration before July 15, 2025, we recommend that you delete and re-create your configuration by using [`create_bucket_metadata_configuration`][s3_create_bucket_metadata_configuration] so that you can expire journal table records and create a live inventory table.
#' 
#' Deletes a V1 S3 Metadata configuration from a general purpose bucket. For more information, see [Accelerating data discovery with S3 Metadata](https://docs.aws.amazon.com/AmazonS3/latest/userguide/metadata-tables-overview.html) in the *Amazon S3 User Guide*.
#' 
#' You can use the V2 [`delete_bucket_metadata_configuration`][s3_delete_bucket_metadata_configuration] API operation with V1 or V2 metadata table configurations. However, if you try to use the V1 [`delete_bucket_metadata_table_configuration`][s3_delete_bucket_metadata_table_configuration] API operation with V2 configurations, you will receive an HTTP `405 Method Not Allowed` error.
#' 
#' Make sure that you update your processes to use the new V2 API operations ([`create_bucket_metadata_configuration`][s3_create_bucket_metadata_configuration], [`get_bucket_metadata_configuration`][s3_get_bucket_metadata_configuration], and [`delete_bucket_metadata_configuration`][s3_delete_bucket_metadata_configuration]) instead of the V1 API operations.
#' 
#' ### Permissions
#' 
#' To use this operation, you must have the `s3:DeleteBucketMetadataTableConfiguration` permission. For more information, see [Setting up permissions for configuring metadata tables](https://docs.aws.amazon.com/AmazonS3/latest/userguide/metadata-tables-permissions.html) in the *Amazon S3 User Guide*.
#' 
#' The following operations are related to [`delete_bucket_metadata_table_configuration`][s3_delete_bucket_metadata_table_configuration]:
#' 
...
#'   ExpectedBucketOwner)
#'
#' @param Bucket &#91;required&#93; The general purpose bucket that you want to remove the metadata table configuration from.
#' @param ExpectedBucketOwner The expected bucket owner of the general purpose bucket that you want to remove the metadata table configuration from.
#'
#' @return
#' An empty list.
...
#' **Directory buckets** - The HTTP Host header syntax is `s3express-control.region-code.amazonaws.com`.
#' 
#' For information about CloudWatch request metrics for Amazon S3, see [Monitoring Metrics with Amazon CloudWatch](https://docs.aws.amazon.com/AmazonS3/latest/userguide/cloudwatch-monitoring.html).
#' 
#' The following operations are related to [`delete_bucket_metrics_configuration`][s3_delete_bucket_metrics_configuration]:
#' 
...
#' 
#' **Directory buckets** - When you use this operation with a directory bucket, you must use path-style requests in the format `https://s3express-control.region-code.amazonaws.com/bucket-name `. Virtual-hosted-style requests aren't supported. Directory bucket names must be unique in the chosen Zone (Availability Zone or Local Zone). Bucket names must also follow the format ` bucket-base-name--zone-id--x-s3` (for example, ` DOC-EXAMPLE-BUCKET--usw2-az1--x-s3`). For information about bucket naming restrictions, see [Directory bucket naming rules](https://docs.aws.amazon.com/AmazonS3/latest/userguide/directory-bucket-naming-rules.html) in the *Amazon S3 User Guide*
#' @param Id &#91;required&#93; The ID used to identify the metrics configuration. The ID has a 64 character limit and can only contain letters, numbers, periods, dashes, and underscores.
#' @param ExpectedBucketOwner The account ID of the expected bucket owner. If the account ID that you provide does not match the actual owner of the bucket, the request fails with the HTTP status code `403 Forbidden` (access denied).
#' 
#' For directory buckets, this header is not supported in this API operation. If you specify this header, the request fails with the HTTP status code `501 Not Implemented`.
#'
...
#' 
#' The following operations are related to [`delete_bucket_ownership_controls`][s3_delete_bucket_ownership_controls]:
#' 
...
#' ### Permissions
#' 
#' If you are using an identity other than the root user of the Amazon Web Services account that owns the bucket, the calling identity must both have the [`delete_bucket_policy`][s3_delete_bucket_policy] permissions on the specified bucket and belong to the bucket owner's account in order to use this operation.
#' 
#' If you don't have [`delete_bucket_policy`][s3_delete_bucket_policy] permissions, Amazon S3 returns a `403 Access Denied` error. If you have the correct permissions, but you're not using an identity that belongs to the bucket owner's account, Amazon S3 returns a `405 Method Not Allowed` error.
#' 
#' To ensure that bucket owners don't inadvertently lock themselves out of their own buckets, the root principal in a bucket owner's Amazon Web Services account can perform the [`get_bucket_policy`][s3_get_bucket_policy], [`put_bucket_policy`][s3_put_bucket_policy], and [`delete_bucket_policy`][s3_delete_bucket_policy] API actions, even if their bucket policy explicitly denies the root principal's access. Bucket owner root principals can only be blocked from performing these API actions by VPC endpoint policies and Amazon Web Services Organizations policies.
#' 
...
#' **Directory buckets** - The HTTP Host header syntax is `s3express-control.region-code.amazonaws.com`.
#' 
#' The following operations are related to [`delete_bucket_policy`][s3_delete_bucket_policy]
#' 
#' -   [`create_bucket`][s3_create_bucket]
#' 
#' -   [`delete_object`][s3_delete_object]
...
#' 
#' For information about replication configuration, see [Replication](https://docs.aws.amazon.com/AmazonS3/latest/userguide/replication.html) in the *Amazon S3 User Guide*.
#' 
#' The following operations are related to [`delete_bucket_replication`][s3_delete_bucket_replication]:
#' 
...
#' 
#' The following operations are related to [`delete_bucket_tagging`][s3_delete_bucket_tagging]:
#' 
...
#' ### Permissions
#' 
#' -   **General purpose bucket permissions** - The following permissions are required in your policies when your [`delete_objects`][s3_delete_objects] request includes specific headers.
#' 
...
#' 
#' This functionality is not supported for directory buckets.
#' @param BypassGovernanceRetention Indicates whether S3 Object Lock should bypass Governance-mode restrictions to process this operation. To use this header, you must have the `s3:BypassGovernanceRetention` permission.
#' 
#' This functionality is not supported for directory buckets.
#' @param ExpectedBucketOwner The account ID of the expected bucket owner. If the account ID that you provide does not match the actual owner of the bucket, the request fails with the HTTP status code `403 Forbidden` (access denied).
#' @param IfMatch Deletes the object if the ETag (entity tag) value provided during the delete operation matches the ETag of the object in S3. If the ETag values do not match, the operation returns a `412 Precondition Failed` error.
#' 
...
#' 
#' This functionality is only supported for directory buckets.
#' @param IfMatchSize If present, the object is deleted only if its size matches the provided size in bytes. If the `Size` value does not match, the operation returns a `412 Precondition Failed` error. If the `Size` matches or if the object doesn’t exist, the operation returns a `204 Success (No Content)` response.
#' 
#' This functionality is only supported for directory buckets.
#' 
...
#' 
#' The following operations are related to [`delete_object_tagging`][s3_delete_object_tagging]:
#' 
...
#' 
#' **S3 on Outposts** - When you use this action with S3 on Outposts, you must direct requests to the S3 on Outposts hostname. The S3 on Outposts hostname takes the form ` AccessPointName-AccountId.outpostID.s3-outposts.Region.amazonaws.com`. When you use this action with S3 on Outposts, the destination bucket must be the Outposts access point ARN or the access point alias. For more information about S3 on Outposts, see [What is S3 on Outposts?](https://docs.aws.amazon.com/AmazonS3/latest/s3-outposts/S3onOutposts.html) in the *Amazon S3 User Guide*.
#' @param Key &#91;required&#93; The key that identifies the object in the bucket from which to remove all tags.
...
#' 
#' When performing this action on an MFA Delete enabled bucket, that attempts to delete any versioned objects, you must include an MFA token. If you do not provide one, the entire request will fail, even if there are non-versioned objects you are trying to delete. If you provide an invalid token, whether there are versioned keys in the request or not, the entire Multi-Object Delete request will fail. For information about MFA Delete, see [MFA Delete](https://docs.aws.amazon.com/AmazonS3/latest/userguide/Versioning.html#MultiFactorAuthenticationDelete) in the *Amazon S3 User Guide*.
#' 
#' **Directory buckets** - MFA delete is not supported by directory buckets.
#' 
#' ### Permissions
...
#' **Directory buckets** - The HTTP Host header syntax is ` Bucket-name.s3express-zone-id.region-code.amazonaws.com`.
#' 
#' The following operations are related to [`delete_objects`][s3_delete_objects]:
#' 
#' -   [`create_multipart_upload`][s3_create_multipart_upload]
#' 
#' -   [`upload_part`][s3_upload_part]
...
#' @param MFA The concatenation of the authentication device's serial number, a space, and the value that is displayed on your authentication device. Required to permanently delete a versioned object if versioning is configured with MFA delete enabled.
#' 
#' When performing the [`delete_objects`][s3_delete_objects] operation on an MFA delete enabled bucket, which attempts to delete the specified versioned objects, you must include an MFA token. If you don't provide an MFA token, the entire request will fail, even if there are non-versioned objects that you are trying to delete. If you provide an invalid token, whether there are versioned object keys in the request or not, the entire Multi-Object Delete request will fail. For information about MFA Delete, see [MFA Delete](https://docs.aws.amazon.com/AmazonS3/latest/userguide/Versioning.html#MultiFactorAuthenticationDelete) in the *Amazon S3 User Guide*.
#' 
#' This functionality is not supported for directory buckets.
#' @param RequestPayer Confirms that the requester knows that they will be charged for the request. Bucket owners need not specify this parameter in their requests. If either the source or destination S3 bucket has Requester Pays enabled, the requester will pay for the corresponding charges. For information about downloading objects from Requester Pays buckets, see [Downloading Objects in Requester Pays Buckets](https://docs.aws.amazon.com/AmazonS3/latest/userguide/ObjectsinRequesterPaysBuckets.html) in the *Amazon S3 User Guide*.
#' 
#' This functionality is not supported for directory buckets.
#' @param BypassGovernanceRetention Specifies whether you want to delete this object even if it has a Governance-type Object Lock in place. To use this header, you must have the `s3:BypassGovernanceRetention` permission.
#' 
#' This functionality is not supported for directory buckets.
#' @param ExpectedBucketOwner The account ID of the expected bucket owner. If the account ID that you provide does not match the actual owner of the bucket, the request fails with the HTTP status code `403 Forbidden` (access denied).
...
#'
#' @param Bucket &#91;required&#93; The Amazon S3 bucket whose `PublicAccessBlock` configuration you want to delete.
#' @param ExpectedBucketOwner The account ID of the expected bucket owner. If the account ID that you provide does not match the actual owner of the bucket, the request fails with the HTTP status code `403 Forbidden` (access denied).
#'
#' @return
#' An empty list.
...
#' @param ExpectedBucketOwner The Amazon Web Services account ID of the general purpose bucket's owner.
#'
#' @return
#' A list with the following syntax:
#' ```
...
#' 
#' You set the Transfer Acceleration state of an existing bucket to `Enabled` or `Suspended` by using the [`put_bucket_accelerate_configuration`][s3_put_bucket_accelerate_configuration] operation.
#' 
#' A GET `accelerate` request does not return a state value for a bucket that has no transfer acceleration state. A bucket has no Transfer Acceleration state if a state has never been set on the bucket.
#' 
#' For more information about transfer acceleration, see [Transfer Acceleration](https://docs.aws.amazon.com/AmazonS3/latest/userguide/transfer-acceleration.html) in the Amazon S3 User Guide.
#' 
#' The following operations are related to [`get_bucket_accelerate_configuration`][s3_get_bucket_accelerate_configuration]:
#' 
...
#'   RequestPayer)
#'
#' @param Bucket &#91;required&#93; The name of the bucket for which the accelerate configuration is retrieved.
#' @param ExpectedBucketOwner The account ID of the expected bucket owner. If the account ID that you provide does not match the actual owner of the bucket, the request fails with the HTTP status code `403 Forbidden` (access denied).
#' @param RequestPayer Confirms that the requester knows that they will be charged for the request. Bucket owners need not specify this parameter in their requests. If either the source or destination S3 bucket has Requester Pays enabled, the requester will pay for the corresponding charges. For information about downloading objects from Requester Pays buckets, see [Downloading Objects in Requester Pays Buckets](https://docs.aws.amazon.com/AmazonS3/latest/userguide/ObjectsinRequesterPaysBuckets.html) in the *Amazon S3 User Guide*.
#' 
#' This functionality is not supported for directory buckets.
...
#' You must URL encode any signed header values that contain spaces. For example, if your header value is `my file.txt`, containing two spaces after `my`, you must URL encode this value to `my%20%20file.txt`.
#' 
#' The following operations are related to [`get_bucket_acl`][s3_get_bucket_acl]:
#' 
...
#' 
#' For information about Amazon S3 analytics feature, see [Amazon S3 Analytics – Storage Class Analysis](https://docs.aws.amazon.com/AmazonS3/latest/userguide/analytics-storage-class.html) in the *Amazon S3 User Guide*.
#' 
#' The following operations are related to [`get_bucket_analytics_configuration`][s3_get_bucket_analytics_configuration]:
#' 
...
#' 
#' The following operations are related to [`get_bucket_cors`][s3_get_bucket_cors]:
#' 
#' -   [`put_bucket_cors`][s3_put_bucket_cors]
#' 
...
#' **Directory buckets** - The HTTP Host header syntax is `s3express-control.region-code.amazonaws.com`.
#' 
#' The following operations are related to [`get_bucket_encryption`][s3_get_bucket_encryption]:
#' 
#' -   [`put_bucket_encryption`][s3_put_bucket_encryption]
#' 
...
#' For more information, see [Storage class for automatically optimizing frequently and infrequently accessed objects](https://docs.aws.amazon.com/AmazonS3/latest/userguide/storage-class-intro.html#sc-dynamic-data-access).
#' 
#' Operations related to [`get_bucket_intelligent_tiering_configuration`][s3_get_bucket_intelligent_tiering_configuration] include:
#' 
...
#' For information about the Amazon S3 inventory feature, see [Amazon S3 Inventory](https://docs.aws.amazon.com/AmazonS3/latest/userguide/storage-inventory.html).
#' 
#' The following operations are related to [`get_bucket_inventory_configuration`][s3_get_bucket_inventory_configuration]:
#' 
...
#' 
#' [`get_bucket_lifecycle`][s3_get_bucket_lifecycle] has the following special error:
#' 
...
#' 
#' The following operations are related to [`get_bucket_lifecycle`][s3_get_bucket_lifecycle]:
#' 
#' -   [`get_bucket_lifecycle_configuration`][s3_get_bucket_lifecycle_configuration]
#' 
...
#' Returns the lifecycle configuration information set on the bucket. For information about lifecycle configuration, see [Object Lifecycle Management](https://docs.aws.amazon.com/AmazonS3/latest/userguide/object-lifecycle-mgmt.html?refid=awsmp_sol_lp_hlth_lp).
#' 
#' Bucket lifecycle configuration now supports specifying a lifecycle rule using an object key name prefix, one or more object tags, object size, or any combination of these. Accordingly, this section describes the latest API, which is compatible with the new functionality. The previous version of the API supported filtering based only on an object key name prefix, which is supported for general purpose buckets for backward compatibility. For the related API description, see [`get_bucket_lifecycle`][s3_get_bucket_lifecycle].
#' 
...
#' **Directory buckets** - The HTTP Host header syntax is `s3express-control.region.amazonaws.com`.
#' 
#' [`get_bucket_lifecycle_configuration`][s3_get_bucket_lifecycle_configuration] has the following special error:
#' 
#' -   Error code: `NoSuchLifecycleConfiguration`
#' 
#'     -   Description: The lifecycle configuration does not exist.
...
#'     -   SOAP Fault Code Prefix: Client
#' 
#' The following operations are related to [`get_bucket_lifecycle_configuration`][s3_get_bucket_lifecycle_configuration]:
#' 
...
#' 
#' In a bucket's home Region, calls to the [`get_bucket_location`][s3_get_bucket_location] operation are governed by the bucket's policy. In other Regions, the bucket policy doesn't apply, which means that cross-account access won't be authorized. However, calls to the [`head_bucket`][s3_head_bucket] operation always return the bucket’s location through an HTTP response header, whether access to the bucket is authorized or not. Therefore, we recommend using the [`head_bucket`][s3_head_bucket] operation for bucket Region discovery and to avoid using the [`get_bucket_location`][s3_get_bucket_location] operation.
#' 
#' When you use this API operation with an access point, provide the alias of the access point in place of the bucket name.
#' 
#' When you use this API operation with an Object Lambda access point, provide the alias of the Object Lambda access point in place of the bucket name. If the Object Lambda access point alias in a request is not valid, the error code `InvalidAccessPointAliasError` is returned. For more information about `InvalidAccessPointAliasError`, see [List of Error Codes](https://docs.aws.amazon.com/AmazonS3/latest/API/ErrorResponses.html#ErrorCodeList).
...
#' This operation is not supported for directory buckets.
#' 
#' The following operations are related to [`get_bucket_location`][s3_get_bucket_location]:
#' 
#' -   [`get_object`][s3_get_object]
#' 
#' -   [`create_bucket`][s3_create_bucket]
...
#' 
#' The following operations are related to [`get_bucket_logging`][s3_get_bucket_logging]:
#' 
#' -   [`create_bucket`][s3_create_bucket]
#' 
...
#' 
#' You can use the V2 [`get_bucket_metadata_configuration`][s3_get_bucket_metadata_configuration] API operation with V1 or V2 metadata configurations. However, if you try to use the V1 [`get_bucket_metadata_table_configuration`][s3_get_bucket_metadata_table_configuration] API operation with V2 configurations, you will receive an HTTP `405 Method Not Allowed` error.
#' 
#' ### Permissions
#' 
...
#' The IAM policy action name is the same for the V1 and V2 API operations.
#' 
#' The following operations are related to [`get_bucket_metadata_configuration`][s3_get_bucket_metadata_configuration]:
#' 
#' -   [`create_bucket_metadata_configuration`][s3_create_bucket_metadata_configuration]
#' 
#' -   [`delete_bucket_metadata_configuration`][s3_delete_bucket_metadata_configuration]
...
#'
#' @param Bucket &#91;required&#93; The general purpose bucket that corresponds to the metadata configuration that you want to retrieve.
#' @param ExpectedBucketOwner The expected owner of the general purpose bucket that you want to retrieve the metadata table configuration for.
#'
#' @return
#' A list with the following syntax:
#' ```
...
#' 
#' You can use the V2 [`get_bucket_metadata_configuration`][s3_get_bucket_metadata_configuration] API operation with V1 or V2 metadata table configurations. However, if you try to use the V1 [`get_bucket_metadata_table_configuration`][s3_get_bucket_metadata_table_configuration] API operation with V2 configurations, you will receive an HTTP `405 Method Not Allowed` error.
#' 
#' Make sure that you update your processes to use the new V2 API operations ([`create_bucket_metadata_configuration`][s3_create_bucket_metadata_configuration], [`get_bucket_metadata_configuration`][s3_get_bucket_metadata_configuration], and [`delete_bucket_metadata_configuration`][s3_delete_bucket_metadata_configuration]) instead of the V1 API operations.
#' 
#' ### Permissions
...
#' To use this operation, you must have the `s3:GetBucketMetadataTableConfiguration` permission. For more information, see [Setting up permissions for configuring metadata tables](https://docs.aws.amazon.com/AmazonS3/latest/userguide/metadata-tables-permissions.html) in the *Amazon S3 User Guide*.
#' 
#' The following operations are related to [`get_bucket_metadata_table_configuration`][s3_get_bucket_metadata_table_configuration]:
#' 
#' -   [`create_bucket_metadata_table_configuration`][s3_create_bucket_metadata_table_configuration]
#' 
#' -   [`delete_bucket_metadata_table_configuration`][s3_delete_bucket_metadata_table_configuration]
...
#'
#' @param Bucket &#91;required&#93; The general purpose bucket that corresponds to the metadata table configuration that you want to retrieve.
#' @param ExpectedBucketOwner The expected owner of the general purpose bucket that you want to retrieve the metadata table configuration for.
#'
#' @return
#' A list with the following syntax:
...
#' For information about CloudWatch request metrics for Amazon S3, see [Monitoring Metrics with Amazon CloudWatch](https://docs.aws.amazon.com/AmazonS3/latest/userguide/cloudwatch-monitoring.html).
#' 
#' The following operations are related to [`get_bucket_metrics_configuration`][s3_get_bucket_metrics_configuration]:
#' 
#' -   [`put_bucket_metrics_configuration`][s3_put_bucket_metrics_configuration]
#' 
...
#' 
#' -   You've manually deleted the bucket ownership control value using the [`delete_bucket_ownership_controls`][s3_delete_bucket_ownership_controls] API operation.
#' 
#' By default, Amazon S3 sets `OwnershipControls` for all newly created buckets.
#' 
#' For information about Amazon S3 Object Ownership, see [Using Object Ownership](https://docs.aws.amazon.com/AmazonS3/latest/userguide/about-object-ownership.html).
#' 
#' The following operations are related to [`get_bucket_ownership_controls`][s3_get_bucket_ownership_controls]:
#' 
#' -   [`put_bucket_ownership_controls`][s3_put_bucket_ownership_controls]
#' 
...
#'
#' @param Bucket &#91;required&#93; The name of the Amazon S3 bucket whose `OwnershipControls` you want to retrieve.
#' @param ExpectedBucketOwner The account ID of the expected bucket owner. If the account ID that you provide does not match the actual owner of the bucket, the request fails with the HTTP status code `403 Forbidden` (access denied).
#'
#' @return
#' A list with the following syntax:
...
#' ### Permissions
#' 
#' If you are using an identity other than the root user of the Amazon Web Services account that owns the bucket, the calling identity must both have the [`get_bucket_policy`][s3_get_bucket_policy] permissions on the specified bucket and belong to the bucket owner's account in order to use this operation.
#' 
#' If you don't have [`get_bucket_policy`][s3_get_bucket_policy] permissions, Amazon S3 returns a `403 Access Denied` error. If you have the correct permissions, but you're not using an identity that belongs to the bucket owner's account, Amazon S3 returns a `405 Method Not Allowed` error.
#' 
#' To ensure that bucket owners don't inadvertently lock themselves out of their own buckets, the root principal in a bucket owner's Amazon Web Services account can perform the [`get_bucket_policy`][s3_get_bucket_policy], [`put_bucket_policy`][s3_put_bucket_policy], and [`delete_bucket_policy`][s3_delete_bucket_policy] API actions, even if their bucket policy explicitly denies the root principal's access. Bucket owner root principals can only be blocked from performing these API actions by VPC endpoint policies and Amazon Web Services Organizations policies.
#' 
...
#' **Directory buckets** - The HTTP Host header syntax is `s3express-control.region-code.amazonaws.com`.
#' 
#' The following action is related to [`get_bucket_policy`][s3_get_bucket_policy]:
#' 
#' -   [`get_object`][s3_get_object]
#' 
#' You must URL encode any signed header values that contain spaces. For example, if your header value is `my file.txt`, containing two spaces after `my`, you must URL encode this value to `my%20%20file.txt`.
...
#'
#' @param Bucket &#91;required&#93; The name of the Amazon S3 bucket whose policy status you want to retrieve.
#' @param ExpectedBucketOwner The account ID of the expected bucket owner. If the account ID that you provide does not match the actual owner of the bucket, the request fails with the HTTP status code `403 Forbidden` (access denied).
#'
#' @return
#' A list with the following syntax:
...
#' For information about replication configuration, see [Replication](https://docs.aws.amazon.com/AmazonS3/latest/userguide/replication.html) in the *Amazon S3 User Guide*.
#' 
#' This action requires permissions for the `s3:GetReplicationConfiguration` action. For more information about permissions, see [Using Bucket Policies and User Policies](https://docs.aws.amazon.com/AmazonS3/latest/userguide/security_iam_service-with-iam.html).
#' 
...
#'
#' @param Bucket &#91;required&#93; The name of the bucket for which to get the payment request configuration
#' @param ExpectedBucketOwner The account ID of the expected bucket owner. If the account ID that you provide does not match the actual owner of the bucket, the request fails with the HTTP status code `403 Forbidden` (access denied).
#'
#' @return
#' A list with the following syntax:
...
#' 
#' [`get_bucket_tagging`][s3_get_bucket_tagging] has the following special error:
#' 
#' -   Error code: `NoSuchTagSet`
#' 
#'     -   Description: There is no tag set associated with the bucket.
#' 
#' The following operations are related to [`get_bucket_tagging`][s3_get_bucket_tagging]:
#' 
#' -   [`put_bucket_tagging`][s3_put_bucket_tagging]
#' 
...
#' 
#' The following operations are related to [`get_bucket_versioning`][s3_get_bucket_versioning]:
#' 
#' -   [`get_object`][s3_get_object]
#' 
#' -   [`put_object`][s3_put_object]
...
#' 
#' The following operations are related to [`get_bucket_website`][s3_get_bucket_website]:
#' 
...
#' Retrieves an object from Amazon S3.
#' 
#' In the [`get_object`][s3_get_object] request, specify the full key name for the object.
#' 
#' **General purpose buckets** - Both the virtual-hosted-style requests and the path-style requests are supported. For a virtual hosted-style request example, if you have the object `photos/2006/February/sample.jpg`, specify the object key name as `/photos/2006/February/sample.jpg`. For a path-style request example, if you have the object `photos/2006/February/sample.jpg` in the bucket named `examplebucket`, specify the object key name as `/examplebucket/photos/2006/February/sample.jpg`. For more information about request types, see [HTTP Host Header Bucket Specification](https://docs.aws.amazon.com/AmazonS3/latest/userguide/VirtualHosting.html#VirtualHostingSpecifyBucket) in the *Amazon S3 User Guide*.
#' 
...
#' ### Permissions
#' 
#' -   **General purpose bucket permissions** - You must have the required permissions in a policy. To use [`get_object`][s3_get_object], you must have the `READ` access to the object (or version). If you grant `READ` access to the anonymous user, the [`get_object`][s3_get_object] operation returns the object without using an authorization header. For more information, see [Specifying permissions in a policy](https://docs.aws.amazon.com/AmazonS3/latest/userguide/security_iam_service-with-iam.html) in the *Amazon S3 User Guide*.
#' 
...
#' ### Storage classes
#' 
#' If the object you are retrieving is stored in the S3 Glacier Flexible Retrieval storage class, the S3 Glacier Deep Archive storage class, the S3 Intelligent-Tiering Archive Access tier, or the S3 Intelligent-Tiering Deep Archive Access tier, before you can retrieve the object you must first restore a copy using [`restore_object`][s3_restore_object]. Otherwise, this operation returns an `InvalidObjectState` error. For information about restoring archived objects, see [Restoring Archived Objects](https://docs.aws.amazon.com/AmazonS3/latest/userguide/restoring-objects.html) in the *Amazon S3 User Guide*.
#' 
#' **Directory buckets** - Directory buckets only support `EXPRESS_ONEZONE` (the S3 Express One Zone storage class) in Availability Zones and `ONEZONE_IA` (the S3 One Zone-Infrequent Access storage class) in Dedicated Local Zones. Unsupported storage class values won't write a destination object and will respond with the HTTP status code `400 Bad Request`.
#' 
#' ### Encryption
#' 
#' Encryption request headers, like `x-amz-server-side-encryption`, should not be sent for the [`get_object`][s3_get_object] requests, if your object uses server-side encryption with Amazon S3 managed encryption keys (SSE-S3), server-side encryption with Key Management Service (KMS) keys (SSE-KMS), or dual-layer server-side encryption with Amazon Web Services KMS keys (DSSE-KMS). If you include the header in your [`get_object`][s3_get_object] requests for the object that uses these types of keys, you’ll get an HTTP `400 Bad Request` error.
#' 
#' **Directory buckets** - For directory buckets, there are only two supported options for server-side encryption: SSE-S3 and SSE-KMS. SSE-C isn't supported. For more information, see [Protecting data with server-side encryption](https://docs.aws.amazon.com/AmazonS3/latest/userguide/) in the *Amazon S3 User Guide*.
#' 
#' ### Overriding response header values through the request
#' 
#' There are times when you want to override certain response header values of a [`get_object`][s3_get_object] response. For example, you might override the `Content-Disposition` response header value through your [`get_object`][s3_get_object] request.
#' 
...
#' 
#' The response headers that you can override for the [`get_object`][s3_get_object] response are `Cache-Control`, `Content-Disposition`, `Content-Encoding`, `Content-Language`, `Content-Type`, and `Expires`.
#' 
#' To override values for a set of response headers in the [`get_object`][s3_get_object] response, you can use the following query parameters in the request.
#' 
...
#' 
#' **S3 on Outposts** - When you use this action with S3 on Outposts, you must direct requests to the S3 on Outposts hostname. The S3 on Outposts hostname takes the form ` AccessPointName-AccountId.outpostID.s3-outposts.Region.amazonaws.com`. When you use this action with S3 on Outposts, the destination bucket must be the Outposts access point ARN or the access point alias. For more information about S3 on Outposts, see [What is S3 on Outposts?](https://docs.aws.amazon.com/AmazonS3/latest/s3-outposts/S3onOutposts.html) in the *Amazon S3 User Guide*.
#' @param IfMatch Return the object only if its entity tag (ETag) is the same as the one specified in this header; otherwise, return a `412 Precondition Failed` error.
#' 
...
#' @param VersionId Version ID used to reference a specific version of the object.
#' 
#' By default, the [`get_object`][s3_get_object] operation returns the current version of an object. To return a different version, use the `versionId` subresource.
#' 
#' -   If you include a `versionId` in your request header, you must have the `s3:GetObjectVersion` permission to access a specific version of an object. The `s3:GetObject` permission is not required in this scenario.
#' 
#' -   If you request the current version of an object without a specific `versionId` in the request header, only the `s3:GetObject` permission is required. The `s3:GetObjectVersion` permission is not required in this scenario.
...
#' -   **Directory buckets** - S3 Versioning isn't enabled and supported for directory buckets. For this API operation, only the `null` value of the version ID is supported by directory buckets. You can only specify `null` to the `versionId` query parameter in the request.
#' 
#' For more information about versioning, see [`put_bucket_versioning`][s3_put_bucket_versioning].
#' @param SSECustomerAlgorithm Specifies the algorithm to use when decrypting the object (for example, `AES256`).
#' 
...
#' -   `x-amz-server-side-encryption-customer-key-MD5`
#' 
#' For more information about SSE-C, see [Server-Side Encryption (Using Customer-Provided Encryption Keys)](https://docs.aws.amazon.com/AmazonS3/latest/userguide/ServerSideEncryptionCustomerKeys.html) in the *Amazon S3 User Guide*.
#' 
#' This functionality is not supported for directory buckets.
#' @param SSECustomerKey Specifies the customer-provided encryption key that you originally provided for Amazon S3 to encrypt the data before storing it. This value is used to decrypt the object when recovering it and must match the one used when storing the data. The key must be appropriate for use with the algorithm specified in the `x-amz-server-side-encryption-customer-algorithm` header.
#' 
#' If you encrypt an object by using server-side encryption with customer-provided encryption keys (SSE-C) when you store the object in Amazon S3, then when you GET the object, you must use the following headers:
#' 
#' -   `x-amz-server-side-encryption-customer-algorithm`
...
#' If your bucket uses the bucket owner enforced setting for S3 Object Ownership, requests to read ACLs are still supported and return the `bucket-owner-full-control` ACL with the owner being the account that created the bucket. For more information, see [Controlling object ownership and disabling ACLs](https://docs.aws.amazon.com/AmazonS3/latest/userguide/about-object-ownership.html) in the *Amazon S3 User Guide*.
#' 
#' The following operations are related to [`get_object_acl`][s3_get_object_acl]:
#' 
#' -   [`get_object`][s3_get_object]
#' 
...
#'   ExpectedBucketOwner)
#'
#' @param Bucket &#91;required&#93; The bucket name that contains the object for which to get the ACL information.
#' 
#' **Access points** - When you use this action with an access point for general purpose buckets, you must provide the alias of the access point in place of the bucket name or specify the access point ARN. When you use this action with an access point for directory buckets, you must provide the access point name in place of the bucket name. When using the access point ARN, you must direct requests to the access point hostname. The access point hostname takes the form *AccessPointName*-*AccountId*.s3-accesspoint.*Region*.amazonaws.com. When using this action with an access point through the Amazon Web Services SDKs, you provide the access point ARN in place of the bucket name. For more information about access point ARNs, see [Using access points](https://docs.aws.amazon.com/AmazonS3/latest/userguide/using-access-points.html) in the *Amazon S3 User Guide*.
...
#' 
#' [`get_object_attributes`][s3_get_object_attributes] combines the functionality of [`head_object`][s3_head_object] and [`list_parts`][s3_list_parts]. All of the data returned with both of those individual calls can be returned with a single call to [`get_object_attributes`][s3_get_object_attributes].
#' 
#' **Directory buckets** - For directory buckets, you must make requests for this API operation to the Zonal endpoint. These endpoints support virtual-hosted-style requests in the format `https://amzn-s3-demo-bucket.s3express-zone-id.region-code.amazonaws.com/key-name `. Path-style requests are not supported. For more information about endpoints in Availability Zones, see [Regional and Zonal endpoints for directory buckets in Availability Zones](https://docs.aws.amazon.com/AmazonS3/latest/userguide/endpoint-directory-buckets-AZ.html) in the *Amazon S3 User Guide*. For more information about endpoints in Local Zones, see [Concepts for directory buckets in Local Zones](https://docs.aws.amazon.com/AmazonS3/latest/userguide/s3-lzs-for-directory-buckets.html) in the *Amazon S3 User Guide*.
#' 
#' ### Permissions
#' 
#' -   **General purpose bucket permissions** - To use [`get_object_attributes`][s3_get_object_attributes], you must have READ access to the object.
#' 
#'     The other permissions that you need to use this operation depend on whether the bucket is versioned and if a version ID is passed in the [`get_object_attributes`][s3_get_object_attributes] request.
#' 
...
#' 
#'     For more information, see [Specifying Permissions in a Policy](https://docs.aws.amazon.com/AmazonS3/latest/userguide/security_iam_service-with-iam.html) in the *Amazon S3 User Guide*.
#' 
...
#' **Directory buckets** - The HTTP Host header syntax is ` Bucket-name.s3express-zone-id.region-code.amazonaws.com`.
#' 
#' The following actions are related to [`get_object_attributes`][s3_get_object_attributes]:
#' 
#' -   [`get_object`][s3_get_object]
#' 
#' -   [`get_object_acl`][s3_get_object_acl]
...
#' @param MaxParts Sets the maximum number of parts to return. For more information, see [Uploading and copying objects using multipart upload in Amazon S3](https://docs.aws.amazon.com/AmazonS3/latest/userguide/mpuoverview.html) in the *Amazon Simple Storage Service user guide*.
#' @param PartNumberMarker Specifies the part after which listing should begin. Only parts with higher part numbers will be listed. For more information, see [Uploading and copying objects using multipart upload in Amazon S3](https://docs.aws.amazon.com/AmazonS3/latest/userguide/mpuoverview.html) in the *Amazon Simple Storage Service user guide*.
#' @param SSECustomerAlgorithm Specifies the algorithm to use when encrypting the object (for example, AES256).
#' 
#' This functionality is not supported for directory buckets.
#' @param SSECustomerKey Specifies the customer-provided encryption key for Amazon S3 to use in encrypting data. This value is used to store the object and then it is discarded; Amazon S3 does not store the encryption key. The key must be appropriate for use with the algorithm specified in the `x-amz-server-side-encryption-customer-algorithm` header.
...
#' This functionality is not supported for directory buckets.
#' @param ExpectedBucketOwner The account ID of the expected bucket owner. If the account ID that you provide does not match the actual owner of the bucket, the request fails with the HTTP status code `403 Forbidden` (access denied).
#' @param ObjectAttributes &#91;required&#93; Specifies the fields at the root level that you want returned in the response. Fields that you do not specify are not returned.
#'
#' @return
#' A list with the following syntax:
#' ```
...
#' This functionality is not supported for Amazon S3 on Outposts.
#' 
#' The following action is related to [`get_object_legal_hold`][s3_get_object_legal_hold]:
#' 
#' -   [`get_object_attributes`][s3_get_object_attributes]
#' 
#' You must URL encode any signed header values that contain spaces. For example, if your header value is `my file.txt`, containing two spaces after `my`, you must URL encode this value to `my%20%20file.txt`.
...
#'   ExpectedBucketOwner)
#'
#' @param Bucket &#91;required&#93; The bucket name containing the object whose legal hold status you want to retrieve.
#' 
#' **Access points** - When you use this action with an access point for general purpose buckets, you must provide the alias of the access point in place of the bucket name or specify the access point ARN. When you use this action with an access point for directory buckets, you must provide the access point name in place of the bucket name. When using the access point ARN, you must direct requests to the access point hostname. The access point hostname takes the form *AccessPointName*-*AccountId*.s3-accesspoint.*Region*.amazonaws.com. When using this action with an access point through the Amazon Web Services SDKs, you provide the access point ARN in place of the bucket name. For more information about access point ARNs, see [Using access points](https://docs.aws.amazon.com/AmazonS3/latest/userguide/using-access-points.html) in the *Amazon S3 User Guide*.
#' @param Key &#91;required&#93; The key name for the object whose legal hold status you want to retrieve.
...
#' This functionality is not supported for Amazon S3 on Outposts.
#' 
#' The following action is related to [`get_object_retention`][s3_get_object_retention]:
#' 
#' -   [`get_object_attributes`][s3_get_object_attributes]
#' 
#' You must URL encode any signed header values that contain spaces. For example, if your header value is `my file.txt`, containing two spaces after `my`, you must URL encode this value to `my%20%20file.txt`.
...
#'   ExpectedBucketOwner)
#'
#' @param Bucket &#91;required&#93; The bucket name containing the object whose retention settings you want to retrieve.
#' 
#' **Access points** - When you use this action with an access point for general purpose buckets, you must provide the alias of the access point in place of the bucket name or specify the access point ARN. When you use this action with an access point for directory buckets, you must provide the access point name in place of the bucket name. When using the access point ARN, you must direct requests to the access point hostname. The access point hostname takes the form *AccessPointName*-*AccountId*.s3-accesspoint.*Region*.amazonaws.com. When using this action with an access point through the Amazon Web Services SDKs, you provide the access point ARN in place of the bucket name. For more information about access point ARNs, see [Using access points](https://docs.aws.amazon.com/AmazonS3/latest/userguide/using-access-points.html) in the *Amazon S3 User Guide*.
#' @param Key &#91;required&#93; The key name for the object whose retention settings you want to retrieve.
...
#' 
#' For information about the Amazon S3 object tagging feature, see [Object Tagging](https://docs.aws.amazon.com/AmazonS3/latest/userguide/object-tagging.html).
#' 
#' The following actions are related to [`get_object_tagging`][s3_get_object_tagging]:
#' 
...
#'   RequestPayer)
#'
#' @param Bucket &#91;required&#93; The bucket name containing the object for which to get the tagging information.
#' 
#' **Access points** - When you use this action with an access point for general purpose buckets, you must provide the alias of the access point in place of the bucket name or specify the access point ARN. When you use this action with an access point for directory buckets, you must provide the access point name in place of the bucket name. When using the access point ARN, you must direct requests to the access point hostname. The access point hostname takes the form *AccessPointName*-*AccountId*.s3-accesspoint.*Region*.amazonaws.com. When using this action with an access point through the Amazon Web Services SDKs, you provide the access point ARN in place of the bucket name. For more information about access point ARNs, see [Using access points](https://docs.aws.amazon.com/AmazonS3/latest/userguide/using-access-points.html) in the *Amazon S3 User Guide*.
#' 
#' **S3 on Outposts** - When you use this action with S3 on Outposts, you must direct requests to the S3 on Outposts hostname. The S3 on Outposts hostname takes the form ` AccessPointName-AccountId.outpostID.s3-outposts.Region.amazonaws.com`. When you use this action with S3 on Outposts, the destination bucket must be the Outposts access point ARN or the access point alias. For more information about S3 on Outposts, see [What is S3 on Outposts?](https://docs.aws.amazon.com/AmazonS3/latest/s3-outposts/S3onOutposts.html) in the *Amazon S3 User Guide*.
...
#' This functionality is not supported for Amazon S3 on Outposts.
#' 
#' The following action is related to [`get_object_torrent`][s3_get_object_torrent]:
#' 
#' -   [`get_object`][s3_get_object]
#' 
#' You must URL encode any signed header values that contain spaces. For example, if your header value is `my file.txt`, containing two spaces after `my`, you must URL encode this value to `my%20%20file.txt`.
...
#'
#' @param Bucket &#91;required&#93; The name of the bucket containing the object for which to get the torrent files.
...
#'
#' @param Bucket &#91;required&#93; The name of the Amazon S3 bucket whose `PublicAccessBlock` configuration you want to retrieve.
#' @param ExpectedBucketOwner The account ID of the expected bucket owner. If the account ID that you provide does not match the actual owner of the bucket, the request fails with the HTTP status code `403 Forbidden` (access denied).
#'
#' @return
#' A list with the following syntax:
...
#' For more information about SSE-C, see [Server-Side Encryption (Using Customer-Provided Encryption Keys)](https://docs.aws.amazon.com/AmazonS3/latest/userguide/ServerSideEncryptionCustomerKeys.html) in the *Amazon S3 User Guide*.
#' 
#' **Directory bucket** - For directory buckets, there are only two supported options for server-side encryption: SSE-S3 and SSE-KMS. SSE-C isn't supported. For more information, see [Protecting data with server-side encryption](https://docs.aws.amazon.com/AmazonS3/latest/userguide/) in the *Amazon S3 User Guide*.
#' 
#' ### Versioning
#' 
...
#' 
#' **S3 on Outposts** - When you use this action with S3 on Outposts, you must direct requests to the S3 on Outposts hostname. The S3 on Outposts hostname takes the form ` AccessPointName-AccountId.outpostID.s3-outposts.Region.amazonaws.com`. When you use this action with S3 on Outposts, the destination bucket must be the Outposts access point ARN or the access point alias. For more information about S3 on Outposts, see [What is S3 on Outposts?](https://docs.aws.amazon.com/AmazonS3/latest/s3-outposts/S3onOutposts.html) in the *Amazon S3 User Guide*.
#' @param IfMatch Return the object only if its entity tag (ETag) is the same as the one specified; otherwise, return a 412 (precondition failed) error.
#' 
...
#' To use this operation, you must have permissions to perform the `s3:GetAnalyticsConfiguration` action. The bucket owner has this permission by default. The bucket owner can grant this permission to others. For more information about permissions, see [Permissions Related to Bucket Subresource Operations](https://docs.aws.amazon.com/AmazonS3/latest/userguide/security_iam_service-with-iam.html#using-with-s3-actions-related-to-bucket-subresources) and [Managing Access Permissions to Your Amazon S3 Resources](https://docs.aws.amazon.com/AmazonS3/latest/userguide/security-iam.html).
#' 
#' For information about Amazon S3 analytics feature, see [Amazon S3 Analytics – Storage Class Analysis](https://docs.aws.amazon.com/AmazonS3/latest/userguide/analytics-storage-class.html).
#' 
#' The following operations are related to [`list_bucket_analytics_configurations`][s3_list_bucket_analytics_configurations]:
#' 
#' -   [`get_bucket_analytics_configuration`][s3_get_bucket_analytics_configuration]
#' 
#' -   [`delete_bucket_analytics_configuration`][s3_delete_bucket_analytics_configuration]
...
#' For more information, see [Storage class for automatically optimizing frequently and infrequently accessed objects](https://docs.aws.amazon.com/AmazonS3/latest/userguide/storage-class-intro.html#sc-dynamic-data-access).
#' 
#' Operations related to [`list_bucket_intelligent_tiering_configurations`][s3_list_bucket_intelligent_tiering_configurations] include:
#' 
#' -   [`delete_bucket_intelligent_tiering_configuration`][s3_delete_bucket_intelligent_tiering_configuration]
#' 
#' -   [`put_bucket_intelligent_tiering_configuration`][s3_put_bucket_intelligent_tiering_configuration]
...
#' For information about the Amazon S3 inventory feature, see [Amazon S3 Inventory](https://docs.aws.amazon.com/AmazonS3/latest/userguide/storage-inventory.html)
#' 
#' The following operations are related to [`list_bucket_inventory_configurations`][s3_list_bucket_inventory_configurations]:
#' 
#' -   [`get_bucket_inventory_configuration`][s3_get_bucket_inventory_configuration]
#' 
#' -   [`delete_bucket_inventory_configuration`][s3_delete_bucket_inventory_configuration]
...
#' **Directory buckets** - The HTTP Host header syntax is `s3express-control.region-code.amazonaws.com`.
#' 
#' For more information about metrics configurations and CloudWatch request metrics, see [Monitoring Metrics with Amazon CloudWatch](https://docs.aws.amazon.com/AmazonS3/latest/userguide/cloudwatch-monitoring.html).
#' 
#' The following operations are related to [`list_bucket_metrics_configurations`][s3_list_bucket_metrics_configurations]:
#' 
#' -   [`put_bucket_metrics_configuration`][s3_put_bucket_metrics_configuration]
#' 
#' -   [`get_bucket_metrics_configuration`][s3_get_bucket_metrics_configuration]
...
#' 
#' We strongly recommend using only paginated [`list_buckets`][s3_list_buckets] requests. Unpaginated [`list_buckets`][s3_list_buckets] requests are only supported for Amazon Web Services accounts set to the default general purpose bucket quota of 10,000. If you have an approved general purpose bucket quota above 10,000, you must send paginated [`list_buckets`][s3_list_buckets] requests to list your account’s buckets. All unpaginated [`list_buckets`][s3_list_buckets] requests will be rejected for Amazon Web Services accounts with a general purpose bucket quota greater than 10,000.
#' 
#' You must URL encode any signed header values that contain spaces. For example, if your header value is `my file.txt`, containing two spaces after `my`, you must URL encode this value to `my%20%20file.txt`.
#'
#' @usage
...
#'
#' @param MaxBuckets Maximum number of buckets to be returned in response. When the number is more than the count of buckets that are owned by an Amazon Web Services account, return all the buckets in response.
...
#' @param BucketRegion Limits the response to buckets that are located in the specified Amazon Web Services Region. The Amazon Web Services Region must be expressed according to the Amazon Web Services Region code, such as `us-west-2` for the US West (Oregon) Region. For a list of the valid values for all of the Amazon Web Services Regions, see [Regions and Endpoints](https://docs.aws.amazon.com/general/latest/gr/rande.html#s3_region).
#' 
...
#' **Directory buckets** - The HTTP Host header syntax is `s3express-control.region.amazonaws.com`.
#' 
#' The `BucketRegion` response element is not part of the [`list_directory_buckets`][s3_list_directory_buckets] Response Syntax.
#' 
#' You must URL encode any signed header values that contain spaces. For example, if your header value is `my file.txt`, containing two spaces after `my`, you must URL encode this value to `my%20%20file.txt`.
#'
#' @usage
...
#'
#' @param ContinuationToken `ContinuationToken` indicates to Amazon S3 that the list is being continued on buckets in this account with a token. `ContinuationToken` is obfuscated and is not a real bucket name. You can use this `ContinuationToken` for the pagination of the list results.
#' @param MaxDirectoryBuckets Maximum number of buckets to be returned in response. When the number is more than the count of buckets that are owned by an Amazon Web Services account, return all the buckets in response.
#'
#' @return
#' A list with the following syntax:
#' ```
...
#' **Directory buckets** - If multipart uploads in a directory bucket are in progress, you can't delete the bucket until all the in-progress multipart uploads are aborted or completed. To delete these in-progress multipart uploads, use the [`list_multipart_uploads`][s3_list_multipart_uploads] operation to list the in-progress multipart uploads in the bucket and use the [`abort_multipart_upload`][s3_abort_multipart_upload] operation to abort all the in-progress multipart uploads.
#' 
#' The [`list_multipart_uploads`][s3_list_multipart_uploads] operation returns a maximum of 1,000 multipart uploads in the response. The limit of 1,000 multipart uploads is also the default value. You can further limit the number of uploads in a response by specifying the `max-uploads` request parameter. If there are more than 1,000 multipart uploads that satisfy your [`list_multipart_uploads`][s3_list_multipart_uploads] request, the response returns an `IsTruncated` element with the value of `true`, a `NextKeyMarker` element, and a `NextUploadIdMarker` element. To list the remaining multipart uploads, you need to make subsequent [`list_multipart_uploads`][s3_list_multipart_uploads] requests. In these requests, include two query parameters: `key-marker` and `upload-id-marker`. Set the value of `key-marker` to the `NextKeyMarker` value from the previous response. Similarly, set the value of `upload-id-marker` to the `NextUploadIdMarker` value from the previous response.
#' 
...
#' 
#' -   **General purpose bucket** - In the [`list_multipart_uploads`][s3_list_multipart_uploads] response, the multipart uploads are sorted based on two criteria:
#' 
...
#' 
#' -   **Directory bucket** - In the [`list_multipart_uploads`][s3_list_multipart_uploads] response, the multipart uploads aren't sorted lexicographically based on the object keys.
#' 
#' ### HTTP Host header syntax
#' 
#' **Directory buckets** - The HTTP Host header syntax is ` Bucket-name.s3express-zone-id.region-code.amazonaws.com`.
#' 
#' The following operations are related to [`list_multipart_uploads`][s3_list_multipart_uploads]:
#' 
#' -   [`create_multipart_upload`][s3_create_multipart_upload]
#' 
#' -   [`upload_part`][s3_upload_part]
...
#' 
#'     In the [`list_multipart_uploads`][s3_list_multipart_uploads] response, the multipart uploads aren't sorted lexicographically based on the object keys.
...
#' @param Prefix Lists in-progress uploads only for those keys that begin with the specified prefix. You can use prefixes to separate a bucket into different grouping of keys. (You can think of using `prefix` to make groups in the same way that you'd use a folder in a file system.)
#' 
...
#' 
#' The following operations are related to [`list_object_versions`][s3_list_object_versions]:
#' 
...
#' @param Prefix Use this parameter to select only those keys that begin with the specified prefix. You can use prefixes to separate a bucket into different groupings of keys. (You can think of using `prefix` to make groups in the same way that you'd use a folder in a file system.) You can use `prefix` with `delimiter` to roll up numerous objects into a single result under `CommonPrefixes`.
#' @param VersionIdMarker Specifies the object version you want to start listing from.
#' @param ExpectedBucketOwner The account ID of the expected bucket owner. If the account ID that you provide does not match the actual owner of the bucket, the request fails with the HTTP status code `403 Forbidden` (access denied).
#' @param RequestPayer Confirms that the requester knows that they will be charged for the request. Bucket owners need not specify this parameter in their requests. If either the source or destination S3 bucket has Requester Pays enabled, the requester will pay for the corresponding charges. For information about downloading objects from Requester Pays buckets, see [Downloading Objects in Requester Pays Buckets](https://docs.aws.amazon.com/AmazonS3/latest/userguide/ObjectsinRequesterPaysBuckets.html) in the *Amazon S3 User Guide*.
#' 
#' This functionality is not supported for directory buckets.
#' @param OptionalObjectAttributes Specifies the optional fields that you want returned in the response. Fields that you do not specify are not returned.
#'
#' @return
#' A list with the following syntax:
#' ```
...
#' 
#' This action has been revised. We recommend that you use the newer version, [`list_objects_v2`][s3_list_objects_v2], when developing applications. For backward compatibility, Amazon S3 continues to support [`list_objects`][s3_list_objects].
#' 
#' The following operations are related to [`list_objects`][s3_list_objects]:
#' 
#' -   [`list_objects_v2`][s3_list_objects_v2]
#' 
#' -   [`get_object`][s3_get_object]
...
#' 
#' When using the URL encoding type, non-ASCII characters that are used in an object's key name will be percent-encoded according to UTF-8 code values. For example, the object `test_file(3).png` will appear as `test_file%283%29.png`.
#' @param Marker Marker is where you want Amazon S3 to start listing from. Amazon S3 starts listing after this specified key. Marker can be any key in the bucket.
#' @param MaxKeys Sets the maximum number of keys returned in the response. By default, the action returns up to 1,000 key names. The response might contain fewer keys but will never contain more.
...
#' 
#' -   **General purpose bucket** - For general purpose buckets, [`list_objects_v2`][s3_list_objects_v2] doesn't return prefixes that are related only to in-progress multipart uploads.
#' 
#' -   **Directory buckets** - For directory buckets, [`list_objects_v2`][s3_list_objects_v2] response includes the prefixes that are related only to in-progress multipart uploads.
#' 
#' -   **Directory buckets** - For directory buckets, you must make requests for this API operation to the Zonal endpoint. These endpoints support virtual-hosted-style requests in the format `https://amzn-s3-demo-bucket.s3express-zone-id.region-code.amazonaws.com/key-name `. Path-style requests are not supported. For more information about endpoints in Availability Zones, see [Regional and Zonal endpoints for directory buckets in Availability Zones](https://docs.aws.amazon.com/AmazonS3/latest/userguide/endpoint-directory-buckets-AZ.html) in the *Amazon S3 User Guide*. For more information about endpoints in Local Zones, see [Concepts for directory buckets in Local Zones](https://docs.aws.amazon.com/AmazonS3/latest/userguide/s3-lzs-for-directory-buckets.html) in the *Amazon S3 User Guide*.
#' 
#' ### Permissions
...
#' 
#' -   **General purpose bucket** - For general purpose buckets, [`list_objects_v2`][s3_list_objects_v2] returns objects in lexicographical order based on their key names.
#' 
#' -   **Directory bucket** - For directory buckets, [`list_objects_v2`][s3_list_objects_v2] does not return objects in lexicographical order.
#' 
#' ### HTTP Host header syntax
#' 
#' **Directory buckets** - The HTTP Host header syntax is ` Bucket-name.s3express-zone-id.region-code.amazonaws.com`.
#' 
#' This section describes the latest revision of this action. We recommend that you use this revised API operation for application development. For backward compatibility, Amazon S3 continues to support the prior version of this API operation, [`list_objects`][s3_list_objects].
#' 
#' The following operations are related to [`list_objects_v2`][s3_list_objects_v2]:
#' 
#' -   [`get_object`][s3_get_object]
#' 
#' -   [`put_object`][s3_put_object]
...
#'
#' @param Bucket &#91;required&#93; **Directory buckets** - When you use this operation with a directory bucket, you must use virtual-hosted-style requests in the format ` Bucket-name.s3express-zone-id.region-code.amazonaws.com`. Path-style requests are not supported. Directory bucket names must be unique in the chosen Zone (Availability Zone or Local Zone). Bucket names must follow the format ` bucket-base-name--zone-id--x-s3` (for example, ` amzn-s3-demo-bucket--usw2-az1--x-s3`). For information about bucket naming restrictions, see [Directory bucket naming rules](https://docs.aws.amazon.com/AmazonS3/latest/userguide/directory-bucket-naming-rules.html) in the *Amazon S3 User Guide*.
#' 
#' **Access points** - When you use this action with an access point for general purpose buckets, you must provide the alias of the access point in place of the bucket name or specify the access point ARN. When you use this action with an access point for directory buckets, you must provide the access point name in place of the bucket name. When using the access point ARN, you must direct requests to the access point hostname. The access point hostname takes the form *AccessPointName*-*AccountId*.s3-accesspoint.*Region*.amazonaws.com. When using this action with an access point through the Amazon Web Services SDKs, you provide the access point ARN in place of the bucket name. For more information about access point ARNs, see [Using access points](https://docs.aws.amazon.com/AmazonS3/latest/userguide/using-access-points.html) in the *Amazon S3 User Guide*.
#' 
#' Object Lambda access points are not supported by directory buckets.
...
#' **Directory buckets** - For directory buckets, only prefixes that end in a delimiter (`/`) are supported.
#' @param ContinuationToken `ContinuationToken` indicates to Amazon S3 that the list is being continued on this bucket with a token. `ContinuationToken` is obfuscated and is not a real key. You can use this `ContinuationToken` for pagination of the list results.
#' @param FetchOwner The owner field is not present in [`list_objects_v2`][s3_list_objects_v2] by default. If you want to return the owner field with each key in the result, then set the `FetchOwner` field to `true`.
#' 
...
#' Lists the parts that have been uploaded for a specific multipart upload.
#' 
#' To use this operation, you must provide the `upload ID` in the request. You obtain this uploadID by sending the initiate multipart upload request through [`create_multipart_upload`][s3_create_multipart_upload].
#' 
#' The [`list_parts`][s3_list_parts] request returns a maximum of 1,000 uploaded parts. The limit of 1,000 parts is also the default value. You can restrict the number of parts in a response by specifying the `max-parts` request parameter. If your multipart upload consists of more than 1,000 parts, the response returns an `IsTruncated` field with the value of `true`, and a `NextPartNumberMarker` element. To list remaining uploaded parts, in subsequent [`list_parts`][s3_list_parts] requests, include the `part-number-marker` query string parameter and set its value to the `NextPartNumberMarker` field value from the previous response.
#' 
#' For more information on multipart uploads, see [Uploading Objects Using Multipart Upload](https://docs.aws.amazon.com/AmazonS3/latest/userguide/mpuoverview.html) in the *Amazon S3 User Guide*.
#' 
#' **Directory buckets** - For directory buckets, you must make requests for this API operation to the Zonal endpoint. These endpoints support virtual-hosted-style requests in the format `https://amzn-s3-demo-bucket.s3express-zone-id.region-code.amazonaws.com/key-name `. Path-style requests are not supported. For more information about endpoints in Availability Zones, see [Regional and Zonal endpoints for directory buckets in Availability Zones](https://docs.aws.amazon.com/AmazonS3/latest/userguide/endpoint-directory-buckets-AZ.html) in the *Amazon S3 User Guide*. For more information about endpoints in Local Zones, see [Concepts for directory buckets in Local Zones](https://docs.aws.amazon.com/AmazonS3/latest/userguide/s3-lzs-for-directory-buckets.html) in the *Amazon S3 User Guide*.
#' 
#' ### Permissions
...
#' -   **General purpose bucket permissions** - For information about permissions required to use the multipart upload API, see [Multipart Upload and Permissions](https://docs.aws.amazon.com/AmazonS3/latest/userguide/mpuoverview.html) in the *Amazon S3 User Guide*.
#' 
#'     If the upload was created using server-side encryption with Key Management Service (KMS) keys (SSE-KMS) or dual-layer server-side encryption with Amazon Web Services KMS keys (DSSE-KMS), you must have permission to the `kms:Decrypt` action for the [`list_parts`][s3_list_parts] request to succeed.
#' 
#' -   **Directory bucket permissions** - To grant access to this API operation on a directory bucket, we recommend that you use the [`create_session`](https://docs.aws.amazon.com/AmazonS3/latest/API/API_CreateSession.html) API operation for session-based authorization. Specifically, you grant the `s3express:CreateSession` permission to the directory bucket in a bucket policy or an IAM identity-based policy. Then, you make the [`create_session`][s3_create_session] API call on the bucket to obtain a session token. With the session token in your request header, you can make API requests to this operation. After the session token expires, you make another [`create_session`][s3_create_session] API call to generate a new session token for use. Amazon Web Services CLI or SDKs create session and refresh the session token automatically to avoid service interruptions when a session expires. For more information about authorization, see [`create_session`](https://docs.aws.amazon.com/AmazonS3/latest/API/API_CreateSession.html) .
#' 
#' ### HTTP Host header syntax
...
#'
#' @param Bucket &#91;required&#93; The name of the general purpose bucket.
#' @param ContentMD5 The MD5 hash of the [`put_bucket_abac`][s3_put_bucket_abac] request body.
#' 
#' For requests made using the Amazon Web Services Command Line Interface (CLI) or Amazon Web Services SDKs, this field is calculated automatically.
#' @param ChecksumAlgorithm Indicates the algorithm that you want Amazon S3 to use to create the checksum. For more information, see [Checking object integrity](https://docs.aws.amazon.com/AmazonS3/latest/userguide/checking-object-integrity.html) in the *Amazon S3 User Guide*.
#' @param ExpectedBucketOwner The Amazon Web Services account ID of the general purpose bucket's owner.
#' @param AbacStatus &#91;required&#93; The ABAC status of the general purpose bucket. When ABAC is enabled for the general purpose bucket, you can use tags to manage access to the general purpose buckets as well as for cost tracking purposes. When ABAC is disabled for the general purpose buckets, you can only use tags for cost tracking purposes. For more information, see [Using tags with S3 general purpose buckets](https://docs.aws.amazon.com/AmazonS3/latest/userguide/buckets-tagging.html).
#'
#' @return
#' An empty list.
#'
...
#' 
#' The [`get_bucket_accelerate_configuration`][s3_get_bucket_accelerate_configuration] action returns the transfer acceleration state of a bucket.
#' 
#' After setting the Transfer Acceleration state of a bucket to Enabled, it might take up to thirty minutes before the data transfer rates to the bucket increase.
#' 
#' The name of the bucket used for Transfer Acceleration must be DNS-compliant and must not contain periods (".").
#' 
#' For more information about transfer acceleration, see [Transfer Acceleration](https://docs.aws.amazon.com/AmazonS3/latest/userguide/transfer-acceleration.html).
#' 
#' The following operations are related to [`put_bucket_accelerate_configuration`][s3_put_bucket_accelerate_configuration]:
#' 
...
#' This operation is not supported for directory buckets.
#' 
#' Sets the permissions on an existing bucket using access control lists (ACL). For more information, see [Using ACLs](https://docs.aws.amazon.com/AmazonS3/latest/userguide/acl-overview.html). To set the ACL of a bucket, you must have the `WRITE_ACP` permission.
#' 
...
#'     For a list of all the Amazon S3 supported Regions and endpoints, see [Regions and Endpoints](https://docs.aws.amazon.com/general/latest/gr/rande.html#s3_region) in the Amazon Web Services General Reference.
#' 
#' The following operations are related to [`put_bucket_acl`][s3_put_bucket_acl]:
#' 
#' -   [`create_bucket`][s3_create_bucket]
#' 
#' -   [`delete_bucket`][s3_delete_bucket]
...
#' To use this operation, you must have permissions to perform the `s3:PutAnalyticsConfiguration` action. The bucket owner has this permission by default. The bucket owner can grant this permission to others. For more information about permissions, see [Permissions Related to Bucket Subresource Operations](https://docs.aws.amazon.com/AmazonS3/latest/userguide/security_iam_service-with-iam.html#using-with-s3-actions-related-to-bucket-subresources) and [Managing Access Permissions to Your Amazon S3 Resources](https://docs.aws.amazon.com/AmazonS3/latest/userguide/security-iam.html).
#' 
#' [`put_bucket_analytics_configuration`][s3_put_bucket_analytics_configuration] has the following special errors:
#' 
...
#' 
#' The following operations are related to [`put_bucket_analytics_configuration`][s3_put_bucket_analytics_configuration]:
#' 
#' -   [`get_bucket_analytics_configuration`][s3_get_bucket_analytics_configuration]
#' 
#' -   [`delete_bucket_analytics_configuration`][s3_delete_bucket_analytics_configuration]
...
#' 
#' For more information about CORS, go to [Enabling Cross-Origin Resource Sharing](https://docs.aws.amazon.com/AmazonS3/latest/userguide/cors.html) in the *Amazon S3 User Guide*.
#' 
#' The following operations are related to [`put_bucket_cors`][s3_put_bucket_cors]:
#' 
...
#' **Directory buckets** - The HTTP Host header syntax is `s3express-control.region-code.amazonaws.com`.
#' 
#' The following operations are related to [`put_bucket_encryption`][s3_put_bucket_encryption]:
#' 
#' -   [`get_bucket_encryption`][s3_get_bucket_encryption]
#' 
#' -   [`delete_bucket_encryption`][s3_delete_bucket_encryption]
...
#'
#' @param Bucket &#91;required&#93; Specifies default encryption for a bucket using server-side encryption with different key options.
#' 
#' **Directory buckets** - When you use this operation with a directory bucket, you must use path-style requests in the format `https://s3express-control.region-code.amazonaws.com/bucket-name `. Virtual-hosted-style requests aren't supported. Directory bucket names must be unique in the chosen Zone (Availability Zone or Local Zone). Bucket names must also follow the format ` bucket-base-name--zone-id--x-s3` (for example, ` DOC-EXAMPLE-BUCKET--usw2-az1--x-s3`). For information about bucket naming restrictions, see [Directory bucket naming rules](https://docs.aws.amazon.com/AmazonS3/latest/userguide/directory-bucket-naming-rules.html) in the *Amazon S3 User Guide*
...
#' 
#' For directory buckets, when you use Amazon Web Services SDKs, `CRC32` is the default checksum algorithm that's used for performance.
#' @param ServerSideEncryptionConfiguration &#91;required&#93; Specifies the default server-side-encryption configuration.
#' @param ExpectedBucketOwner The account ID of the expected bucket owner. If the account ID that you provide does not match the actual owner of the bucket, the request fails with the HTTP status code `403 Forbidden` (access denied).
#' 
#' For directory buckets, this header is not supported in this API operation. If you specify this header, the request fails with the HTTP status code `501 Not Implemented`.
#'
...
#' For more information, see [Storage class for automatically optimizing frequently and infrequently accessed objects](https://docs.aws.amazon.com/AmazonS3/latest/userguide/storage-class-intro.html#sc-dynamic-data-access).
#' 
#' Operations related to [`put_bucket_intelligent_tiering_configuration`][s3_put_bucket_intelligent_tiering_configuration] include:
#' 
#' -   [`delete_bucket_intelligent_tiering_configuration`][s3_delete_bucket_intelligent_tiering_configuration]
#' 
#' -   [`get_bucket_intelligent_tiering_configuration`][s3_get_bucket_intelligent_tiering_configuration]
...
#' 
#' [`put_bucket_intelligent_tiering_configuration`][s3_put_bucket_intelligent_tiering_configuration] has the following special errors:
#' 
...
#' 
#' The following operations are related to [`put_bucket_inventory_configuration`][s3_put_bucket_inventory_configuration]:
#' 
#' -   [`get_bucket_inventory_configuration`][s3_get_bucket_inventory_configuration]
#' 
#' -   [`delete_bucket_inventory_configuration`][s3_delete_bucket_inventory_configuration]
...
#' This operation is not supported for directory buckets.
#' 
#' Creates a new lifecycle configuration for the bucket or replaces an existing lifecycle configuration. For information about lifecycle configuration, see [Object Lifecycle Management](https://docs.aws.amazon.com/AmazonS3/latest/userguide/object-lifecycle-mgmt.html?refid=awsmp_sol_lp_hlth_lp) in the *Amazon S3 User Guide*.
#' 
...
#' 
#' For more information about permissions, see [Managing Access Permissions to your Amazon S3 Resources](https://docs.aws.amazon.com/AmazonS3/latest/userguide/security-iam.html) in the *Amazon S3 User Guide*.
#' 
#' For more examples of transitioning objects to storage classes such as STANDARD_IA or ONEZONE_IA, see [Examples of Lifecycle Configuration](https://docs.aws.amazon.com/AmazonS3/latest/userguide/intro-lifecycle-rules.html#lifecycle-configuration-examples).
#' 
#' The following operations are related to [`put_bucket_lifecycle`][s3_put_bucket_lifecycle]:
#' 
...
#' 
#'     -   [Specifying Permissions in a Policy](https://docs.aws.amazon.com/AmazonS3/latest/userguide/security_iam_service-with-iam.html)
#' 
#'     -   [Managing Access Permissions to your Amazon S3 Resources](https://docs.aws.amazon.com/AmazonS3/latest/userguide/security-iam.html)
#' 
#' You must URL encode any signed header values that contain spaces. For example, if your header value is `my file.txt`, containing two spaces after `my`, you must URL encode this value to `my%20%20file.txt`.
#'
#' @usage
...
#' @param ContentMD5 For requests made using the Amazon Web Services Command Line Interface (CLI) or Amazon Web Services SDKs, this field is calculated automatically.
#' @param ChecksumAlgorithm Indicates the algorithm used to create the checksum for the request when you use the SDK. This header will not provide any additional functionality if you don't use the SDK. When you send this header, there must be a corresponding `x-amz-checksum` or `x-amz-trailer` header sent. Otherwise, Amazon S3 fails the request with the HTTP status code `400 Bad Request`. For more information, see [Checking object integrity](https://docs.aws.amazon.com/AmazonS3/latest/userguide/checking-object-integrity.html) in the *Amazon S3 User Guide*.
#' 
#' If you provide an individual checksum, Amazon S3 ignores any provided `ChecksumAlgorithm` parameter.
...
#' 
#' Bucket lifecycle configuration now supports specifying a lifecycle rule using an object key name prefix, one or more object tags, object size, or any combination of these. Accordingly, this section describes the latest API. The previous version of the API supported filtering based only on an object key name prefix, which is supported for backward compatibility. For the related API description, see [`put_bucket_lifecycle`][s3_put_bucket_lifecycle].
#' 
#' ### Rules
#' 
...
#' 
#' For more information, see [Object Lifecycle Management](https://docs.aws.amazon.com/AmazonS3/latest/userguide/object-lifecycle-mgmt.html?refid=awsmp_sol_lp_hlth_lp) and [Lifecycle Configuration Elements](https://docs.aws.amazon.com/AmazonS3/latest/userguide/intro-lifecycle-rules.html).
#' 
#' ### Permissions
#' 
#' -   **General purpose bucket permissions** - By default, all Amazon S3 resources are private, including buckets, objects, and related subresources (for example, lifecycle configuration and website configuration). Only the resource owner (that is, the Amazon Web Services account that created it) can access the resource. The resource owner can optionally grant access permissions to others by writing an access policy. For this operation, a user must have the `s3:PutLifecycleConfiguration` permission.
...
#' **Directory buckets** - The HTTP Host header syntax is `s3express-control.region.amazonaws.com`.
#' 
#' The following operations are related to [`put_bucket_lifecycle_configuration`][s3_put_bucket_lifecycle_configuration]:
#' 
#' -   [`get_bucket_lifecycle_configuration`][s3_get_bucket_lifecycle_configuration]
#' 
#' -   [`delete_bucket_lifecycle`][s3_delete_bucket_lifecycle]
...
#' 
#' This parameter applies to general purpose buckets only. It is not supported for directory bucket lifecycle configurations.
#' @param TransitionDefaultMinimumObjectSize Indicates which default minimum object size behavior is applied to the lifecycle configuration.
#' 
#' This parameter applies to general purpose buckets only. It is not supported for directory bucket lifecycle configurations.
#' 
...
#' 
#' For more information about creating a bucket, see [`create_bucket`][s3_create_bucket]. For more information about returning the logging status of a bucket, see [`get_bucket_logging`][s3_get_bucket_logging].
#' 
#' The following operations are related to [`put_bucket_logging`][s3_put_bucket_logging]:
#' 
#' -   [`put_object`][s3_put_object]
#' 
#' -   [`delete_bucket`][s3_delete_bucket]
...
#' @param ContentMD5 The MD5 hash of the [`put_bucket_logging`][s3_put_bucket_logging] request body.
#' 
#' For requests made using the Amazon Web Services Command Line Interface (CLI) or Amazon Web Services SDKs, this field is calculated automatically.
#' @param ChecksumAlgorithm Indicates the algorithm used to create the checksum for the request when you use the SDK. This header will not provide any additional functionality if you don't use the SDK. When you send this header, there must be a corresponding `x-amz-checksum` or `x-amz-trailer` header sent. Otherwise, Amazon S3 fails the request with the HTTP status code `400 Bad Request`. For more information, see [Checking object integrity](https://docs.aws.amazon.com/AmazonS3/latest/userguide/checking-object-integrity.html) in the *Amazon S3 User Guide*.
#' 
...
#' For information about CloudWatch request metrics for Amazon S3, see [Monitoring Metrics with Amazon CloudWatch](https://docs.aws.amazon.com/AmazonS3/latest/userguide/cloudwatch-monitoring.html).
#' 
#' The following operations are related to [`put_bucket_metrics_configuration`][s3_put_bucket_metrics_configuration]:
#' 
#' -   [`delete_bucket_metrics_configuration`][s3_delete_bucket_metrics_configuration]
#' 
#' -   [`get_bucket_metrics_configuration`][s3_get_bucket_metrics_configuration]
...
#' -   [`list_bucket_metrics_configurations`][s3_list_bucket_metrics_configurations]
#' 
#' [`put_bucket_metrics_configuration`][s3_put_bucket_metrics_configuration] has the following special error:
#' 
...
#' @param ContentMD5 The MD5 hash of the [`put_public_access_block`][s3_put_public_access_block] request body.
#' 
#' For requests made using the Amazon Web Services Command Line Interface (CLI) or Amazon Web Services SDKs, this field is calculated automatically.
#' @param ChecksumAlgorithm Indicates the algorithm used to create the checksum for the request when you use the SDK. This header will not provide any additional functionality if you don't use the SDK. When you send this header, there must be a corresponding `x-amz-checksum` or `x-amz-trailer` header sent. Otherwise, Amazon S3 fails the request with the HTTP status code `400 Bad Request`. For more information, see [Checking object integrity](https://docs.aws.amazon.com/AmazonS3/latest/userguide/checking-object-integrity.html) in the *Amazon S3 User Guide*.
#' 
...
#' This operation is not supported for directory buckets.
#' 
#' Enables notifications of specified events for a bucket. For more information about event notifications, see [Configuring Event Notifications](https://docs.aws.amazon.com/AmazonS3/latest/userguide/EventNotifications.html?refid=awsmp_sol_lp_hlth_lp).
#' 
...
#' 
#' The following action is related to [`put_bucket_notification_configuration`][s3_put_bucket_notification_configuration]:
#' 
...
#'
#' @param Bucket &#91;required&#93; The name of the bucket.
#' @param NotificationConfiguration &#91;required&#93; A container for specifying the notification configuration of the bucket. If this element is empty, notifications are turned off for the bucket.
#' @param ExpectedBucketOwner The account ID of the expected bucket owner. If the account ID that you provide does not match the actual owner of the bucket, the request fails with the HTTP status code `403 Forbidden` (access denied).
#' @param SkipDestinationValidation Skips validation of Amazon SQS, Amazon SNS, and Lambda destinations. True or false value.
#'
#' @return
#' An empty list.
#'
...
#' For information about Amazon S3 Object Ownership, see [Using object ownership](https://docs.aws.amazon.com/AmazonS3/latest/userguide/about-object-ownership.html).
#' 
#' The following operations are related to [`put_bucket_ownership_controls`][s3_put_bucket_ownership_controls]:
#' 
#' -   [`get_bucket_ownership_controls`][s3_get_bucket_ownership_controls]
#' 
#' -   [`delete_bucket_ownership_controls`][s3_delete_bucket_ownership_controls]
...
#'
#' @param Bucket &#91;required&#93; The name of the Amazon S3 bucket whose `OwnershipControls` you want to set.
#' @param ContentMD5 The MD5 hash of the `OwnershipControls` request body.
#' 
#' For requests made using the Amazon Web Services Command Line Interface (CLI) or Amazon Web Services SDKs, this field is calculated automatically.
#' @param ExpectedBucketOwner The account ID of the expected bucket owner. If the account ID that you provide does not match the actual owner of the bucket, the request fails with the HTTP status code `403 Forbidden` (access denied).
#' @param OwnershipControls &#91;required&#93; The `OwnershipControls` (BucketOwnerEnforced, BucketOwnerPreferred, or ObjectWriter) that you want to apply to this Amazon S3 bucket.
...
#' ### Permissions
#' 
#' If you are using an identity other than the root user of the Amazon Web Services account that owns the bucket, the calling identity must both have the [`put_bucket_policy`][s3_put_bucket_policy] permissions on the specified bucket and belong to the bucket owner's account in order to use this operation.
#' 
#' If you don't have [`put_bucket_policy`][s3_put_bucket_policy] permissions, Amazon S3 returns a `403 Access Denied` error. If you have the correct permissions, but you're not using an identity that belongs to the bucket owner's account, Amazon S3 returns a `405 Method Not Allowed` error.
#' 
#' To ensure that bucket owners don't inadvertently lock themselves out of their own buckets, the root principal in a bucket owner's Amazon Web Services account can perform the [`get_bucket_policy`][s3_get_bucket_policy], [`put_bucket_policy`][s3_put_bucket_policy], and [`delete_bucket_policy`][s3_delete_bucket_policy] API actions, even if their bucket policy explicitly denies the root principal's access. Bucket owner root principals can only be blocked from performing these API actions by VPC endpoint policies and Amazon Web Services Organizations policies.
#' 
...
#' **Directory buckets** - The HTTP Host header syntax is `s3express-control.region-code.amazonaws.com`.
#' 
#' The following operations are related to [`put_bucket_policy`][s3_put_bucket_policy]:
#' 
#' -   [`create_bucket`][s3_create_bucket]
#' 
#' -   [`delete_bucket`][s3_delete_bucket]
...
#' 
#' If you are using an earlier version of the replication configuration, Amazon S3 handles replication of delete markers differently. For more information, see [Backward Compatibility](https://docs.aws.amazon.com/AmazonS3/latest/userguide/replication-add-config.html#replication-backward-compat-considerations).
#' 
#' For information about enabling versioning on a bucket, see [Using Versioning](https://docs.aws.amazon.com/AmazonS3/latest/userguide/Versioning.html).
#' 
...
#' ### Permissions
#' 
#' To create a [`put_bucket_replication`][s3_put_bucket_replication] request, you must have `s3:PutReplicationConfiguration` permissions for the bucket.
#' 
#' By default, a resource owner, in this case the Amazon Web Services account that created the bucket, can perform this operation. The resource owner can also grant others permissions to perform the operation. For more information about permissions, see [Specifying Permissions in a Policy](https://docs.aws.amazon.com/AmazonS3/latest/userguide/security_iam_service-with-iam.html) and [Managing Access Permissions to Your Amazon S3 Resources](https://docs.aws.amazon.com/AmazonS3/latest/userguide/security-iam.html).
#' 
...
#' 
#' The following operations are related to [`put_bucket_replication`][s3_put_bucket_replication]:
#' 
#' -   [`get_bucket_replication`][s3_get_bucket_replication]
#' 
#' -   [`delete_bucket_replication`][s3_delete_bucket_replication]
...
#' 
#' If you provide an individual checksum, Amazon S3 ignores any provided `ChecksumAlgorithm` parameter.
#' @param ReplicationConfiguration &#91;required&#93; A container for replication rules. You can add up to 1,000 rules. The maximum size of a replication configuration is 2 MB.
...
#' 
#' The following operations are related to [`put_bucket_tagging`][s3_put_bucket_tagging]:
#' 
#' -   [`get_bucket_tagging`][s3_get_bucket_tagging]
#' 
#' -   [`delete_bucket_tagging`][s3_delete_bucket_tagging]
...
#' 
#' If the versioning state has never been set on a bucket, it has no versioning state; a [`get_bucket_versioning`][s3_get_bucket_versioning] request does not return a versioning state value.
#' 
...
#' 
#' If you have an object expiration lifecycle configuration in your non-versioned bucket and you want to maintain the same permanent delete behavior when you enable versioning, you must add a noncurrent expiration policy. The noncurrent expiration lifecycle configuration will manage the deletes of the noncurrent object versions in the version-enabled bucket. (A version-enabled bucket maintains one current and zero or more noncurrent object versions.) For more information, see [Lifecycle and Versioning](https://docs.aws.amazon.com/AmazonS3/latest/userguide/object-lifecycle-mgmt.html?refid=awsmp_sol_lp_hlth_lp#lifecycle-and-other-bucket-config).
#' 
#' The following operations are related to [`put_bucket_versioning`][s3_put_bucket_versioning]:
#' 
#' -   [`create_bucket`][s3_create_bucket]
#' 
#' -   [`delete_bucket`][s3_delete_bucket]
...
#' 
#' If you provide an individual checksum, Amazon S3 ignores any provided `ChecksumAlgorithm` parameter.
#' @param MFA The concatenation of the authentication device's serial number, a space, and the value that is displayed on your authentication device. The serial number is the number that uniquely identifies the MFA device. For physical MFA devices, this is the unique serial number that's provided with the device. For virtual MFA devices, the serial number is the device ARN. For more information, see [Enabling versioning on buckets](https://docs.aws.amazon.com/AmazonS3/latest/userguide/manage-versioning-examples.html) and [Configuring MFA delete](https://docs.aws.amazon.com/AmazonS3/latest/userguide/MultiFactorAuthenticationDelete.html) in the *Amazon Simple Storage Service User Guide*.
...
#' 
#' -   Amazon S3 never adds partial objects; if you receive a success response, Amazon S3 added the entire object to the bucket. You cannot use [`put_object`][s3_put_object] to only update a single piece of metadata for an existing object. You must put the entire object with updated metadata if you want to update some values.
#' 
...
#' 
#'     For more information, see [Add preconditions to S3 operations with conditional requests](https://docs.aws.amazon.com/AmazonS3/latest/userguide/conditional-requests.html) in the *Amazon S3 User Guide* or [RFC 7232](https://datatracker.ietf.org/doc/rfc7232/).
#' 
#'     This functionality is not supported for S3 on Outposts.
#' 
#' -   **S3 Versioning** - When you enable versioning for a bucket, if Amazon S3 receives multiple write requests for the same object simultaneously, it stores all versions of the objects. For each write request that is made to the same object, Amazon S3 automatically generates a unique version ID of that object being stored in Amazon S3. You can retrieve, replace, or delete any version of the object. For more information about versioning, see [Adding Objects to Versioning-Enabled Buckets](https://docs.aws.amazon.com/AmazonS3/latest/userguide/AddingObjectstoVersioningEnabledBuckets.html) in the *Amazon S3 User Guide*. For information about returning the versioning state of a bucket, see [`get_bucket_versioning`][s3_get_bucket_versioning].
#' 
#'     This functionality is not supported for directory buckets.
#' 
#' ### Permissions
#' 
#' -   **General purpose bucket permissions** - The following permissions are required in your policies when your [`put_object`][s3_put_object] request includes specific headers.
#' 
#'     -   **`s3:PutObject`** - To successfully complete the [`put_object`][s3_put_object] request, you must always have the `s3:PutObject` permission on a bucket to add an object to it.
#' 
#'     -   **`s3:PutObjectAcl`** - To successfully change the objects ACL of your [`put_object`][s3_put_object] request, you must have the `s3:PutObjectAcl`.
#' 
#'     -   **`s3:PutObjectTagging`** - To successfully set the tag-set with your [`put_object`][s3_put_object] request, you must have the `s3:PutObjectTagging`.
#' 
#' -   **Directory bucket permissions** - To grant access to this API operation on a directory bucket, we recommend that you use the [`create_session`](https://docs.aws.amazon.com/AmazonS3/latest/API/API_CreateSession.html) API operation for session-based authorization. Specifically, you grant the `s3express:CreateSession` permission to the directory bucket in a bucket policy or an IAM identity-based policy. Then, you make the [`create_session`][s3_create_session] API call on the bucket to obtain a session token. With the session token in your request header, you can make API requests to this operation. After the session token expires, you make another [`create_session`][s3_create_session] API call to generate a new session token for use. Amazon Web Services CLI or SDKs create session and refresh the session token automatically to avoid service interruptions when a session expires. For more information about authorization, see [`create_session`](https://docs.aws.amazon.com/AmazonS3/latest/API/API_CreateSession.html) .
#' 
#'     If the object is encrypted with SSE-KMS, you must also have the `kms:GenerateDataKey` and `kms:Decrypt` permissions in IAM identity-based policies and KMS key policies for the KMS key.
...
#'   ExpectedBucketOwner)
#'
#' @param ACL The canned ACL to apply to the object. For more information, see [Canned ACL](https://docs.aws.amazon.com/AmazonS3/latest/userguide/acl-overview.html#CannedACL) in the *Amazon S3 User Guide*.
#' 
#' When adding a new object, you can use headers to grant ACL-based permissions to individual Amazon Web Services accounts or to predefined groups defined by Amazon S3. These permissions are then added to the ACL on the object. By default, all objects are private. Only the owner has full access control. For more information, see [Access Control List (ACL) Overview](https://docs.aws.amazon.com/AmazonS3/latest/userguide/acl-overview.html) and [Managing ACLs Using the REST API](https://docs.aws.amazon.com/AmazonS3/latest/userguide/managing-acls.html) in the *Amazon S3 User Guide*.
#' 
...
#' @param WriteOffsetBytes Specifies the offset for appending data to existing objects in bytes. The offset must be equal to the size of the existing object being appended to. If no object exists, setting this header to 0 will create a new object.
#' 
#' This functionality is only supported for objects in the Amazon S3 Express One Zone storage class in directory buckets.
#' @param Metadata A map of metadata to store with the object in S3.
#' @param ServerSideEncryption The server-side encryption algorithm that was used when you store this object in Amazon S3 or Amazon FSx.
#' 
...
#' 
#' The following operations are related to [`put_object_acl`][s3_put_object_acl]:
#' 
#' -   [`copy_object`][s3_copy_object]
#' 
#' -   [`get_object`][s3_get_object]
...
#'   ExpectedBucketOwner)
#'
#' @param ACL The canned ACL to apply to the object. For more information, see [Canned ACL](https://docs.aws.amazon.com/AmazonS3/latest/userguide/acl-overview.html#CannedACL).
#' @param AccessControlPolicy Contains the elements that set the ACL permissions for an object per grantee.
#' @param Bucket &#91;required&#93; The bucket name that contains the object to which you want to attach the ACL.
#' 
#' **Access points** - When you use this action with an access point for general purpose buckets, you must provide the alias of the access point in place of the bucket name or specify the access point ARN. When you use this action with an access point for directory buckets, you must provide the access point name in place of the bucket name. When using the access point ARN, you must direct requests to the access point hostname. The access point hostname takes the form *AccessPointName*-*AccountId*.s3-accesspoint.*Region*.amazonaws.com. When using this action with an access point through the Amazon Web Services SDKs, you provide the access point ARN in place of the bucket name. For more information about access point ARNs, see [Using access points](https://docs.aws.amazon.com/AmazonS3/latest/userguide/using-access-points.html) in the *Amazon S3 User Guide*.
#' 
#' **S3 on Outposts** - When you use this action with S3 on Outposts, you must direct requests to the S3 on Outposts hostname. The S3 on Outposts hostname takes the form ` AccessPointName-AccountId.outpostID.s3-outposts.Region.amazonaws.com`. When you use this action with S3 on Outposts, the destination bucket must be the Outposts access point ARN or the access point alias. For more information about S3 on Outposts, see [What is S3 on Outposts?](https://docs.aws.amazon.com/AmazonS3/latest/s3-outposts/S3onOutposts.html) in the *Amazon S3 User Guide*.
...
#'
#' @param Bucket &#91;required&#93; The bucket name containing the object that you want to place a legal hold on.
#' 
#' **Access points** - When you use this action with an access point for general purpose buckets, you must provide the alias of the access point in place of the bucket name or specify the access point ARN. When you use this action with an access point for directory buckets, you must provide the access point name in place of the bucket name. When using the access point ARN, you must direct requests to the access point hostname. The access point hostname takes the form *AccessPointName*-*AccountId*.s3-accesspoint.*Region*.amazonaws.com. When using this action with an access point through the Amazon Web Services SDKs, you provide the access point ARN in place of the bucket name. For more information about access point ARNs, see [Using access points](https://docs.aws.amazon.com/AmazonS3/latest/userguide/using-access-points.html) in the *Amazon S3 User Guide*.
...
#'
#' @param Bucket &#91;required&#93; The bucket whose Object Lock configuration you want to create or replace.
...
#'   ExpectedBucketOwner)
#'
#' @param Bucket &#91;required&#93; The bucket name that contains the object you want to apply this Object Retention configuration to.
#' 
#' **Access points** - When you use this action with an access point for general purpose buckets, you must provide the alias of the access point in place of the bucket name or specify the access point ARN. When you use this action with an access point for directory buckets, you must provide the access point name in place of the bucket name. When using the access point ARN, you must direct requests to the access point hostname. The access point hostname takes the form *AccessPointName*-*AccountId*.s3-accesspoint.*Region*.amazonaws.com. When using this action with an access point through the Amazon Web Services SDKs, you provide the access point ARN in place of the bucket name. For more information about access point ARNs, see [Using access points](https://docs.aws.amazon.com/AmazonS3/latest/userguide/using-access-points.html) in the *Amazon S3 User Guide*.
#' @param Key &#91;required&#93; The key name for the object that you want to apply this Object Retention configuration to.
...
#' @param BypassGovernanceRetention Indicates whether this action should bypass Governance-mode restrictions.
#' @param ContentMD5 The MD5 hash for the request body.
#' 
#' For requests made using the Amazon Web Services Command Line Interface (CLI) or Amazon Web Services SDKs, this field is calculated automatically.
#' @param ChecksumAlgorithm Indicates the algorithm used to create the checksum for the object when you use the SDK. This header will not provide any additional functionality if you don't use the SDK. When you send this header, there must be a corresponding `x-amz-checksum` or `x-amz-trailer` header sent. Otherwise, Amazon S3 fails the request with the HTTP status code `400 Bad Request`. For more information, see [Checking object integrity](https://docs.aws.amazon.com/AmazonS3/latest/userguide/checking-object-integrity.html) in the *Amazon S3 User Guide*.
...
#' 
#' You can associate tags with an object by sending a PUT request against the tagging subresource that is associated with the object. You can retrieve tags by sending a GET request. For more information, see [`get_object_tagging`][s3_get_object_tagging].
#' 
#' For tagging-related restrictions related to characters and encodings, see [Tag Restrictions](https://docs.aws.amazon.com/awsaccountbilling/latest/aboutv2/). Note that Amazon S3 limits the maximum number of tags to 10 tags per object.
#' 
...
#' 
#' The following operations are related to [`put_object_tagging`][s3_put_object_tagging]:
#' 
#' -   [`get_object_tagging`][s3_get_object_tagging]
#' 
#' -   [`delete_object_tagging`][s3_delete_object_tagging]
...
#' @param Tagging &#91;required&#93; Container for the `TagSet` and `Tag` elements
#' @param ExpectedBucketOwner The account ID of the expected bucket owner. If the account ID that you provide does not match the actual owner of the bucket, the request fails with the HTTP status code `403 Forbidden` (access denied).
#' @param RequestPayer Confirms that the requester knows that she or he will be charged for the tagging object request. Bucket owners need not specify this parameter in their requests.
#'
#' @return
#' A list with the following syntax:
#' ```
...
#'
#' @param Bucket &#91;required&#93; The name of the Amazon S3 bucket whose `PublicAccessBlock` configuration you want to set.
#' @param ContentMD5 The MD5 hash of the [`put_public_access_block`][s3_put_public_access_block] request body.
#' 
#' For requests made using the Amazon Web Services Command Line Interface (CLI) or Amazon Web Services SDKs, this field is calculated automatically.
#' @param ChecksumAlgorithm Indicates the algorithm used to create the checksum for the object when you use the SDK. This header will not provide any additional functionality if you don't use the SDK. When you send this header, there must be a corresponding `x-amz-checksum` or `x-amz-trailer` header sent. Otherwise, Amazon S3 fails the request with the HTTP status code `400 Bad Request`. For more information, see [Checking object integrity](https://docs.aws.amazon.com/AmazonS3/latest/userguide/checking-object-integrity.html) in the *Amazon S3 User Guide*.
...
#' 
#' [`rename_object`][s3_rename_object] is only supported for objects stored in the S3 Express One Zone storage class.
#' 
#' To prevent overwriting an object, you can use the `If-None-Match` conditional header.
#' 
#' -   **If-None-Match** - Renames the object only if an object with the specified name does not already exist in the directory bucket. If you don't want to overwrite an existing object, you can add the `If-None-Match` conditional header with the value `‘*’` in the [`rename_object`][s3_rename_object] request. Amazon S3 then returns a `412 Precondition Failed` error if the object with the specified name already exists. For more information, see [RFC 7232](https://datatracker.ietf.org/doc/rfc7232/).
#' 
#' ### Permissions
#' 
#' To grant access to the [`rename_object`][s3_rename_object] operation on a directory bucket, we recommend that you use the [`create_session`][s3_create_session] operation for session-based authorization. Specifically, you grant the `s3express:CreateSession` permission to the directory bucket in a bucket policy or an IAM identity-based policy. Then, you make the [`create_session`][s3_create_session] API call on the directory bucket to obtain a session token. With the session token in your request header, you can make API requests to this operation. After the session token expires, you make another [`create_session`][s3_create_session] API call to generate a new session token for use. The Amazon Web Services CLI and SDKs will create and manage your session including refreshing the session token automatically to avoid service interruptions when a session expires. In your bucket policy, you can specify the `s3express:SessionMode` condition key to control who can create a `ReadWrite` or `ReadOnly` session. A `ReadWrite` session is required for executing all the Zonal endpoint API operations, including [`rename_object`][s3_rename_object]. For more information about authorization, see [`create_session`](https://docs.aws.amazon.com/AmazonS3/latest/API/API_CreateSession.html) . To learn more about Zonal endpoint API operations, see [Authorizing Zonal endpoint API operations with CreateSession](https://docs.aws.amazon.com/AmazonS3/latest/userguide/s3-express-create-session.html) in the *Amazon S3 User Guide*.
#' 
#' ### HTTP Host header syntax
#' 
#' **Directory buckets** - The HTTP Host header syntax is ` Bucket-name.s3express-zone-id.region-code.amazonaws.com`.
...
#' @param RenameSource &#91;required&#93; Specifies the source for the rename operation. The value must be URL encoded.
#' @param DestinationIfMatch Renames the object only if the ETag (entity tag) value provided during the operation matches the ETag of the object in S3. The `If-Match` header field makes the request method conditional on ETags. If the ETag values do not match, the operation returns a `412 Precondition Failed` error.
#' 
#' Expects the ETag value as a string.
#' @param DestinationIfNoneMatch Renames the object only if the destination does not already exist in the specified directory bucket. If the object does exist when you send a request with `If-None-Match:*`, the S3 API will return a `412 Precondition Failed` error, preventing an overwrite. The `If-None-Match` header prevents overwrites of existing data by validating that there's not an object with the same key name already in your directory bucket.
#' 
#' Expects the `*` character (asterisk).
#' @param DestinationIfModifiedSince Renames the object if the destination exists and if it has been modified since the specified time.
#' @param DestinationIfUnmodifiedSince Renames the object if it hasn't been modified since the specified time.
#' @param SourceIfMatch Renames the object if the source exists and if its entity tag (ETag) matches the specified ETag.
#' @param SourceIfNoneMatch Renames the object if the source exists and if its entity tag (ETag) is different than the specified ETag. If an asterisk (`*`) character is provided, the operation will fail and return a `412 Precondition Failed` error.
#' @param SourceIfModifiedSince Renames the object if the source exists and if it has been modified since the specified time.
#' @param SourceIfUnmodifiedSince Renames the object if the source exists and hasn't been modified since the specified time.
#' @param ClientToken A unique string with a max of 64 ASCII characters in the ASCII range of 33 - 126.
#' 
#' [`rename_object`][s3_rename_object] supports idempotency using a client token. To make an idempotent API request using [`rename_object`][s3_rename_object], specify a client token in the request. You should not reuse the same client token for other API requests. If you retry a request that completed successfully using the same client token and the same parameters, the retry succeeds without performing any further actions. If you retry a successful request using the same client token, but one or more of the parameters are different, the retry fails and an `IdempotentParameterMismatch` error is returned.
#'
#' @return
#' An empty list.
#'
...
#' -   [`put_object`][s3_put_object]
#' 
#' -   [Managing Access with ACLs](https://docs.aws.amazon.com/AmazonS3/latest/userguide/acl-overview.html) in the *Amazon S3 User Guide*
#' 
#' -   [Protecting Data Using Server-Side Encryption](https://docs.aws.amazon.com/AmazonS3/latest/userguide/serv-side-encryption.html) in the *Amazon S3 User Guide*
#' 
#' ### Permissions
#' 
...
#' 
#' For more information about archive retrieval options and provisioned capacity for `Expedited` data access, see [Restoring Archived Objects](https://docs.aws.amazon.com/AmazonS3/latest/userguide/restoring-objects.html) in the *Amazon S3 User Guide*.
#' 
...
#' 
#' The following operations are related to [`restore_object`][s3_restore_object]:
#' 
#' -   [`put_bucket_lifecycle_configuration`][s3_put_bucket_lifecycle_configuration]
#' 
#' -   [`get_bucket_notification_configuration`][s3_get_bucket_notification_configuration]
...
#' This functionality is not supported for Amazon S3 on Outposts.
#' 
#' For more information about Amazon S3 Select, see [Selecting Content from Objects](https://docs.aws.amazon.com/AmazonS3/latest/userguide/selecting-content-from-objects.html) and [SELECT Command](https://docs.aws.amazon.com/AmazonS3/latest/userguide/s3-select-sql-reference-select.html) in the *Amazon S3 User Guide*.
#' 
#' ### Permissions
#' 
...
#' 
#'     For objects that are encrypted with customer-provided encryption keys (SSE-C), you must use HTTPS, and you must use the headers that are documented in the [`get_object`][s3_get_object]. For more information about SSE-C, see [Server-Side Encryption (Using Customer-Provided Encryption Keys)](https://docs.aws.amazon.com/AmazonS3/latest/userguide/ServerSideEncryptionCustomerKeys.html) in the *Amazon S3 User Guide*.
#' 
...
#' ### GetObject Support
#' 
#' The [`select_object_content`][s3_select_object_content] action does not support the following [`get_object`][s3_get_object] functionality. For more information, see [`get_object`][s3_get_object].
#' 
...
#' -   `kms:DescribeKey`
#' 
#' The following operations are related to [`update_bucket_metadata_inventory_table_configuration`][s3_update_bucket_metadata_inventory_table_configuration]:
#' 
#' -   [`create_bucket_metadata_configuration`][s3_create_bucket_metadata_configuration]
#' 
#' -   [`delete_bucket_metadata_configuration`][s3_delete_bucket_metadata_configuration]
...
#'   ExpectedBucketOwner)
#'
#' @param Bucket &#91;required&#93; The general purpose bucket that corresponds to the metadata configuration that you want to enable or disable an inventory table for.
...
#' @param ExpectedBucketOwner The expected owner of the general purpose bucket that corresponds to the metadata table configuration that you want to enable or disable an inventory table for.
#'
#' @return
#' An empty list.
#'
...
#' 
#' The following operations are related to [`update_bucket_metadata_journal_table_configuration`][s3_update_bucket_metadata_journal_table_configuration]:
#' 
#' -   [`create_bucket_metadata_configuration`][s3_create_bucket_metadata_configuration]
#' 
#' -   [`delete_bucket_metadata_configuration`][s3_delete_bucket_metadata_configuration]
...
#'   ExpectedBucketOwner)
#'
#' @param Bucket &#91;required&#93; The general purpose bucket that corresponds to the metadata configuration that you want to enable or disable journal table record expiration for.
...
#' @param ExpectedBucketOwner The expected owner of the general purpose bucket that corresponds to the metadata table configuration that you want to enable or disable journal table record expiration for.
#'
#' @return
#' An empty list.
#'
...
#' 
#' Updates the server-side encryption type of an existing encrypted object in a general purpose bucket. You can use the [`update_object_encryption`][s3_update_object_encryption] operation to change encrypted objects from server-side encryption with Amazon S3 managed keys (SSE-S3) to server-side encryption with Key Management Service (KMS) keys (SSE-KMS), or to apply S3 Bucket Keys. You can also use the [`update_object_encryption`][s3_update_object_encryption] operation to change the customer-managed KMS key used to encrypt your data so that you can comply with custom key-rotation standards.
#' 
#' Using the [`update_object_encryption`][s3_update_object_encryption] operation, you can atomically update the server-side encryption type of an existing object in a general purpose bucket without any data movement. The [`update_object_encryption`][s3_update_object_encryption] operation uses envelope encryption to re-encrypt the data key used to encrypt and decrypt your object with your newly specified server-side encryption type. In other words, when you use the [`update_object_encryption`][s3_update_object_encryption] operation, your data isn't copied, archived objects in the S3 Glacier Flexible Retrieval and S3 Glacier Deep Archive storage classes aren't restored, and objects in the S3 Intelligent-Tiering storage class aren't moved between tiers. Additionally, the [`update_object_encryption`][s3_update_object_encryption] operation preserves all object metadata properties, including the storage class, creation date, last modified date, ETag, and checksum properties. For more information, see [Updating server-side encryption for existing objects](https://docs.aws.amazon.com/AmazonS3/latest/userguide/update-sse-encryption.html) in the *Amazon S3 User Guide*.
#' 
#' By default, all [`update_object_encryption`][s3_update_object_encryption] requests that specify a customer-managed KMS key are restricted to KMS keys that are owned by the bucket owner's Amazon Web Services account. If you're using Organizations, you can request the ability to use KMS keys owned by other member accounts within your organization by contacting Amazon Web Services Support.
#' 
#' Source objects that are unencrypted, or encrypted with either dual-layer server-side encryption with KMS keys (DSSE-KMS) or server-side encryption with customer-provided keys (SSE-C) aren't supported by this operation. Additionally, you cannot specify SSE-S3 encryption as the requested new encryption type [`update_object_encryption`][s3_update_object_encryption] request.
#' 
#' ### Permissions
#' 
...
#' -   You might receive an `InvalidRequest` error for several reasons. Depending on the reason for the error, you might receive one of the following messages:
#' 
#'     -   The [`update_object_encryption`][s3_update_object_encryption] operation doesn't supported unencrypted source objects. Only source objects encrypted with SSE-S3 or SSE-KMS are supported.
#' 
#'     -   The [`update_object_encryption`][s3_update_object_encryption] operation doesn't support source objects with the encryption type DSSE-KMS or SSE-C. Only source objects encrypted with SSE-S3 or SSE-KMS are supported.
#' 
#'     -   The [`update_object_encryption`][s3_update_object_encryption] operation doesn't support updating the encryption type to DSSE-KMS or SSE-C. Modify the request to specify SSE-KMS for the updated encryption type, and then try again.
#' 
...
#' 
#'     -   The bucket owner's account was approved to make [`update_object_encryption`][s3_update_object_encryption] requests that use any Amazon Web Services KMS key in their organization, but the bucket owner's account isn't part of an organization in Organizations. Make sure that the bucket owner's account and the specified KMS key belong to the same organization, and then try again.
#' 
...
#' 
#'     -   The encryption type for the specified object can’t be updated because that object is protected by S3 Object Lock. If the object has a governance-mode retention period or a legal hold, you must first remove the Object Lock status on the object before you issue your [`update_object_encryption`][s3_update_object_encryption] request. You can't use the [`update_object_encryption`][s3_update_object_encryption] operation with objects that have an Object Lock compliance mode retention period applied to them.
#'
#' @usage
...
#'
#' @param Bucket &#91;required&#93; The name of the general purpose bucket that contains the specified object key name.
#' 
...
#' @param Key &#91;required&#93; The key name of the object that you want to update the server-side encryption type for.
#' @param VersionId The version ID of the object that you want to update the server-side encryption type for.
#' @param ObjectEncryption &#91;required&#93; The updated server-side encryption type for this object. The [`update_object_encryption`][s3_update_object_encryption] operation supports the SSE-S3 and SSE-KMS encryption types.
#' 
#' Valid Values: `SSES3` | `SSEKMS`
#' @param RequestPayer Confirms that the requester knows that they will be charged for the request. Bucket owners need not specify this parameter in their requests. If either the source or destination S3 bucket has Requester Pays enabled, the requester will pay for the corresponding charges. For information about downloading objects from Requester Pays buckets, see [Downloading Objects in Requester Pays Buckets](https://docs.aws.amazon.com/AmazonS3/latest/userguide/ObjectsinRequesterPaysBuckets.html) in the *Amazon S3 User Guide*.
#' 
#' This functionality is not supported for directory buckets.
#' @param ExpectedBucketOwner The account ID of the expected bucket owner. If the account ID that you provide doesn't match the actual owner of the bucket, the request fails with the HTTP status code `403 Forbidden` (access denied).
#' @param ContentMD5 The MD5 hash for the request body. For requests made using the Amazon Web Services Command Line Interface (CLI) or Amazon Web Services SDKs, this field is calculated automatically.
...
#' Uploads a part in a multipart upload.
#' 
#' In this operation, you provide new data as a part of an object in your request. However, you have an option to specify your existing Amazon S3 object as a data source for the part you are uploading. To upload a part from an existing object, you use the [`upload_part_copy`][s3_upload_part_copy] operation.
#' 
#' You must initiate a multipart upload (see [`create_multipart_upload`][s3_create_multipart_upload]) before you can upload any part. In response to your initiate request, Amazon S3 returns an upload ID, a unique identifier that you must include in your upload part request.
#' 
...
#' ### Permissions
#' 
#' -   **General purpose bucket permissions** - To perform a multipart upload with encryption using an Key Management Service key, the requester must have permission to the `kms:Decrypt` and `kms:GenerateDataKey` actions on the key. The requester must also have permissions for the `kms:GenerateDataKey` action for the [`create_multipart_upload`][s3_create_multipart_upload] API. Then, the requester needs permissions for the `kms:Decrypt` action on the [`upload_part`][s3_upload_part] and [`upload_part_copy`][s3_upload_part_copy] APIs.
#' 
...
#' 
#'     Server-side encryption is supported by the S3 Multipart Upload operations. Unless you are using a customer-provided encryption key (SSE-C), you don't need to specify the encryption parameters in each UploadPart request. Instead, you only need to specify the server-side encryption parameters in the initial Initiate Multipart request. For more information, see [`create_multipart_upload`][s3_create_multipart_upload].
#' 
...
#'     -   x-amz-server-side-encryption-customer-key-MD5
#' 
#'     For more information, see [Using Server-Side Encryption](https://docs.aws.amazon.com/AmazonS3/latest/userguide/UsingServerSideEncryption.html) in the *Amazon S3 User Guide*.
#' 
...
#' If you provide an individual checksum, Amazon S3 ignores any provided `ChecksumAlgorithm` parameter.
#' 
#' This checksum algorithm must be the same for all parts and it match the checksum value supplied in the [`create_multipart_upload`][s3_create_multipart_upload] request.
#' @param ChecksumCRC32 This header can be used as a data integrity check to verify that the data received is the same data that was originally sent. This header specifies the Base64 encoded, 32-bit `CRC32` checksum of the object. For more information, see [Checking object integrity](https://docs.aws.amazon.com/AmazonS3/latest/userguide/checking-object-integrity.html) in the *Amazon S3 User Guide*.
#' @param ChecksumCRC32C This header can be used as a data integrity check to verify that the data received is the same data that was originally sent. This header specifies the Base64 encoded, 32-bit `CRC32C` checksum of the object. For more information, see [Checking object integrity](https://docs.aws.amazon.com/AmazonS3/latest/userguide/checking-object-integrity.html) in the *Amazon S3 User Guide*.
...
#' For information about maximum and minimum part sizes and other multipart upload specifications, see [Multipart upload limits](https://docs.aws.amazon.com/AmazonS3/latest/userguide/qfacts.html) in the *Amazon S3 User Guide*.
#' 
#' Instead of copying data from an existing object as part data, you might use the [`upload_part`][s3_upload_part] action to upload new data as a part of an object in your request.
#' 
#' You must initiate a multipart upload before you can upload any part. In response to your initiate request, Amazon S3 returns the upload ID, a unique identifier that you must include in your upload part request.
#' 
#' For conceptual information about multipart uploads, see [Uploading Objects Using Multipart Upload](https://docs.aws.amazon.com/AmazonS3/latest/userguide/mpuoverview.html) in the *Amazon S3 User Guide*. For information about copying objects using a single atomic action vs. a multipart upload, see [Operations on Objects](https://docs.aws.amazon.com/AmazonS3/latest/userguide/uploading-downloading-objects.html) in the *Amazon S3 User Guide*.
#' 
#' **Directory buckets** - For directory buckets, you must make requests for this API operation to the Zonal endpoint. These endpoints support virtual-hosted-style requests in the format `https://amzn-s3-demo-bucket.s3express-zone-id.region-code.amazonaws.com/key-name `. Path-style requests are not supported. For more information about endpoints in Availability Zones, see [Regional and Zonal endpoints for directory buckets in Availability Zones](https://docs.aws.amazon.com/AmazonS3/latest/userguide/endpoint-directory-buckets-AZ.html) in the *Amazon S3 User Guide*. For more information about endpoints in Local Zones, see [Concepts for directory buckets in Local Zones](https://docs.aws.amazon.com/AmazonS3/latest/userguide/s3-lzs-for-directory-buckets.html) in the *Amazon S3 User Guide*.
#' 
#' ### Authentication and authorization
...
#' You must have `READ` access to the source object and `WRITE` access to the destination bucket.
#' 
#' -   **General purpose bucket permissions** - You must have the permissions in a policy based on the bucket types of your source bucket and destination bucket in an [`upload_part_copy`][s3_upload_part_copy] operation.
#' 
...
#'     -   To perform a multipart upload with encryption using an Key Management Service key, the requester must have permission to the `kms:Decrypt` and `kms:GenerateDataKey` actions on the key. The requester must also have permissions for the `kms:GenerateDataKey` action for the [`create_multipart_upload`][s3_create_multipart_upload] API. Then, the requester needs permissions for the `kms:Decrypt` action on the [`upload_part`][s3_upload_part] and [`upload_part_copy`][s3_upload_part_copy] APIs. These permissions are required because Amazon S3 must decrypt and read data from the encrypted file parts before it completes the multipart upload. For more information about KMS permissions, see [Protecting data using server-side encryption with KMS](https://docs.aws.amazon.com/AmazonS3/latest/userguide/UsingKMSEncryption.html) in the *Amazon S3 User Guide*. For information about the permissions required to use the multipart upload API, see [Multipart upload and permissions](https://docs.aws.amazon.com/AmazonS3/latest/userguide/mpuoverview.html) and [Multipart upload API and permissions](https://docs.aws.amazon.com/AmazonS3/latest/userguide/mpuoverview.html#mpuAndPermissions) in the *Amazon S3 User Guide*.
#' 
#' -   **Directory bucket permissions** - You must have permissions in a bucket policy or an IAM identity-based policy based on the source and destination bucket types in an [`upload_part_copy`][s3_upload_part_copy] operation.
#' 
#'     -   If the source object that you want to copy is in a directory bucket, you must have the **`s3express:CreateSession`** permission in the `Action` element of a policy to read the object. If no session mode is specified, the session will be created with the maximum allowable privilege, attempting `ReadWrite` first, then `ReadOnly` if `ReadWrite` is not permitted. If you want to explicitly restrict the access to be read-only, you can set the `s3express:SessionMode` condition key to `ReadOnly` on the copy source bucket.
#' 
...
#' ### Encryption
#' 
#' -   **General purpose buckets** - For information about using server-side encryption with customer-provided encryption keys with the [`upload_part_copy`][s3_upload_part_copy] operation, see [`copy_object`][s3_copy_object] and [`upload_part`][s3_upload_part].
#' 
#'     If you have server-side encryption with customer-provided keys (SSE-C) blocked for your general purpose bucket, you will get an HTTP 403 Access Denied error when you specify the SSE-C request headers while writing new data to your bucket. For more information, see [Blocking or unblocking SSE-C for a general purpose bucket](https://docs.aws.amazon.com/AmazonS3/latest/userguide/blocking-unblocking-s3-c-encryption-gpb.html).
#' 
...
#' **Directory buckets** - The HTTP Host header syntax is ` Bucket-name.s3express-zone-id.region-code.amazonaws.com`.
#' 
#' The following operations are related to [`upload_part_copy`][s3_upload_part_copy]:
#' 
#' -   [`create_multipart_upload`][s3_create_multipart_upload]
#' 
#' -   [`upload_part`][s3_upload_part]
...
#' 
#' **S3 on Outposts** - When you use this action with S3 on Outposts, you must direct requests to the S3 on Outposts hostname. The S3 on Outposts hostname takes the form ` AccessPointName-AccountId.outpostID.s3-outposts.Region.amazonaws.com`. When you use this action with S3 on Outposts, the destination bucket must be the Outposts access point ARN or the access point alias. For more information about S3 on Outposts, see [What is S3 on Outposts?](https://docs.aws.amazon.com/AmazonS3/latest/s3-outposts/S3onOutposts.html) in the *Amazon S3 User Guide*.
#' @param CopySource &#91;required&#93; Specifies the source object for the copy operation. You specify the value in one of two formats, depending on whether you want to access the source object through an [access point](https://docs.aws.amazon.com/AmazonS3/latest/userguide/access-points.html):
#' 
...
#' 
#' This operation supports metadata that can be returned by [`get_object`][s3_get_object], in addition to `RequestRoute`, `RequestToken`, `StatusCode`, `ErrorCode`, and `ErrorMessage`. The [`get_object`][s3_get_object] response metadata is supported so that the [`write_get_object_response`][s3_write_get_object_response] caller, typically an Lambda function, can provide the same metadata when it internally invokes [`get_object`][s3_get_object]. When [`write_get_object_response`][s3_write_get_object_response] is called by a customer-owned Lambda function, the metadata returned to the end user [`get_object`][s3_get_object] call might differ from what Amazon S3 would normally return.
#' 
#' You can include any number of metadata headers. When including a metadata header, it should be prefaced with `x-amz-meta`. For example, `x-amz-meta-my-custom-header: MyCustomValue`. The primary use case for this is to forward [`get_object`][s3_get_object] metadata.
#' 
...
#' @param RequestToken &#91;required&#93; A single use encrypted token that maps [`write_get_object_response`][s3_write_get_object_response] to the end user [`get_object`][s3_get_object] request.
#' @param Body The object data.
#' @param StatusCode The integer status code for an HTTP response of a corresponding [`get_object`][s3_get_object] request. The following is a list of status codes.
#' 
#' -   `200 - OK`
#' 
#' -   `206 - Partial Content`
...
#' @param ErrorCode A string that uniquely identifies an error condition. Returned in the \<Code\> tag of the error XML response for a corresponding [`get_object`][s3_get_object] call. Cannot be used with a successful `StatusCode` header or when the transformed object is provided in the body. All error codes from S3 are sentence-cased. The regular expression (regex) value is `"^[A-Z][a-zA-Z]+$"`.
#' @param ErrorMessage Contains a generic description of the error condition. Returned in the \<Message\> tag of the error XML response for a corresponding [`get_object`][s3_get_object] call. Cannot be used with a successful `StatusCode` header or when the transformed object is provided in body.
...
#' @param ChecksumXXHASH3 This header can be used as a data integrity check to verify that the data received is the same data that was originally sent. This header specifies the Base64 encoded, 64-bit `XXHASH3` checksum of the part. For more information, see [Checking object integrity](https://docs.aws.amazon.com/AmazonS3/latest/userguide/checking-object-integrity.html) in the *Amazon S3 User Guide*.
#' @param ChecksumXXHASH128 This header can be used as a data integrity check to verify that the data received is the same data that was originally sent. This header specifies the Base64 encoded, 128-bit `XXHASH128` checksum of the part. For more information, see [Checking object integrity](https://docs.aws.amazon.com/AmazonS3/latest/userguide/checking-object-integrity.html) in the *Amazon S3 User Guide*.
#' @param DeleteMarker Specifies whether an object stored in Amazon S3 is (`true`) or is not (`false`) a delete marker. To learn more about delete markers, see [Working with delete markers](https://docs.aws.amazon.com/AmazonS3/latest/userguide/DeleteMarker.html).
#' @param ETag An opaque identifier assigned by a web server to a specific version of a resource found at a URL.
#' @param Expires The date and time at which the object is no longer cacheable.
#' @param Expiration If the object expiration is configured (see PUT Bucket lifecycle), the response includes this header. It includes the `expiry-date` and `rule-id` key-value pairs that provide the object expiration information. The value of the `rule-id` is URL-encoded.
#' @param LastModified The date and time that the object was last modified.
#' @param MissingMeta Set to the number of metadata entries not returned in `x-amz-meta` headers. This can happen if you create metadata using an API like SOAP that supports more flexible metadata than the REST API. For example, using SOAP, you can create metadata whose values are not legal HTTP headers.
#' @param Metadata A map of metadata to store with the object in S3.
#' @param ObjectLockMode Indicates whether an object stored in Amazon S3 has Object Lock enabled. For more information about S3 Object Lock, see [Object Lock](https://docs.aws.amazon.com/AmazonS3/latest/userguide/object-lock.html).
#' @param ObjectLockLegalHoldStatus Indicates whether an object stored in Amazon S3 has an active legal hold.
...
#' @param ReplicationStatus Indicates if request involves bucket that is either a source or destination in a Replication rule. For more information about S3 Replication, see [Replication](https://docs.aws.amazon.com/AmazonS3/latest/userguide/replication.html).
...
#' @param ServerSideEncryption The server-side encryption algorithm used when storing requested object in Amazon S3 or Amazon FSx.
#' 
...
#' @param SSECustomerAlgorithm Encryption algorithm used if server-side encryption with a customer-provided encryption key was specified for object stored in Amazon S3.
#' @param SSEKMSKeyId If present, specifies the ID (Key ID, Key ARN, or Key Alias) of the Amazon Web Services Key Management Service (Amazon Web Services KMS) symmetric encryption customer managed key that was used for stored in Amazon S3 object.
#' @param SSECustomerKeyMD5 128-bit MD5 digest of customer-provided encryption key used in Amazon S3 to encrypt data stored in S3. For more information, see [Protecting data using server-side encryption with customer-provided encryption keys (SSE-C)](https://docs.aws.amazon.com/AmazonS3/latest/userguide/ServerSideEncryptionCustomerKeys.html).
#' @param StorageClass Provides storage class information of the object. Amazon S3 returns this header for all objects except for S3 Standard storage class objects.
#' 
#' For more information, see [Storage Classes](https://docs.aws.amazon.com/AmazonS3/latest/userguide/storage-class-intro.html).
...
#' @param BucketKeyEnabled Indicates whether the object stored in Amazon S3 uses an S3 bucket key for server-side encryption with Amazon Web Services KMS (SSE-KMS).
#'
#' @return
#' An empty list.
#'
