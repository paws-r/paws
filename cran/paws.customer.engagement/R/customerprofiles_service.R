...
#'  \link[=customerprofiles_create_upload_job]{create_upload_job} \tab Creates an Upload job to ingest data for segment imports\cr
...
#'  \link[=customerprofiles_delete_domain_layout]{delete_domain_layout} \tab Deletes the layout used to view data for a specific domain\cr
#'  \link[=customerprofiles_delete_domain_object_type]{delete_domain_object_type} \tab Delete a DomainObjectType for the given Domain and ObjectType name\cr
...
#'  \link[=customerprofiles_get_domain_layout]{get_domain_layout} \tab Gets the layout to view data for a specific domain\cr
#'  \link[=customerprofiles_get_domain_object_type]{get_domain_object_type} \tab Return a DomainObjectType for the input Domain and ObjectType names\cr
...
#'  \link[=customerprofiles_get_object_type_attribute_statistics]{get_object_type_attribute_statistics} \tab The GetObjectTypeAttributeValues API delivers statistical insights about attributes within a specific object type, but is exclusively available for domains with data store enabled\cr
#'  \link[=customerprofiles_get_profile_history_record]{get_profile_history_record} \tab Returns a history record for a specific profile, for a specific domain\cr
...
#'  \link[=customerprofiles_list_domain_layouts]{list_domain_layouts} \tab Lists the existing layouts that can be used to view data for a specific domain\cr
#'  \link[=customerprofiles_list_domain_object_types]{list_domain_object_types} \tab List all DomainObjectType(s) in a Customer Profiles domain\cr
...
#'  \link[=customerprofiles_list_object_type_attribute_values]{list_object_type_attribute_values} \tab The ListObjectTypeAttributeValues API provides access to the most recent distinct values for any specified attribute, making it valuable for real-time data validation and consistency checks within your object types\cr
...
#'  \link[=customerprofiles_list_profile_history_records]{list_profile_history_records} \tab Returns a list of history records for a specific profile, for a specific domain\cr
...
#'  \link[=customerprofiles_put_domain_object_type]{put_domain_object_type} \tab Create/Update a DomainObjectType in a Customer Profiles domain\cr
...
#'  \link[=customerprofiles_stop_recommender]{stop_recommender} \tab Stops a recommender, suspending its ability to generate recommendations\cr
#'  \link[=customerprofiles_stop_upload_job]{stop_upload_job} \tab This API stops the processing of an upload job\cr
...
#'  \link[=customerprofiles_update_domain_layout]{update_domain_layout} \tab Updates the layout used to view data for a specific domain\cr
#'  \link[=customerprofiles_update_event_trigger]{update_event_trigger} \tab Update the properties of an Event Trigger\cr
#'  \link[=customerprofiles_update_profile]{update_profile} \tab Updates the properties of a profile\cr
#'  \link[=customerprofiles_update_recommender]{update_recommender} \tab Updates the properties of an existing recommender, allowing you to modify its configuration and description
#' }
#'
#' @return
...
.customerprofiles$metadata <- list(
  ...
  endpoints = list("^(us|eu|ap|sa|ca|me|af|il|mx)\\-\\w+\\-\\d+$" = list(endpoint = "profile.{region}.amazonaws.com", global = FALSE), "^cn\\-\\w+\\-\\d+$" = list(endpoint = "profile.{region}.amazonaws.com.cn", global = FALSE), "^us\\-gov\\-\\w+\\-\\d+$" = list(endpoint = "profile.{region}.amazonaws.com", global = FALSE), "^us\\-iso\\-\\w+\\-\\d+$" = list(endpoint = "profile.{region}.c2s.ic.gov", global = FALSE), "^us\\-isob\\-\\w+\\-\\d+$" = list(endpoint = "profile.{region}.sc2s.sgov.gov", global = FALSE), "^eu\\-isoe\\-\\w+\\-\\d+$" = list(endpoint = "profile.{region}.cloud.adc-e.uk", global = FALSE), "^us\\-isof\\-\\w+\\-\\d+$" = list(endpoint = "profile.{region}.csp.hci.ic.gov", global = FALSE), "^eusc\\-(de)\\-\\w+\\-\\d+$" = list(endpoint = "profile.{region}.amazonaws.eu", global = FALSE)),
