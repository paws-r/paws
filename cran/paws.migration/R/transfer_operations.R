...
#' internet traffic, you will need to invoke the `UpdateServer` API and
#' attach an Elastic IP to your server\'s endpoint.
#' @param EndpointType The type of VPC endpoint that you want your SFTP server to connect to.
...
#' @param IdentityProviderDetails This parameter is required when the `IdentityProviderType` is set to
...
#' @param IdentityProviderType Specifies the mode of authentication for the SFTP server. The default
...
#' SFTP servers that have the `IdentityProviderType` set to
...
#' @param HomeDirectoryMappings Logical directory mappings that specify what S3 paths and keys should be
#' visible to your user and how you want to make them visible. You will
#' need to specify the \"`Entry`\" and \"`Target`\" pair, where `Entry`
#' shows how the path is made visible and `Target` is the actual S3 path.
...
#' entry will be ignored. As a workaround, you can use the S3 api to create
#' 0 byte objects as place holders for your directory. If using the CLI,
#' use the s3api call instead of s3 so you can use the put-object
...
#' @param UserName &#91;required&#93; This request parameter is the name of the user account to be tested.
...
#'
#' @param HomeDirectory A parameter that specifies the landing directory (folder) for a user
