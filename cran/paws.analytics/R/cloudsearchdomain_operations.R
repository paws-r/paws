...
#'   Ranges are specified using the same syntax that you use to search for
#'   a range of values. For more information, see [Searching for a Range of
...
#'   it cannot be used in conjunction with `buckets`.
#' 
...
#'   search term(s) you want to highlight. By default, the first occurrence
#'   is highlighted.
...
#' @param query &#91;required&#93; Specifies the search criteria for the request. How you specify the
...
#'   terms in the search string. For example: `defaultOperator: 'or'`. For
#'   the `dismax` parser, you specify a percentage that represents the
#'   percentage of terms in the search string (rounded down) that must
...
#'   the percent (%) symbol. For example, `defaultOperator: 50%`. Valid
#'   values: `and`, `or`, a percentage in the range 0%-100% (`dismax`).
#'   Default: `and` (`simple`, `structured`, `lucene`) or `100` (`dismax`).
...
#'   disabling `phrase` disables the ability to search for phrases by
#'   enclosing phrases in double quotes. Disabling precedence disables the
#'   ability to control order of precedence using parentheses. Disabling
#'   `near` disables the ability to use the ~ operator to perform a sloppy
#'   phrase search. Disabling the `fuzzy` operator disables the ability to
#'   use the ~ operator to perform a fuzzy search. `escape` disables the
#'   ability to use a backslash (`\`) to escape special characters within
#'   the search string. Disabling whitespace is an advanced option that
#'   prevents the parser from tokenizing on whitespace, which can be useful
#'   for Vietnamese. (It prevents Vietnamese words from being split
...
#'   `prefix`, `whitespace`. Default: All operators and special characters
#'   are enabled. Valid for: `simple`.
#' - `phraseFields`: An array of the `text` or `text-array` fields you want
#'   to use for phrase searches. When the terms in the search string appear
#'   in close proximity within a field, the field scores higher. You can
#'   specify a weight for each field to boost that score. The `phraseSlop`
...
#'   (`^`) symbol and the weight to the field name. For example, to boost
#'   phrase matches in the `title` field over the `abstract` field, you
#'   could specify: `"phraseFields":["title^3", "plot"]` Valid values: The
...
#'   can deviate from the search phrase when the phrase is enclosed in
#'   double quotes in the search string. (Phrases that exceed this
...
#'   the `queryOptions` parameter.) You can use the `-` (NOT), `|` (OR),
#'   and `*` (wildcard) operators to exclude particular terms, find results
#'   that match any of the specified terms, or search for a prefix. To
#'   search for a phrase rather than individual terms, enclose the phrase
#'   in double quotes. For more information, see [Searching for
...
#'   advanced options such as term boosting, `matchall`, and `near`. For
#'   more information, see [Constructing Compound
...
.cloudsearchdomain$operations$search <- cloudsearchdomain_search
...
.cloudsearchdomain$operations$suggest <- cloudsearchdomain_suggest
...
.cloudsearchdomain$operations$upload_documents <- cloudsearchdomain_upload_documents
