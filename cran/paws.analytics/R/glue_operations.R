...
#'   of memory) with 84GB disk (approximately 34GB free), and provides 1
#'   executor per worker. We recommend this worker type for workloads such
#'   as data transforms, joins, and queries, to offers a scalable and cost
#'   effective way to run most jobs.
#' 
#' - For the `G.2X` worker type, each worker maps to 2 DPU (8 vCPUs, 32 GB
#'   of memory) with 128GB disk (approximately 77GB free), and provides 1
#'   executor per worker. We recommend this worker type for workloads such
#'   as data transforms, joins, and queries, to offers a scalable and cost
#'   effective way to run most jobs.
#' 
...
#'   executor per worker. We recommend this worker type for jobs whose
...
#'   GB of memory) with 512GB disk (approximately 487GB free), and provides
#'   1 executor per worker. We recommend this worker type for jobs whose
#'   workloads contain your most demanding transforms, aggregations, joins,
#'   and queries. This worker type is available only for Glue version 3.0
#'   or later Spark ETL jobs, in the same Amazon Web Services Regions as
...
#'   executor per worker. We recommend this worker type for low volume
#'   streaming jobs. This worker type is only available for Glue version
#'   3.0 streaming jobs.
#' 
#' - For the `Z.2X` worker type, each worker maps to 2 M-DPU (8vCPUs, 64 GB
#'   of memory) with 128 GB disk (approximately 120GB free), and provides
...
#'   choice when you need to add fields or delete optional fields, and
#'   check compatibility against all previous schema versions.
#' 
...
#'   false for the default script generated by an Glue job. For example, to
...
#'   and nested column filtering to restrict access to columns and/or
