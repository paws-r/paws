...
make_shape_list <- function(shape, api, path) {
  ...
  if (!is_empty(member$locationName)) {
    proto <- add_tags(proto, list(locationNameList = member$locationName))
  }
...
make_shape_map <- function(shape, api, path) {
  ...
  if (!is_empty(key$locationName)) {
    proto <- add_tags(proto, list(locationNameKey = key$locationName))
  }
  if (!is_empty(value$locationName)) {
    proto <- add_tags(proto, list(locationNameValue = value$locationName))
  }
  return(proto)
}
