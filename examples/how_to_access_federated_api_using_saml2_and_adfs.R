...
adfs_saml2 <- function(adfs_url, region, username = NULL, password = NULL) {
  ...
  for (inputtag in xml_find_all(formsoup, ".//INPUT|.//input")) {
    name <- if (is.na(xml_attr(inputtag, "name"))) {
      ""
    } else {
      xml_attr(inputtag, "name")
    }
    value <- if (is.na(xml_attr(inputtag, "value"))) {
      ""
    } else {
      xml_attr(inputtag, "value")
    }
  ...
  if (is.na(SAMLResponse)) {
    stop('Response did not contain a valid SAML assertion')
  }
...
cred <- paws::sts()$assume_role_with_saml(
  RoleArn = obj$SamlRole["role_arn"],
