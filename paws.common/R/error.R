...
#'
#' This function generate S3 error objects which are passed to
#' [stop()] to generate classes errors.
...
#'   `statusCode` (numeric), `errorResponse` (list).
...
#'
#' @export
http_error <- function(e, call = sys.call(-1), use_call = FALSE) {
  full_message <- sprintf("%s (HTTP %d). %s", e$code, e$statusCode, e$message)

  status_type <- (e$statusCode %/% 100) * 100
  http_class <- paste0("http_", unique(c(e$statusCode, status_type, "error")))
  if(use_call){
    structure(
