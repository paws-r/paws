      ...
      switch (TYPEOF(val)) {
        ...
        case LGLSXP:
          if (Rf_length(val) > 0) {
            ...
            return (logical_val == TRUE) ? "TRUE" :
                   (logical_val == FALSE) ? "FALSE" : "";
          }
          break;
      }
...
std::string json_build_scalar(SEXP values) {
  ...
  if (len == 0) {
    ...
    return (type_tag == "list") ? "[]" :
           (type_tag == "structure" || type_tag == "map") ? "{}" : "";
  }
  ...
  if (t == "blob") {
    ...
    return (TYPEOF(first_element) == RAWSXP) ?
           "\"" + base64_encode(Rcpp::as<Rcpp::RawVector>(first_element)) + "\"" : "\"\"";
  }
...
std::string json_build_list(SEXP values) {
  ...
  if (n == 0) return "[]"; // Empty list is an empty JSON array
  ...
  if (elems.empty()) return "[]"; // If all elements were empty or filtered out
  ...
  for (size_t i = 0; i < elems.size(); ++i) {
    if (i > 0) result += ','; // Add comma separator
    result += elems[i];
  }
  result += ']';
...
std::string json_build_structure(SEXP values) {
  ...
  if (!payload_name.empty()) {
    Rcpp::Rcout << "payload_name: " << payload_name << std::endl;
  ...
  for (R_xlen_t i = 0; i < n; i++) {
    ...
    if (tag_get(val, "json") == "-" || // Tag to explicitly skip
        ...
        !tag_get(val, "ignore").empty()) { // General ignore tag
      continue;
    }
  ...
  if (fields.empty()) return "{}"; // If no valid fields remain
...
std::string json_build_any(SEXP values) {
  ...
  if (t == "structure") return json_build_structure(values);
  if (t == "list") return json_build_list(values);
  if (t == "map") return json_build_map(values);
